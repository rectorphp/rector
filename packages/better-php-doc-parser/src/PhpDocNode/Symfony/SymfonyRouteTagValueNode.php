...
final class SymfonyRouteTagValueNode extends AbstractTagValueNode implements ShortNameAwareTagInterface
{
    ...
    public function __toString(): string
    {
        ...
        if ($this->path) {
            $contentItems['path'] = $this->printValueWithOptionalQuotes('path', $this->path);
        } elseif ($this->localizedPaths) {
        ...
        if ($this->methods !== []) {
            $contentItems['methods'] = $this->printArrayItemWithSeparator($this->methods, 'methods', '=');
        }

        if ($this->options !== []) {
            $contentItems['options'] = $this->printArrayItemWithSeparator($this->options, 'options', '=');
        }

        if ($this->defaults !== []) {
            $contentItems['defaults'] = $this->printArrayItemWithSeparator($this->defaults, 'defaults', '=');
        }
