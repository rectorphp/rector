...
final class PhpDocInfo
{
    ...
    public function removeByType(string $type): void
    {
        ...
        foreach ($this->phpDocNode->children as $key => $child) {
            if (is_a($child, $type, true)) {
    ...
    {
        foreach (self::TAGS_TYPES_TO_NAMES as $tagValueNodeType => $self::TAGS_TYPES_TO_NAME) {
            if ($phpDocTagValueNode instanceof $tagValueNodeType) {
                return $self::TAGS_TYPES_TO_NAME;
            }
        }
