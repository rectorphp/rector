...
final class PHPStanNodeScopeResolver
{
    ...
    private function isTemplateExtendsInSource(array $nodes, string $currentFileName) : bool
    {
        return (bool) $this->betterNodeFinder->findFirst($nodes, function (\PhpParser\Node $node) use($currentFileName) : bool {
            ...
            return (bool) \RectorPrefix20211025\Nette\Utils\Strings::match($print, \Rector\Core\Application\FileProcessor::TEMPLATE_EXTENDS_REGEX);
        });
    }
    /**
     * @param Stmt[] $nodes
    ...
    private function processNodesWithMixinHandling(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, array $nodes, \PHPStan\Analyser\MutatingScope $mutatingScope, callable $nodeCallback) : array
    {
        ...
        try {
            ...
            if (!\RectorPrefix20211025\Nette\Utils\Strings::match($throwable->getMessage(), self::NOT_AN_INTERFACE_EXCEPTION_REGEX)) {
                throw $throwable;
            }
        }
    ...
    private function resolveClassOrInterfaceScope($classLike, \PHPStan\Analyser\MutatingScope $mutatingScope) : \PHPStan\Analyser\MutatingScope
    {
        ...
        if ($classLike instanceof \PhpParser\Node\Stmt\Class_ && \RectorPrefix20211025\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_START_REGEX)) {
