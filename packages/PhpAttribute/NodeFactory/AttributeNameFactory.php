    ...
    private function matchUseAliasMetadata(array $uses, \Rector\BetterPhpDocParser\PhpDoc\DoctrineAnnotationTagValueNode $doctrineAnnotationTagValueNode, \Rector\Php80\ValueObject\AnnotationToAttribute $annotationToAttribute) : ?\Rector\PhpAttribute\ValueObject\UseAliasMetadata
    {
        ...
        foreach ($uses as $use) {
            foreach ($use->uses as $useUse) {
                ...
                // previous keyword
                $lastImportKeyword = \RectorPrefix20220530\Nette\Utils\Strings::after($importName, '\\', -1);
                if ($lastImportKeyword === null) {
                    continue;
                }
                // resolve new short name
                $newShortname = \RectorPrefix20220530\Nette\Utils\Strings::after($annotationToAttribute->getAttributeClass(), $lastImportKeyword);
                $beforeImportName = \RectorPrefix20220530\Nette\Utils\Strings::before($annotationToAttribute->getAttributeClass(), $lastImportKeyword) . $lastImportKeyword;
