...
use RectorPrefix20220122\Symplify\EasyParallel\Enum\Content;
use RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactCommand;
use RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactEvent;
use RectorPrefix20220122\Symplify\EasyParallel\ValueObject\ParallelProcess;
use RectorPrefix20220122\Symplify\EasyParallel\ValueObject\ProcessPool;
use RectorPrefix20220122\Symplify\EasyParallel\ValueObject\Schedule;
...
final class ParallelFileProcessor
{
    ...
    public function __construct(\Rector\Parallel\Command\WorkerCommandLineFactory $workerCommandLineFactory, \RectorPrefix20220122\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider)
    {
     ...
     * @return mixed[]
     */
    public function process(\RectorPrefix20220122\Symplify\EasyParallel\ValueObject\Schedule $schedule, string $mainScript, \Closure $postFileCallback, \RectorPrefix20220122\Symfony\Component\Console\Input\InputInterface $input) : array
    {
        ...
        $systemErrors = [];
        $tcpServer = new \RectorPrefix20220122\React\Socket\TcpServer('127.0.0.1:0', $streamSelectLoop);
        $this->processPool = new \RectorPrefix20220122\Symplify\EasyParallel\ValueObject\ProcessPool($tcpServer);
        $tcpServer->on(\RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactEvent::CONNECTION, function (\RectorPrefix20220122\React\Socket\ConnectionInterface $connection) use(&$jobs) : void {
            $inDecoder = new \RectorPrefix20220122\Clue\React\NDJson\Decoder($connection, \true, 512, 0, 4 * 1024 * 1024);
            $outEncoder = new \RectorPrefix20220122\Clue\React\NDJson\Encoder($connection);
            $inDecoder->on(\RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactEvent::DATA, function (array $data) use(&$jobs, $inDecoder, $outEncoder) : void {
                $action = $data[\RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactCommand::ACTION];
                if ($action !== \RectorPrefix20220122\Symplify\EasyParallel\Enum\Action::HELLO) {
                    return;
                }
                ...
                $job = \array_pop($jobs);
                $parallelProcess->request([\RectorPrefix20220122\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \RectorPrefix20220122\Symplify\EasyParallel\Enum\Action::MAIN, \RectorPrefix20220122\Symplify\EasyParallel\Enum\Content::FILES => $job]);
            });
        });
        ...
        for ($i = 0; $i < $numberOfProcesses; ++$i) {
            ...
            $processIdentifier = \RectorPrefix20220122\Nette\Utils\Random::generate();
            $workerCommandLine = $this->workerCommandLineFactory->create($mainScript, \Rector\Core\Console\Command\ProcessCommand::class, \RectorPrefix20220122\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Rector\Core\Console\Command\WorkerCommand::class), $input, $processIdentifier, $serverPort);
            $parallelProcess = new \RectorPrefix20220122\Symplify\EasyParallel\ValueObject\ParallelProcess($workerCommandLine, $streamSelectLoop, $timeoutInSeconds);
            $parallelProcess->start(
                ...
                function ($exitCode, string $stdErr) use(&$systemErrors, $processIdentifier) : void {
                    ...
                    if ($exitCode === \RectorPrefix20220122\Symfony\Component\Console\Command\Command::SUCCESS) {
                        return;
                    }
                    if ($exitCode === null) {
                        return;
