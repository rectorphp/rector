...
use Closure;
use RectorPrefix20211209\Clue\React\NDJson\Decoder;
use RectorPrefix20211209\Clue\React\NDJson\Encoder;
use RectorPrefix20211209\Nette\Utils\Random;
use RectorPrefix20211209\React\EventLoop\StreamSelectLoop;
use RectorPrefix20211209\React\Socket\ConnectionInterface;
use RectorPrefix20211209\React\Socket\TcpServer;
...
use RectorPrefix20211209\Symplify\EasyParallel\Enum\Action;
use RectorPrefix20211209\Symplify\EasyParallel\Enum\Content;
use RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactCommand;
use RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactEvent;
use RectorPrefix20211209\Symplify\EasyParallel\ValueObject\ParallelProcess;
use RectorPrefix20211209\Symplify\EasyParallel\ValueObject\ProcessPool;
use RectorPrefix20211209\Symplify\EasyParallel\ValueObject\Schedule;
...
final class ParallelFileProcessor
{
     ...
     * @return mixed[]
     */
    public function check(\RectorPrefix20211209\Symplify\EasyParallel\ValueObject\Schedule $schedule, string $mainScript, \Closure $postFileCallback, ?string $projectConfigFile, \RectorPrefix20211209\Symfony\Component\Console\Input\InputInterface $input) : array
    {
        $jobs = \array_reverse($schedule->getJobs());
        $streamSelectLoop = new \RectorPrefix20211209\React\EventLoop\StreamSelectLoop();
        // basic properties setup
        $numberOfProcesses = $schedule->getNumberOfProcesses();
        // initial counters
        $fileDiffs = [];
        $systemErrors = [];
        $tcpServer = new \RectorPrefix20211209\React\Socket\TcpServer('127.0.0.1:0', $streamSelectLoop);
        $this->processPool = new \RectorPrefix20211209\Symplify\EasyParallel\ValueObject\ProcessPool($tcpServer);
        $tcpServer->on(\RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactEvent::CONNECTION, function (\RectorPrefix20211209\React\Socket\ConnectionInterface $connection) use(&$jobs) : void {
            $inDecoder = new \RectorPrefix20211209\Clue\React\NDJson\Decoder($connection, \true, 512, 0, 4 * 1024 * 1024);
            $outEncoder = new \RectorPrefix20211209\Clue\React\NDJson\Encoder($connection);
            $inDecoder->on(\RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactEvent::DATA, function (array $data) use(&$jobs, $inDecoder, $outEncoder) : void {
                $action = $data[\RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactCommand::ACTION];
                if ($action !== \RectorPrefix20211209\Symplify\EasyParallel\Enum\Action::HELLO) {
                    return;
                }
                ...
                $job = \array_pop($jobs);
                $parallelProcess->request([\RectorPrefix20211209\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \RectorPrefix20211209\Symplify\EasyParallel\Enum\Action::MAIN, \RectorPrefix20211209\Symplify\EasyParallel\Enum\Content::FILES => $job]);
            });
        });
        ...
        for ($i = 0; $i < $numberOfProcesses; ++$i) {
            ...
            $processIdentifier = \RectorPrefix20211209\Nette\Utils\Random::generate();
            $workerCommandLine = $this->workerCommandLineFactory->create($mainScript, \RectorPrefix20211209\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Rector\Core\Console\Command\ProcessCommand::class), \RectorPrefix20211209\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Rector\Core\Console\Command\WorkerCommand::class), $projectConfigFile, $input, $processIdentifier, $serverPort);
            $parallelProcess = new \RectorPrefix20211209\Symplify\EasyParallel\ValueObject\ParallelProcess($workerCommandLine, $streamSelectLoop, self::TIMEOUT_IN_SECONDS);
            $parallelProcess->start(
                ...
                function ($exitCode, string $stdErr) use(&$systemErrors, $processIdentifier) : void {
                    ...
                    if ($exitCode === \RectorPrefix20211209\Symfony\Component\Console\Command\Command::SUCCESS) {
                        return;
                    }
                    if ($exitCode === null) {
                        return;
