...
final class MissedRectorDueVersionChecker
{
    ...
    public function __construct(\Rector\Core\Php\PhpVersionProvider $phpVersionProvider, \RectorPrefix20210802\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle)
    {
    ...
    private function reportMissedRectors(array $minPhpVersions) : void
    {
        ...
        foreach ($minPhpVersions as $minPhpVersion) {
            ...
            $shortRectorClass = \RectorPrefix20210802\Nette\Utils\Strings::after(\get_class($minPhpVersion), '\\', -1);
    ...
    private function reportWarningMessage(int $minProjectPhpVersion, array $minPhpVersions) : void
    {
        ...
        $warningMessage = \sprintf('Your project requires min PHP version "%s".%s%d%sSome Rectors rules defined in your configuration require higher PHP version and will not run,%sto avoid breaking your codebase.', $phpVersion->getVersionString(), \count($minPhpVersions), \PHP_EOL, \PHP_EOL . \PHP_EOL, \PHP_EOL);
