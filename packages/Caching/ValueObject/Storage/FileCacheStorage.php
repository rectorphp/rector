...
use RectorPrefix20211026\Nette\Utils\Random;
use PHPStan\File\FileWriter;
use Rector\Caching\Contract\ValueObject\Storage\CacheStorageInterface;
use Rector\Caching\ValueObject\CacheFilePaths;
use Rector\Caching\ValueObject\CacheItem;
use RectorPrefix20211026\Symplify\EasyCodingStandard\Caching\Exception\CachingException;
...
final class FileCacheStorage implements \Rector\Caching\Contract\ValueObject\Storage\CacheStorageInterface
{
    ...
    public function __construct(string $directory, \RectorPrefix20211026\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem)
    {
    ...
    public function save($key, $variableKey, $data) : void
    {
        ...
        $tmpPath = \sprintf('%s/%s.tmp', $this->directory, \RectorPrefix20211026\Nette\Utils\Random::generate());
        $errorBefore = \error_get_last();
        $exported = @\var_export(new \Rector\Caching\ValueObject\CacheItem($variableKey, $data), \true);
        $errorAfter = \error_get_last();
        if ($errorAfter !== null && $errorBefore !== $errorAfter) {
            throw new \RectorPrefix20211026\Symplify\EasyCodingStandard\Caching\Exception\CachingException(\sprintf('Error occurred while saving item %s (%s) to cache: %s', $key, $variableKey, $errorAfter['message']));
        }
        ...
        if (\DIRECTORY_SEPARATOR === '/' || !\file_exists($path)) {
            throw new \RectorPrefix20211026\Symplify\EasyCodingStandard\Caching\Exception\CachingException(\sprintf('Could not write data to cache file %s.', $path));
        }
    }
    /**
     * @param string $key
