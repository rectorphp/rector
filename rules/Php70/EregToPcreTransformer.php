...
final class EregToPcreTransformer
{
    ...
    private function processCurlyBracket(string $s, int $i, array &$r, int $rr) : int
    {
        ...
        $start = $i + 1;
        $length = $ii - ($i + 1);
        $bound = \RectorPrefix20210605\Nette\Utils\Strings::substring($s, $start, $length);
        $matches = \RectorPrefix20210605\Nette\Utils\Strings::match($bound, self::BOUND_REGEX);
        if ($matches === null) {
    ...
    private function processCharacterClass(string $content, int $i, string $cls) : array
    {
        ...
        $start = $i + 1;
        $length = $ii - ($i + 1);
        $ccls = \RectorPrefix20210605\Nette\Utils\Strings::substring($content, $start, $length);
