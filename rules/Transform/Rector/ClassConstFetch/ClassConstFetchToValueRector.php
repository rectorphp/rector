...
final class ClassConstFetchToValueRector extends AbstractRector implements ConfigurableRectorInterface
{
    ...
    {
        $configuration = [self::CLASS_CONST_FETCHES_TO_VALUES => [new ClassConstFetchToValue('Nette\\Configurator', 'DEVELOPMENT', 'development'), new ClassConstFetchToValue('Nette\\Configurator', 'PRODUCTION', 'production')]];
        return new RuleDefinition('Replaces constant by value', [new ConfiguredCodeSample('$value === Nette\\Configurator::DEVELOPMENT', '$value === "development"', $configuration)]);
    }
    /**
