...
use RectorPrefix20220201\Doctrine\Inflector\Inflector;
...
final class InflectorSingularResolver
{
     ...
     */
    private $inflector;
    public function __construct(\RectorPrefix20220201\Doctrine\Inflector\Inflector $inflector)
    {
        $this->inflector = $inflector;
    }
    public function resolve(string $currentName) : string
    {
        $matchBy = \RectorPrefix20220201\Nette\Utils\Strings::match($currentName, self::BY_MIDDLE_REGEX);
        if ($matchBy !== null) {
            return \RectorPrefix20220201\Nette\Utils\Strings::substring($currentName, 0, -\strlen((string) $matchBy['by']));
        }
    ...
    private function resolveSingularizeMap(string $currentName)
    {
        foreach (self::SINGULARIZE_MAP as $plural => $singular) {
            ...
            if (\Rector\Core\Util\StringUtils::isMatch($currentName, '#' . \ucfirst($plural) . '#')) {
                $resolvedValue = \RectorPrefix20220201\Nette\Utils\Strings::replace($currentName, '#' . \ucfirst($plural) . '#', \ucfirst($singular));
                return $this->singularizeCamelParts($resolvedValue);
            }
            if (\Rector\Core\Util\StringUtils::isMatch($currentName, '#' . $plural . '#')) {
                $resolvedValue = \RectorPrefix20220201\Nette\Utils\Strings::replace($currentName, '#' . $plural . '#', $singular);
                return $this->singularizeCamelParts($resolvedValue);
            }
        }
        return null;
    ...
    {
        $camelCases = \RectorPrefix20220201\Nette\Utils\Strings::matchAll($currentName, self::CAMELCASE_REGEX);
        $resolvedName = '';
