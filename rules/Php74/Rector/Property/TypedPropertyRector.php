...
final class TypedPropertyRector extends AbstractRector implements ConfigurableRectorInterface
{
    ...
    public function __construct(PropertyTypeInferer $propertyTypeInferer, VendorLockResolver $vendorLockResolver, DoctrineTypeAnalyzer $doctrineTypeAnalyzer, VarTagRemover $varTagRemover, ReflectionProvider $reflectionProvider)
    {
    ...
    {
        return new RuleDefinition('Changes property `@var` annotations from annotation to type.', [new ConfiguredCodeSample(<<<'CODE_SAMPLE'
    ...
    public function refactor(Node $node) : ?Node
    {
        ...
        if ($varType instanceof UnionType) {
            ...
            if (\count($types) === 2 && $types[0] instanceof TemplateType) {
                $node->type = $this->staticTypeMapper->mapPHPStanTypeToPhpParserNode($types[0]->getBound(), TypeKind::KIND_PROPERTY);
                return $node;
            }
        }
