...
final class RegexMatcher
{
    ...
    public function resolvePatternExpressionWithoutEIfFound(\PhpParser\Node\Expr $expr) : ?\PhpParser\Node\Expr
    {
        if ($expr instanceof \PhpParser\Node\Scalar\String_) {
            ...
            $modifiers = \RectorPrefix20211017\Nette\Utils\Strings::after($pattern, $delimiter, -1);
    ...
    {
        $modifiersWithoutE = \RectorPrefix20211017\Nette\Utils\Strings::replace($modifiers, '#e#', '');
        return \RectorPrefix20211017\Nette\Utils\Strings::before($pattern, $delimiter, -1) . $delimiter . $modifiersWithoutE;
    }
    private function matchConcat(\PhpParser\Node\Expr\BinaryOp\Concat $concat) : ?\PhpParser\Node\Expr
    {
        ...
        $matches = \RectorPrefix20211017\Nette\Utils\Strings::match($lastItem->value, self::LETTER_SUFFIX_REGEX);
        ...
        $lastItem->value = \RectorPrefix20211017\Nette\Utils\Strings::replace($lastItem->value, self::LAST_E_REGEX, '$1$2');
        return $concat;
    }
}
