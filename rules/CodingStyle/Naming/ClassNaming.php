...
final class ClassNaming
{
    ...
    public function getShortName($name) : string
    {
        ...
        $name = \trim($name, '\\');
        return \RectorPrefix20211015\Nette\Utils\Strings::after($name, '\\', -1) ?: $name;
    }
    public function getNamespace(string $fullyQualifiedName) : ?string
    {
        $fullyQualifiedName = \trim($fullyQualifiedName, '\\');
        return \RectorPrefix20211015\Nette\Utils\Strings::before($fullyQualifiedName, '\\', -1) ?: null;
    }
    public function getNameFromFileInfo(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : string
    {
        ...
        if (\Rector\Testing\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            $basenameWithoutSuffix = \RectorPrefix20211015\Nette\Utils\Strings::replace($basenameWithoutSuffix, self::INPUT_HASH_NAMING_REGEX, '');
        }
        $stringy = new \RectorPrefix20211015\Stringy\Stringy($basenameWithoutSuffix);
    ...
    public function createMethodNameFromFunction(\PhpParser\Node\Stmt\Function_ $function) : string
    {
        ...
        $stringy = new \RectorPrefix20211015\Stringy\Stringy($functionName);
    ...
    public function replaceSuffix(string $content, string $oldSuffix, string $newSuffix) : string
    {
        ...
        $contentWithoutOldSuffix = \RectorPrefix20211015\Nette\Utils\Strings::substring($content, 0, -\RectorPrefix20211015\Nette\Utils\Strings::length($oldSuffix));
