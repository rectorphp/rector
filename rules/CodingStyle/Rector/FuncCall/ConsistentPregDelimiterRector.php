...
, [self::DELIMITER => '#'])]);
    ...
    {
        $matchInnerRegex = \RectorPrefix20220524\Nette\Utils\Strings::match($string, self::INNER_REGEX);
        $matchInnerUnionRegex = \RectorPrefix20220524\Nette\Utils\Strings::match($string, self::INNER_UNICODE_REGEX);
    ...
    private function refactorArgument($node, \PhpParser\Node\Arg $arg)
    {
        ...
        $string->value = \RectorPrefix20220524\Nette\Utils\Strings::replace($string->value, self::INNER_REGEX, function (array $match) use(&$string) : string {
