...
, [self::DELIMITER => '#'])]);
    ...
    private function refactorArgument(\PhpParser\Node\Arg $arg) : void
    {
        ...
        /** @var String_ $string */
        $string = $arg->value;
        $string->value = \RectorPrefix20211022\Nette\Utils\Strings::replace($string->value, self::INNER_REGEX, function (array $match) use(&$string) : string {
            $printedString = $this->betterStandardPrinter->print($string);
            if (\RectorPrefix20211022\Nette\Utils\Strings::match($printedString, self::DOUBLE_QUOTED_REGEX)) {
