...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (\strncmp($includeValue, './', \strlen('./')) === 0) {
            $node->expr->value = \RectorPrefix20220411\Nette\Utils\Strings::substring($includeValue, 1);
        } else {
    ...
    private function removeExtraDotSlash(\PhpParser\Node\Scalar\String_ $string) : void
    {
        ...
        $string->value = \RectorPrefix20220411\Nette\Utils\Strings::replace($string->value, '#^\\.\\/#', '/');
    }
