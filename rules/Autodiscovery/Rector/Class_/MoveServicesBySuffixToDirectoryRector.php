...
, [self::GROUP_NAMES_BY_SUFFIX => ['Repository']])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20210731\Webmozart\Assert\Assert::allString($groupNamesBySuffix);
    ...
    private function processGroupNamesBySuffix(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, \Rector\Core\ValueObject\Application\File $file, array $groupNamesBySuffix) : void
    {
        foreach ($groupNamesBySuffix as $groupNames) {
            ...
            if (!\RectorPrefix20210731\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }
            ...
            if (\RectorPrefix20210731\Nette\Utils\Strings::match($smartFileInfo->getPath(), '#' . $groupNames . '$#')) {
                continue;
            }
