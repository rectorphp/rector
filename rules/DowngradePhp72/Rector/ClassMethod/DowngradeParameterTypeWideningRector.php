...
, [self::SAFE_TYPES => [], self::SAFE_TYPES_TO_METHODS => []])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20211031\Webmozart\Assert\Assert::allString($safeTypes);
        $this->safeTypes = $safeTypes;
        $safeTypesToMethods = $configuration[self::SAFE_TYPES_TO_METHODS] ?? [];
        \RectorPrefix20211031\Webmozart\Assert\Assert::isArray($safeTypesToMethods);
        foreach ($safeTypesToMethods as $key => $value) {
            \RectorPrefix20211031\Webmozart\Assert\Assert::string($key);
            \RectorPrefix20211031\Webmozart\Assert\Assert::allString($value);
        }
