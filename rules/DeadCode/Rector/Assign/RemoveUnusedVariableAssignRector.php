...
final class RemoveUnusedVariableAssignRector extends AbstractRector
{
    ...
    public function __construct(ReservedKeywordAnalyzer $reservedKeywordAnalyzer, CompactFuncCallAnalyzer $compactFuncCallAnalyzer, ConditionSearcher $conditionSearcher, UsedVariableNameAnalyzer $usedVariableNameAnalyzer)
    {
...
CODE_SAMPLE
    ...
    private function shouldSkip(Assign $assign) : bool
    {
        ...
        return $variable->name instanceof Variable && $this->betterNodeFinder->findFirstNext($assign, function (Node $node) : bool {
            return $node instanceof Variable;
        });
    }
    private function isUsed(Assign $assign, Variable $variable) : bool
    {
        $isUsedPrev = (bool) $this->betterNodeFinder->findFirstPreviousOfNode($variable, function (Node $node) use($variable) : bool {
    ...
    private function isUsedInAssignExpr(Expr $expr, Assign $assign) : bool
    {
        foreach ($expr->args as $arg) {
            ...
            $previousAssign = $this->betterNodeFinder->findFirstPreviousOfNode($assign, function (Node $node) use($variable) : bool {
                return $node instanceof Assign && $this->usedVariableNameAnalyzer->isVariableNamed($node->var, $variable);
            });
