...
$autoloadIncluder = new \RectorPrefix20220415\AutoloadIncluder();
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20220415\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $rectorConsoleOutputStyleFactory = new \Rector\Core\Console\Style\RectorConsoleOutputStyleFactory(new \RectorPrefix20220415\Symplify\PackageBuilder\Reflection\PrivatesCaller());
...
\class_alias('RectorPrefix20220415\\AutoloadIncluder', 'AutoloadIncluder', \false);
