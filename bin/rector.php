...
$autoloadIncluder = new \RectorPrefix20210817\AutoloadIncluder();
...
} elseif (\RectorPrefix20210817\should_include_preload()) {
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20210817\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $symfonyStyleFactory = new \Rector\Core\Console\Style\SymfonyStyleFactory(new \RectorPrefix20210817\Symplify\PackageBuilder\Reflection\PrivatesCaller());
        $symfonyStyle = $symfonyStyleFactory->create();
        $symfonyStyle->error($throwable->getMessage());
    }
    exit(\RectorPrefix20210817\Symplify\PackageBuilder\Console\ShellCode::ERROR);
}
...
\class_alias('RectorPrefix20210817\\AutoloadIncluder', 'AutoloadIncluder', \false);
