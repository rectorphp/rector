...
$autoloadIncluder = new \RectorPrefix20211010\AutoloadIncluder();
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20211010\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $symfonyStyleFactory = new \Rector\Core\Console\Style\SymfonyStyleFactory(new \RectorPrefix20211010\Symplify\PackageBuilder\Reflection\PrivatesCaller());
...
\class_alias('RectorPrefix20211010\\AutoloadIncluder', 'AutoloadIncluder', \false);
