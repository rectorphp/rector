...
$autoloadIncluder = new \RectorPrefix20220401\AutoloadIncluder();
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20220401\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $rectorConsoleOutputStyleFactory = new \Rector\Core\Console\Style\RectorConsoleOutputStyleFactory(new \RectorPrefix20220401\Symplify\PackageBuilder\Reflection\PrivatesCaller());
...
\class_alias('RectorPrefix20220401\\AutoloadIncluder', 'AutoloadIncluder', \false);
