...
$autoloadIncluder = new \RectorPrefix20211221\AutoloadIncluder();
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20211221\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $symfonyStyleFactory = new \Rector\Core\Console\Style\SymfonyStyleFactory(new \RectorPrefix20211221\Symplify\PackageBuilder\Reflection\PrivatesCaller());
        $symfonyStyle = $symfonyStyleFactory->create();
        $symfonyStyle->error($throwable->getMessage());
    }
    exit(\RectorPrefix20211221\Symfony\Component\Console\Command\Command::FAILURE);
}
...
\class_alias('RectorPrefix20211221\\AutoloadIncluder', 'AutoloadIncluder', \false);
