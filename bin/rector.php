...
$autoloadIncluder = new \RectorPrefix20210909\AutoloadIncluder();
...
} elseif (\RectorPrefix20210909\should_include_preload()) {
...
try {
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20210909\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $symfonyStyleFactory = new \Rector\Core\Console\Style\SymfonyStyleFactory(new \RectorPrefix20210909\Symplify\PackageBuilder\Reflection\PrivatesCaller());
...
\class_alias('RectorPrefix20210909\\AutoloadIncluder', 'AutoloadIncluder', \false);
