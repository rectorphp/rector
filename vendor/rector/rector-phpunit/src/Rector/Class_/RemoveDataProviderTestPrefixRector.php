...
CODE_SAMPLE
    ...
    private function renameDataProviderAnnotationsAndCollectRenamedMethods(\PhpParser\Node\Stmt\Class_ $class) : void
    {
        foreach ($class->getMethods() as $classMethod) {
            ...
            foreach ($dataProviderTagValueNodes as $dataProviderTagValueNode) {
                ...
                $dataProviderTagValueNode->value->value = \RectorPrefix20220105\Nette\Utils\Strings::replace($oldMethodName, '#' . \preg_quote($oldMethodName, '#') . '#', $newMethodName);
                // invoke reprint
    ...
    {
        $newMethodName = \RectorPrefix20220105\Nette\Utils\Strings::substring($oldMethodName, \strlen('test'));
