...
use RectorPrefix20220521\Symfony\Component\DependencyInjection\Loader\XmlFileLoader;
use RectorPrefix20220521\Symfony\Component\DependencyInjection\Loader\YamlFileLoader;
...
, [self::FROM => 'xml', self::TO => 'yaml'])]);
    ...
    private function replaceSuffix(\PhpParser\Node\Expr\MethodCall $methodCall, string $from, string $to) : void
    {
        ...
        $this->traverseNodesWithCallable([$fileArgument], function (\PhpParser\Node $node) use($from, $to) : ?Node {
            ...
            $node->value = \RectorPrefix20220521\Nette\Utils\Strings::replace($node->value, '#\\.' . $from . '$#', '.' . $to);
            return $node;
        });
    }
}
