...
final class MissingReturnTypeParser
{
    ...
    private function resolveDiffFileToReturnTypeChanges(string $fileDiffPath) : array
    {
        ...
        foreach ($fileDiffs as $fileDiff) {
            ...
            // match file name
            $filenameMatch = \RectorPrefix20220101\Nette\Utils\Strings::match($matches[0]['before'], '# a/src/(?<filename>.*?).php$#');
            ...
            foreach ($matches as $match) {
                // match method name
                $methodNameMatch = \RectorPrefix20220101\Nette\Utils\Strings::match($match['before'], '#(?<method_name>\\w+)\\(#');
                $newTypeMatch = \RectorPrefix20220101\Nette\Utils\Strings::match($match['after'], '#\\): (?<return_type>.*?);?$#');
