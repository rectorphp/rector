...
final class MissingReturnTypeParser
{
    ...
    {
        $diffFileContent = \RectorPrefix20220531\Nette\Utils\FileSystem::read($fileDiffPath);
        $fileDiffs = \explode('diff --git', $diffFileContent);
        $returnTypeChanges = [];
        foreach ($fileDiffs as $fileDiff) {
            $matches = \RectorPrefix20220531\Nette\Utils\Strings::matchAll($fileDiff, self::DIFF_LINES_REGEX);
            if ($matches === []) {
                continue;
            }
            // match file name
            $filenameMatch = \RectorPrefix20220531\Nette\Utils\Strings::match($matches[0]['before'], '# a/src/(?<filename>.*?).php$#');
            ...
            foreach ($matches as $match) {
                // match method name
                $methodNameMatch = \RectorPrefix20220531\Nette\Utils\Strings::match($match['before'], '#(?<method_name>\\w+)\\(#');
                $newTypeMatch = \RectorPrefix20220531\Nette\Utils\Strings::match($match['after'], '#\\): (?<return_type>.*?);?$#');
...
$missingReturnTypeParser = new \RectorPrefix20220531\MissingReturnTypeParser();
