...
, [self::NAMESPACE => 'App\\Http\\Controllers'])]);
    ...
    public function configure($configuration) : void
    {
        ...
        \RectorPrefix20211210\Webmozart\Assert\Assert::isArray($routes);
        \RectorPrefix20211210\Webmozart\Assert\Assert::allString(\array_keys($routes));
        \RectorPrefix20211210\Webmozart\Assert\Assert::allString($routes);
        $this->routes = $routes;
        $namespace = $configuration[self::NAMESPACE] ?? self::DEFAULT_NAMESPACE;
        \RectorPrefix20211210\Webmozart\Assert\Assert::string($namespace);
