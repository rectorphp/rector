...
final class CakePHPFullyQualifiedClassNameResolver
{
    ...
    public function resolveFromPseudoNamespaceAndShortClassName(string $pseudoNamespace, string $shortClass) : string
    {
        ...
        if (\RectorPrefix20210715\Nette\Utils\Strings::match($pseudoNamespace, self::LIB_NAMESPACE_PART_REGEX)) {
            $pseudoNamespace = \RectorPrefix20210715\Nette\Utils\Strings::replace($pseudoNamespace, '#\\\\Lib#', '');
        }
        ...
        if (\RectorPrefix20210715\Nette\Utils\Strings::contains($pseudoNamespace, '\\') && !\RectorPrefix20210715\Nette\Utils\Strings::match($pseudoNamespace, self::PLUGIN_OR_LIB_REGEX)) {
    ...
    {
        return \RectorPrefix20210715\Nette\Utils\Strings::replace($pseudoNamespace, self::SLASH_REGEX, '\\');
    }
}
