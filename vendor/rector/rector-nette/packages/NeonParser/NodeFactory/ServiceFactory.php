...
final class ServiceFactory
{
     ...
     */
    public function create(\RectorPrefix20220605\Nette\Neon\Node $node)
    {
        if (!$node instanceof \RectorPrefix20220605\Nette\Neon\Node\ArrayItemNode) {
            return null;
        }
     ...
     */
    private function resolveArrayItemByKeyword(\RectorPrefix20220605\Nette\Neon\Node\ArrayItemNode $arrayItemNode, string $keyword)
    {
        if (!$arrayItemNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\ArrayNode) {
            return null;
        }
        $arrayNode = $arrayItemNode->value;
        foreach ($arrayNode->items as $arrayItemNode) {
            if (!$arrayItemNode->key instanceof \RectorPrefix20220605\Nette\Neon\Node\LiteralNode) {
                continue;
            }
            if ($arrayItemNode->key->toString() !== $keyword) {
                continue;
            }
            if ($arrayItemNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\EntityNode) {
                return $arrayItemNode->value->value;
            }
            if ($arrayItemNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\LiteralNode) {
     ...
     */
    private function resolveSetupMethodCalls(string $className, \RectorPrefix20220605\Nette\Neon\Node\ArrayItemNode $arrayItemNode) : array
    {
        if (!$arrayItemNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\ArrayNode) {
            return [];
        }
        ...
        foreach ($arrayNode->items as $arrayItemNode) {
            if ($arrayItemNode->key instanceof \RectorPrefix20220605\Nette\Neon\Node\LiteralNode) {
                ...
                foreach ($arrayItemNode->value->items as $setupArrayItemNode) {
                    if ($setupArrayItemNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\EntityNode) {
                        // probably method call
                        $entityNode = $setupArrayItemNode->value;
                        if ($entityNode->value instanceof \RectorPrefix20220605\Nette\Neon\Node\LiteralNode) {
