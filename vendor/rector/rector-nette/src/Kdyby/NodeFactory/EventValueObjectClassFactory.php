...
use RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder;
use RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder;
use RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder;
/**
...
final class EventValueObjectClassFactory
{
    ...
    private function createEventClassBuilder(string $className) : \RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder
    {
        $shortClassName = $this->classNaming->getShortName($className);
        $classBuilder = new \RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder($shortClassName);
     ...
     * @param Arg[] $args
     */
    private function decorateWithConstructorIfHasArgs(\RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder, array $args) : void
    {
        if ($args === []) {
            return;
        }
    ...
    private function wrapClassToNamespace(string $className, \PhpParser\Node\Stmt\Class_ $class) : \PhpParser\Node\Stmt\Namespace_
    {
        ...
        $namespaceBuilder = new \RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder($namespace);
     ...
     */
    private function ensureVariablesAreUnique(array $variablesWithTypes, \RectorPrefix20210907\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder) : void
    {
        $usedVariableNames = [];
