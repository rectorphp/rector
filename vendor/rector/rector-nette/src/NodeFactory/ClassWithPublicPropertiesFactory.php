...
use RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder;
use RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder;
use RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder;
use RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\TraitUseBuilder;
/**
...
final class ClassWithPublicPropertiesFactory
{
    ...
    public function createNode(string $fullyQualifiedName, array $properties, ?string $parent, array $traits)
    {
        ...
        if ($namespace !== '') {
            $namespaceBuilder = new \RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder($namespace);
        }
        $classBuilder = new \RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder($className);
        if ($parent !== null && $parent !== '') {
            $classBuilder->extend($this->fixFullyQualifiedName($parent));
        }
        foreach ($traits as $trait) {
            $classBuilder->addStmt(new \RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\TraitUseBuilder($this->fixFullyQualifiedName($trait)));
        }
        foreach ($properties as $propertyName => $propertySettings) {
            ...
            $propertyBuilder = new \RectorPrefix20211224\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder($propertyName);
