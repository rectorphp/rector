...
final class TemplateTypeBasedOnPresenterTemplateParametersRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
     ...
     */
    private $nodePrinter;
    public function __construct(\Rector\Nette\NodeFactory\ClassWithPublicPropertiesFactory $classWithPublicPropertiesFactory, \Rector\Core\Contract\PhpParser\NodePrinterInterface $nodePrinter)
    {
...
, [self::TEMPLATE_CLASS_PARENT => '', self::TEMPLATE_CLASS_TRAITS => []])]);
    ...
    public function configure(array $configuration) : void
    {
        if (isset($configuration[self::TEMPLATE_CLASS_PARENT])) {
            ...
            \RectorPrefix20220409\Webmozart\Assert\Assert::string($templateClassParent);
        ...
        if (isset($configuration[self::TEMPLATE_CLASS_TRAITS])) {
            ...
            \RectorPrefix20220409\Webmozart\Assert\Assert::isArray($templateClassTraits);
    ...
    private function createTemplateClass(string $presenterName, string $fullPresenterName, string $actionName, array $varTypes) : string
    {
        ...
        $fullTemplateClassName = '\\' . \RectorPrefix20220409\Nette\Utils\Strings::replace($fullPresenterName, $presenterPattern, $upperCasedActionName . 'Template');
