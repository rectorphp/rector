...
use RectorPrefix20210809\Tracy\Debugger;
// session is required for this functionality
\session_start();
// For security reasons, Tracy is visible only on localhost.
// You may force Tracy to run in development mode by passing the Debugger::DEVELOPMENT instead of Debugger::DETECT.
\RectorPrefix20210809\Tracy\Debugger::enable(\RectorPrefix20210809\Tracy\Debugger::DETECT, __DIR__ . '/log');
if (isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    // AJAX request
    \RectorPrefix20210809\bdump('AJAX request ' . \date('H:i:s'));
    if (!empty($_GET['error'])) {
        \RectorPrefix20210809\this_is_fatal_error();
    }
    $data = [\rand(), \rand(), \rand()];
...
\RectorPrefix20210809\bdump('classic request ' . \date('H:i:s'));
?>
...
<?php 
if (\RectorPrefix20210809\Tracy\Debugger::$productionMode) {
