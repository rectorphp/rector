...
namespace RectorPrefix20210705\Tracy\Bridges\Nette;

use RectorPrefix20210705\Latte;
use RectorPrefix20210705\Nette;
use RectorPrefix20210705\Tracy;
use RectorPrefix20210705\Tracy\BlueScreen;
use RectorPrefix20210705\Tracy\Helpers;
/**
...
class Bridge
{
    ...
    {
        $blueScreen = \RectorPrefix20210705\Tracy\Debugger::getBlueScreen();
    ...
    public static function renderLatteError(?\Throwable $e) : ?array
    {
        if ($e instanceof \RectorPrefix20210705\Latte\CompileException && $e->sourceName) {
            ...
            if (\preg_match('#// source: (\\S+\\.latte)#', $lines[1], $m) && @\is_file($m[1])) {
                ...
                return ['tab' => 'Template', 'panel' => '<p><b>File:</b> ' . \RectorPrefix20210705\Tracy\Helpers::editorLink($templateFile, $templateLine) . '</p>' . ($templateLine === null ? '' : \RectorPrefix20210705\Tracy\BlueScreen::highlightFile($templateFile, $templateLine))];
            }
        }
        return null;
    }
    ...
    public static function renderMemberAccessException(?\Throwable $e) : ?array
    {
        ...
        $loc = $e->getTrace()[$e instanceof \RectorPrefix20210705\Nette\MemberAccessException ? 1 : 0];
        if (\preg_match('#Cannot (?:read|write to) an undeclared property .+::\\$(\\w+), did you mean \\$(\\w+)\\?#A', $e->getMessage(), $m)) {
            return ['link' => \RectorPrefix20210705\Tracy\Helpers::editorUri($loc['file'], $loc['line'], 'fix', '->' . $m[1], '->' . $m[2]), 'label' => 'fix it'];
        } elseif (\preg_match('#Call to undefined (static )?method .+::(\\w+)\\(\\), did you mean (\\w+)\\(\\)?#A', $e->getMessage(), $m)) {
            $operator = $m[1] ? '::' : '->';
            return ['link' => \RectorPrefix20210705\Tracy\Helpers::editorUri($loc['file'], $loc['line'], 'fix', $operator . $m[2] . '(', $operator . $m[3] . '('), 'label' => 'fix it'];
        }
        return null;
    }
    ...
    private static function highlightNeon(string $code, int $line) : string
    {
        ...
        $code = \str_replace("\t", "<span class='tracy-dump-whitespace'>→   </span>", $code);
        return '<pre class=code><div>' . \RectorPrefix20210705\Tracy\BlueScreen::highlightLine($code, $line) . '</div></pre>';
    }
}
