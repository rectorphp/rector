...
namespace RectorPrefix20211210\Tracy\Bridges\Nette;

use RectorPrefix20211210\Latte;
use RectorPrefix20211210\Nette;
use RectorPrefix20211210\Tracy;
use RectorPrefix20211210\Tracy\BlueScreen;
use RectorPrefix20211210\Tracy\Helpers;
/**
...
class Bridge
{
    ...
    {
        $blueScreen = \RectorPrefix20211210\Tracy\Debugger::getBlueScreen();
        if (!\class_exists(\RectorPrefix20211210\Latte\Bridges\Tracy\BlueScreenPanel::class)) {
    ...
    public static function renderLatteError($e) : ?array
    {
        if ($e instanceof \RectorPrefix20211210\Latte\CompileException && $e->sourceName) {
            ...
            if (\preg_match('#// source: (\\S+\\.latte)#', $lines[1], $m) && @\is_file($m[1])) {
                ...
                return ['tab' => 'Template', 'panel' => '<p><b>File:</b> ' . \RectorPrefix20211210\Tracy\Helpers::editorLink($templateFile, $templateLine) . '</p>' . ($templateLine === null ? '' : \RectorPrefix20211210\Tracy\BlueScreen::highlightFile($templateFile, $templateLine))];
            }
        }
        return null;
    }
    ...
    public static function renderMemberAccessException($e) : ?array
    {
        ...
        $loc = $e->getTrace()[$e instanceof \RectorPrefix20211210\Nette\MemberAccessException ? 1 : 0];
        if (\preg_match('#Cannot (?:read|write to) an undeclared property .+::\\$(\\w+), did you mean \\$(\\w+)\\?#A', $e->getMessage(), $m)) {
            return ['link' => \RectorPrefix20211210\Tracy\Helpers::editorUri($loc['file'], $loc['line'], 'fix', '->' . $m[1], '->' . $m[2]), 'label' => 'fix it'];
        } elseif (\preg_match('#Call to undefined (static )?method .+::(\\w+)\\(\\), did you mean (\\w+)\\(\\)?#A', $e->getMessage(), $m)) {
            $operator = $m[1] ? '::' : '->';
            return ['link' => \RectorPrefix20211210\Tracy\Helpers::editorUri($loc['file'], $loc['line'], 'fix', $operator . $m[2] . '(', $operator . $m[3] . '('), 'label' => 'fix it'];
        }
        return null;
    }
    /**
    ...
    public static function renderNeonError($e) : ?array
    {
        if (!$e instanceof \RectorPrefix20211210\Nette\Neon\Exception || !\preg_match('#line (\\d+)#', $e->getMessage(), $m)) {
            return null;
        } elseif ($trace = \RectorPrefix20211210\Tracy\Helpers::findTrace($e->getTrace(), [\RectorPrefix20211210\Nette\Neon\Decoder::class, 'decodeFile']) ?? \RectorPrefix20211210\Tracy\Helpers::findTrace($e->getTrace(), [\RectorPrefix20211210\Nette\DI\Config\Adapters\NeonAdapter::class, 'load'])) {
            $panel = '<p><b>File:</b> ' . \RectorPrefix20211210\Tracy\Helpers::editorLink($trace['args'][0], (int) $m[1]) . '</p>' . self::highlightNeon(\file_get_contents($trace['args'][0]), (int) $m[1]);
        } elseif ($trace = \RectorPrefix20211210\Tracy\Helpers::findTrace($e->getTrace(), [\RectorPrefix20211210\Nette\Neon\Decoder::class, 'decode'])) {
    ...
    private static function highlightNeon(string $code, int $line) : string
    {
        ...
        $code = \str_replace("\t", "<span class='tracy-dump-whitespace'>→   </span>", $code);
        return '<pre class=code><div>' . \RectorPrefix20211210\Tracy\BlueScreen::highlightLine($code, $line) . '</div></pre>';
    }
}
