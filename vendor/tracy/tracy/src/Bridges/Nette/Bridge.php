...
class Bridge
{
    ...
    public static function renderLatteError(?\Throwable $e) : ?array
    {
        if ($e instanceof Latte\CompileException && $e->sourceName) {
            ...
            if (\preg_match('#// source: (\\S+\\.latte)#', $lines[1], $m) && @\is_file($m[1])) {
                ...
                return ['tab' => 'Template', 'panel' => '<p><b>File:</b> ' . Helpers::editorLink($templateFile, $templateLine) . '</p>' . ($templateLine === null ? '' : BlueScreen::highlightFile($templateFile, $templateLine))];
            }
        }
        return null;
    }
    ...
    {
        if ($e instanceof Nette\Neon\Exception && \preg_match('#line (\\d+)#', $e->getMessage(), $m) && ($trace = Helpers::findTrace($e->getTrace(), [Nette\Neon\Decoder::class, 'decode']))) {
            return ['tab' => 'NEON', 'panel' => ($trace2 = Helpers::findTrace($e->getTrace(), [Nette\DI\Config\Adapters\NeonAdapter::class, 'load'])) ? '<p><b>File:</b> ' . Helpers::editorLink($trace2['args'][0], (int) $m[1]) . '</p>' . self::highlightNeon(\file_get_contents($trace2['args'][0]), (int) $m[1]) : self::highlightNeon($trace['args'][0], (int) $m[1])];
        }
        return null;
    }
