...
class MailSender
{
    ...
    private $fromEmail;
    public function __construct(\RectorPrefix20220214\Nette\Mail\IMailer $mailer, ?string $fromEmail = null)
    {
        $this->mailer = $mailer;
    ...
    public function send($message, string $email) : void
    {
        ...
        $mail = new \RectorPrefix20220214\Nette\Mail\Message();
        $mail->setHeader('X-Mailer', 'Tracy');
        if ($this->fromEmail || \RectorPrefix20220214\Nette\Utils\Validators::isEmail("noreply@{$host}")) {
        ...
        $mail->setBody(\RectorPrefix20220214\Tracy\Logger::formatMessage($message) . "\n\nsource: " . \RectorPrefix20220214\Tracy\Helpers::getSource());
