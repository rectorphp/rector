...
use RectorPrefix20220427\Nette\Schema\Expect;
...
class TracyExtension extends \RectorPrefix20220427\Nette\DI\CompilerExtension
{
    ...
    public function loadConfiguration()
    {
        ...
        $builder->addDefinition($this->prefix('logger'))->setClass(\RectorPrefix20220427\Tracy\ILogger::class)->setFactory([\RectorPrefix20220427\Tracy\Debugger::class, 'getLogger']);
        $builder->addDefinition($this->prefix('blueScreen'))->setFactory([\RectorPrefix20220427\Tracy\Debugger::class, 'getBlueScreen']);
        $builder->addDefinition($this->prefix('bar'))->setFactory([\RectorPrefix20220427\Tracy\Debugger::class, 'getBar']);
    }
    public function afterCompile(\RectorPrefix20220427\Nette\PhpGenerator\ClassType $class)
    {
        $initialize = $this->initialization ?? new \RectorPrefix20220427\Nette\PhpGenerator\Closure();
        ...
        if (!$logger instanceof \RectorPrefix20220427\Nette\DI\ServiceDefinition || $logger->getFactory()->getEntity() !== [\RectorPrefix20220427\Tracy\Debugger::class, 'getLogger']) {
            $initialize->addBody($builder->formatPhp('Tracy\\Debugger::setLogger(?);', [$logger]));
        }
        if ($this->config->netteMailer && $builder->getByType(\RectorPrefix20220427\Nette\Mail\IMailer::class)) {
            $initialize->addBody($builder->formatPhp('Tracy\\Debugger::getLogger()->mailer = ?;', [[new \RectorPrefix20220427\Nette\DI\Statement(\RectorPrefix20220427\Tracy\Bridges\Nette\MailSender::class, ['fromEmail' => $this->config->fromEmail]), 'send']]));
        }
        if ($this->debugMode) {
            foreach ($this->config->bar as $item) {
                if (\is_string($item) && \substr($item, 0, 1) === '@') {
                    $item = new \RectorPrefix20220427\Nette\DI\Statement(['@' . $builder::THIS_CONTAINER, 'getService'], [\substr($item, 1)]);
                } elseif (\is_string($item)) {
                    $item = new \RectorPrefix20220427\Nette\DI\Statement($item);
                }
                $initialize->addBody($builder->formatPhp('$this->getService(?)->addPanel(?);', \RectorPrefix20220427\Nette\DI\Helpers::filterArguments([$this->prefix('bar'), $item])));
            }
            if (!$this->cliMode && \RectorPrefix20220427\Tracy\Debugger::getSessionStorage() instanceof \RectorPrefix20220427\Tracy\NativeSession && ($name = $builder->getByType(\RectorPrefix20220427\Nette\Http\Session::class))) {
        ...
        foreach ($this->config->blueScreen as $item) {
            $initialize->addBody($builder->formatPhp('$this->getService(?)->addPanel(?);', \RectorPrefix20220427\Nette\DI\Helpers::filterArguments([$this->prefix('blueScreen'), $item])));
        }
        if (empty($this->initialization)) {
            $class->getMethod('initialize')->addBody("({$initialize})();");
        }
        if (($dir = \RectorPrefix20220427\Tracy\Debugger::$logDirectory) && !\is_writable($dir)) {
            throw new \RectorPrefix20220427\Nette\InvalidStateException("Make directory '{$dir}' writable.");
        }
    }
    /**
    ...
    private function parseErrorSeverity($value) : int
    {
        ...
        // @ may fail
        if (!$res) {
            throw new \RectorPrefix20220427\Nette\InvalidStateException("Syntax error in expression '{$value}'");
        }
        return $res;
    }
}
