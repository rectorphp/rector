...
use RectorPrefix20220208\Nette\Schema\Expect;
use RectorPrefix20220208\Tracy;
/**
 * Tracy extension for Nette DI.
 */
class TracyExtension extends \RectorPrefix20220208\Nette\DI\CompilerExtension
{
    ...
    public function afterCompile(\RectorPrefix20220208\Nette\PhpGenerator\ClassType $class)
    {
        ...
        foreach ($options as $key => $value) {
            if ($value !== null) {
                ...
                $initialize->addBody($builder->formatPhp(($tbl[$key] ?? 'Tracy\\Debugger::$' . $key . ' = ?') . ';', \RectorPrefix20220208\Nette\DI\Helpers::filterArguments([$value])));
            }
        }
        $logger = $builder->getDefinition($this->prefix('logger'));
        if (!$logger instanceof \RectorPrefix20220208\Nette\DI\ServiceDefinition || $logger->getFactory()->getEntity() !== [\RectorPrefix20220208\Tracy\Debugger::class, 'getLogger']) {
            $initialize->addBody($builder->formatPhp('Tracy\\Debugger::setLogger(?);', [$logger]));
        }
        if ($this->config->netteMailer && $builder->getByType(\RectorPrefix20220208\Nette\Mail\IMailer::class)) {
            $initialize->addBody($builder->formatPhp('Tracy\\Debugger::getLogger()->mailer = ?;', [[new \RectorPrefix20220208\Nette\DI\Statement(\RectorPrefix20220208\Tracy\Bridges\Nette\MailSender::class, ['fromEmail' => $this->config->fromEmail]), 'send']]));
        }
        if ($this->debugMode) {
            foreach ($this->config->bar as $item) {
                if (\is_string($item) && \substr($item, 0, 1) === '@') {
                    $item = new \RectorPrefix20220208\Nette\DI\Statement(['@' . $builder::THIS_CONTAINER, 'getService'], [\substr($item, 1)]);
                } elseif (\is_string($item)) {
                    $item = new \RectorPrefix20220208\Nette\DI\Statement($item);
                }
                $initialize->addBody($builder->formatPhp('$this->getService(?)->addPanel(?);', \RectorPrefix20220208\Nette\DI\Helpers::filterArguments([$this->prefix('bar'), $item])));
            }
            if (!$this->cliMode && \RectorPrefix20220208\Tracy\Debugger::getSessionStorage() instanceof \RectorPrefix20220208\Tracy\NativeSession && ($name = $builder->getByType(\RectorPrefix20220208\Nette\Http\Session::class))) {
        ...
        foreach ($this->config->blueScreen as $item) {
            $initialize->addBody($builder->formatPhp('$this->getService(?)->addPanel(?);', \RectorPrefix20220208\Nette\DI\Helpers::filterArguments([$this->prefix('blueScreen'), $item])));
        }
        if (empty($this->initialization)) {
            $class->getMethod('initialize')->addBody("({$initialize})();");
        }
        if (($dir = \RectorPrefix20220208\Tracy\Debugger::$logDirectory) && !\is_writable($dir)) {
            throw new \RectorPrefix20220208\Nette\InvalidStateException("Make directory '{$dir}' writable.");
        }
    }
    /**
    ...
    private function parseErrorSeverity($value) : int
    {
        ...
        // @ may fail
        if (!$res) {
            throw new \RectorPrefix20220208\Nette\InvalidStateException("Syntax error in expression '{$value}'");
        }
        return $res;
    }
}
