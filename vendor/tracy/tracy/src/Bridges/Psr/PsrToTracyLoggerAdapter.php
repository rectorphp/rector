...
namespace RectorPrefix20220427\Tracy\Bridges\Psr;

use RectorPrefix20220427\Psr;
...
class PsrToTracyLoggerAdapter implements \RectorPrefix20220427\Tracy\ILogger
{
    ...
    public function log($value, $level = self::INFO)
    {
        if ($value instanceof \Throwable) {
            $message = \RectorPrefix20220427\Tracy\Helpers::getClass($value) . ': ' . $value->getMessage() . ($value->getCode() ? ' #' . $value->getCode() : '') . ' in ' . $value->getFile() . ':' . $value->getLine();
            $context = ['exception' => $value];
        } elseif (!\is_string($value)) {
            $message = \trim(\RectorPrefix20220427\Tracy\Dumper::toText($value));
            $context = [];
        } else {
            $message = $value;
            $context = [];
