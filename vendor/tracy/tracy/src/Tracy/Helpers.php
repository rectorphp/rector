...
class Helpers
{
    ...
    public static function editorUri(string $file, int $line = null, string $action = 'open', string $search = '', string $replace = '') : ?string
    {
        if (\RectorPrefix20211210\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20211210\Tracy\Debugger::$editorMapping);
            return \strtr(\RectorPrefix20211210\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
