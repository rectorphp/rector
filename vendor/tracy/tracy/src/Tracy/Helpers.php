...
class Helpers
{
    ...
    public static function editorUri($file, $line = null, $action = 'open', $search = '', $replace = '') : ?string
    {
        if (\RectorPrefix20210705\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20210705\Tracy\Debugger::$editorMapping);
            return \strtr(\RectorPrefix20210705\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
    /**
