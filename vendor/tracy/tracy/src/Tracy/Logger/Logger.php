 ...
 * Logger.
 */
class Logger implements \RectorPrefix20210818\Tracy\ILogger
{
     ...
     */
    public function __construct(?string $directory, $email = null, \RectorPrefix20210818\Tracy\BlueScreen $blueScreen = null)
    {
    ...
    public static function formatMessage($message) : string
    {
        if ($message instanceof \Throwable) {
            foreach (\RectorPrefix20210818\Tracy\Helpers::getExceptionChain($message) as $exception) {
                $tmp[] = ($exception instanceof \ErrorException ? \RectorPrefix20210818\Tracy\Helpers::errorTypeToString($exception->getSeverity()) . ': ' . $exception->getMessage() : \RectorPrefix20210818\Tracy\Helpers::getClass($exception) . ': ' . $exception->getMessage() . ($exception->getCode() ? ' #' . $exception->getCode() : '')) . ' in ' . $exception->getFile() . ':' . $exception->getLine();
            }
            $message = \implode("\ncaused by ", $tmp);
        } elseif (!\is_string($message)) {
            $message = \RectorPrefix20210818\Tracy\Dumper::toText($message);
        }
        return \trim($message);
    }
    /**
    ...
    {
        return \implode(' ', [\date('[Y-m-d H-i-s]'), \preg_replace('#\\s*\\r?\\n\\s*#', ' ', static::formatMessage($message)), ' @  ' . \RectorPrefix20210818\Tracy\Helpers::getSource(), $exceptionFile ? ' @@  ' . \basename($exceptionFile) : null]);
    }
    /**
    ...
    {
        foreach (\RectorPrefix20210818\Tracy\Helpers::getExceptionChain($exception) as $exception) {
    ...
    protected function logException($exception, $file = null) : string
    {
        ...
        $bs = $this->blueScreen ?: new \RectorPrefix20210818\Tracy\BlueScreen();
