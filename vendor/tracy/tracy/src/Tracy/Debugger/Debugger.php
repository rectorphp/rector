...
class Debugger
{
    ...
    public const DEBUG = \RectorPrefix20220529\Tracy\ILogger::DEBUG, INFO = \RectorPrefix20220529\Tracy\ILogger::INFO, WARNING = \RectorPrefix20220529\Tracy\ILogger::WARNING, ERROR = \RectorPrefix20220529\Tracy\ILogger::ERROR, EXCEPTION = \RectorPrefix20220529\Tracy\ILogger::EXCEPTION, CRITICAL = \RectorPrefix20220529\Tracy\ILogger::CRITICAL;
    /********************* misc ****************d*g**/
    ...
    {
        if (!\RectorPrefix20220529\Tracy\Helpers::isCli() && self::getStrategy()->sendAssets()) {
            self::$showBar = \false;
            exit;
        }
    }
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (\in_array($error['type'] ?? null, [\E_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_PARSE, \E_RECOVERABLE_ERROR, \E_USER_ERROR], \true)) {
            self::exceptionHandler(\RectorPrefix20220529\Tracy\Helpers::fixStack(new \ErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'])));
        } elseif (($error['type'] ?? null) === \E_COMPILE_WARNING) {
            \error_clear_last();
            self::errorHandler($error['type'], $error['message'], $error['file'], $error['line']);
        }
        self::$reserved = null;
        if (self::$showBar && !\RectorPrefix20220529\Tracy\Helpers::isCli()) {
            try {
    ...
    public static function exceptionHandler(\Throwable $exception) : void
    {
        ...
        \RectorPrefix20220529\Tracy\Helpers::improveException($exception);
    ...
    public static function errorHandler(int $severity, string $message, string $file, int $line, ?array $context = null) : bool
    {
        ...
        if ($severity === \E_RECOVERABLE_ERROR || $severity === \E_USER_ERROR) {
            if (\RectorPrefix20220529\Tracy\Helpers::findTrace(\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), '*::__toString')) {
    ...
    /********************* services ****************d*g**/
    public static function getBlueScreen() : \RectorPrefix20220529\Tracy\BlueScreen
    {
        if (!self::$blueScreen) {
            self::$blueScreen = new \RectorPrefix20220529\Tracy\BlueScreen();
            self::$blueScreen->info = ['PHP ' . \PHP_VERSION, $_SERVER['SERVER_SOFTWARE'] ?? null, 'Tracy ' . self::VERSION];
        }
        return self::$blueScreen;
    }
    public static function getBar() : \RectorPrefix20220529\Tracy\Bar
    {
        if (!self::$bar) {
            self::$bar = new \RectorPrefix20220529\Tracy\Bar();
            self::$bar->addPanel($info = new \RectorPrefix20220529\Tracy\DefaultBarPanel('info'), 'Tracy:info');
            $info->cpuUsage = self::$cpuUsage;
            self::$bar->addPanel(new \RectorPrefix20220529\Tracy\DefaultBarPanel('errors'), 'Tracy:errors');
            // filled by errorHandler()
        }
        return self::$bar;
    }
    public static function setLogger(\RectorPrefix20220529\Tracy\ILogger $logger) : void
    {
        self::$logger = $logger;
    }
    public static function getLogger() : \RectorPrefix20220529\Tracy\ILogger
    {
        if (!self::$logger) {
            self::$logger = new \RectorPrefix20220529\Tracy\Logger(self::$logDirectory, self::$email, self::getBlueScreen());
    ...
    public static function getFireLogger() : \RectorPrefix20220529\Tracy\ILogger
    {
        if (!self::$fireLogger) {
            self::$fireLogger = new \RectorPrefix20220529\Tracy\FireLogger();
        }
    ...
    public static function getStrategy()
    {
        if (empty(self::$strategy[self::$productionMode])) {
            self::$strategy[self::$productionMode] = self::$productionMode ? new \RectorPrefix20220529\Tracy\ProductionStrategy() : new \RectorPrefix20220529\Tracy\DevelopmentStrategy(self::getBar(), self::getBlueScreen(), new \RectorPrefix20220529\Tracy\DeferredContent(self::getSessionStorage()));
        }
        return self::$strategy[self::$productionMode];
    }
    public static function setSessionStorage(\RectorPrefix20220529\Tracy\SessionStorage $storage) : void
    {
    ...
    /** @internal */
    public static function getSessionStorage() : \RectorPrefix20220529\Tracy\SessionStorage
    {
        if (!self::$sessionStorage) {
            self::$sessionStorage = @\is_dir($dir = \session_save_path()) || @\is_dir($dir = \ini_get('upload_tmp_dir')) || @\is_dir($dir = \sys_get_temp_dir()) || ($dir = self::$logDirectory) ? new \RectorPrefix20220529\Tracy\FileSession($dir) : new \RectorPrefix20220529\Tracy\NativeSession();
        }
    ...
    {
        if ($return) {
            $options = [\RectorPrefix20220529\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20220529\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20220529\Tracy\Dumper::ITEMS => self::$maxItems];
            return \RectorPrefix20220529\Tracy\Helpers::isCli() ? \RectorPrefix20220529\Tracy\Dumper::toText($var) : \RectorPrefix20220529\Tracy\Helpers::capture(function () use($var, $options) {
                \RectorPrefix20220529\Tracy\Dumper::dump($var, $options);
            });
        } elseif (!self::$productionMode) {
            $html = \RectorPrefix20220529\Tracy\Helpers::isHtmlMode();
            echo $html ? '<tracy-div>' : '';
            \RectorPrefix20220529\Tracy\Dumper::dump($var, [\RectorPrefix20220529\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20220529\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20220529\Tracy\Dumper::ITEMS => self::$maxItems, \RectorPrefix20220529\Tracy\Dumper::LOCATION => self::$showLocation, \RectorPrefix20220529\Tracy\Dumper::THEME => self::$dumpTheme, \RectorPrefix20220529\Tracy\Dumper::KEYS_TO_HIDE => self::$keysToHide]);
            echo $html ? '</tracy-div>' : '';
        }
        return $var;
    }
    ...
    public static function barDump($var, ?string $title = null, array $options = [])
    {
        if (!self::$productionMode) {
            static $panel;
            if (!$panel) {
                self::getBar()->addPanel($panel = new \RectorPrefix20220529\Tracy\DefaultBarPanel('dumps'), 'Tracy:dumps');
            }
            $panel->data[] = ['title' => $title, 'dump' => \RectorPrefix20220529\Tracy\Dumper::toHtml($var, $options + [\RectorPrefix20220529\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20220529\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20220529\Tracy\Dumper::LOCATION => self::$showLocation ?: \RectorPrefix20220529\Tracy\Dumper::LOCATION_CLASS | \RectorPrefix20220529\Tracy\Dumper::LOCATION_SOURCE, \RectorPrefix20220529\Tracy\Dumper::LAZY => \true])];
        }
        return $var;
    }
    /**
     ...
     * @return mixed
     */
    public static function log($message, string $level = \RectorPrefix20220529\Tracy\ILogger::INFO)
    {
