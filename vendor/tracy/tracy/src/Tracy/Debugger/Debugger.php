...
class Debugger
{
    ...
    public const DEBUG = \RectorPrefix20210810\Tracy\ILogger::DEBUG, INFO = \RectorPrefix20210810\Tracy\ILogger::INFO, WARNING = \RectorPrefix20210810\Tracy\ILogger::WARNING, ERROR = \RectorPrefix20210810\Tracy\ILogger::ERROR, EXCEPTION = \RectorPrefix20210810\Tracy\ILogger::EXCEPTION, CRITICAL = \RectorPrefix20210810\Tracy\ILogger::CRITICAL;
    /********************* misc ****************d*g**/
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (\in_array($error['type'] ?? null, [\E_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_PARSE, \E_RECOVERABLE_ERROR, \E_USER_ERROR], \true)) {
            self::exceptionHandler(\RectorPrefix20210810\Tracy\Helpers::fixStack(new \ErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'])));
    ...
    public static function exceptionHandler($exception) : void
    {
        ...
        \RectorPrefix20210810\Tracy\Helpers::improveException($exception);
        ...
        } elseif ($firstTime && \RectorPrefix20210810\Tracy\Helpers::isHtmlMode() || \RectorPrefix20210810\Tracy\Helpers::isAjax()) {
    ...
    public static function errorHandler($severity, $message, $file, $line, $context = null) : ?bool
    {
        ...
        if ($severity === \E_RECOVERABLE_ERROR || $severity === \E_USER_ERROR) {
            if (\RectorPrefix20210810\Tracy\Helpers::findTrace(\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), '*::__toString')) {
            ...
            if (($severity & self::$logSeverity) === $severity) {
                $e = new \ErrorException($message, 0, $severity, $file, $line);
                $e->context = $context;
                \RectorPrefix20210810\Tracy\Helpers::improveException($e);
            } else {
                $e = 'PHP ' . \RectorPrefix20210810\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20210810\Tracy\Helpers::improveError($message, (array) $context) . " in {$file}:{$line}";
            }
            try {
            ...
            $message = 'PHP ' . \RectorPrefix20210810\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20210810\Tracy\Helpers::improveError($message, (array) $context);
            ...
            if ($count++) {
                ...
                return \RectorPrefix20210810\Tracy\Helpers::isHtmlMode() || \RectorPrefix20210810\Tracy\Helpers::isAjax() ? null : \false;
    ...
    /********************* services ****************d*g**/
    public static function getBlueScreen() : \RectorPrefix20210810\Tracy\BlueScreen
    {
        if (!self::$blueScreen) {
            self::$blueScreen = new \RectorPrefix20210810\Tracy\BlueScreen();
            self::$blueScreen->info = ['PHP ' . \PHP_VERSION, $_SERVER['SERVER_SOFTWARE'] ?? null, 'Tracy ' . self::VERSION];
        }
        return self::$blueScreen;
    }
    public static function getBar() : \RectorPrefix20210810\Tracy\Bar
    {
        if (!self::$bar) {
            self::$bar = new \RectorPrefix20210810\Tracy\Bar();
            self::$bar->addPanel($info = new \RectorPrefix20210810\Tracy\DefaultBarPanel('info'), 'Tracy:info');
            $info->cpuUsage = self::$cpuUsage;
            self::$bar->addPanel(new \RectorPrefix20210810\Tracy\DefaultBarPanel('errors'), 'Tracy:errors');
    ...
    public static function getLogger() : \RectorPrefix20210810\Tracy\ILogger
    {
        if (!self::$logger) {
            self::$logger = new \RectorPrefix20210810\Tracy\Logger(self::$logDirectory, self::$email, self::getBlueScreen());
    ...
    public static function getFireLogger() : \RectorPrefix20210810\Tracy\ILogger
    {
        if (!self::$fireLogger) {
            self::$fireLogger = new \RectorPrefix20210810\Tracy\FireLogger();
        }
    ...
    {
        if ($return) {
            $options = [\RectorPrefix20210810\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210810\Tracy\Dumper::TRUNCATE => self::$maxLength];
            return \PHP_SAPI === 'cli' || \PHP_SAPI === 'phpdbg' ? \RectorPrefix20210810\Tracy\Dumper::toText($var) : \RectorPrefix20210810\Tracy\Helpers::capture(function () use($var, $options) {
                \RectorPrefix20210810\Tracy\Dumper::dump($var, $options);
            });
        } elseif (!self::$productionMode) {
            \RectorPrefix20210810\Tracy\Dumper::dump($var, [\RectorPrefix20210810\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210810\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20210810\Tracy\Dumper::LOCATION => self::$showLocation, \RectorPrefix20210810\Tracy\Dumper::THEME => self::$dumpTheme]);
        }
        return $var;
    }
    /**
    ...
    public static function barDump($var, $title = null, $options = [])
    {
        if (!self::$productionMode) {
            static $panel;
            if (!$panel) {
                self::getBar()->addPanel($panel = new \RectorPrefix20210810\Tracy\DefaultBarPanel('dumps'), 'Tracy:dumps');
            }
            $panel->data[] = ['title' => $title, 'dump' => \RectorPrefix20210810\Tracy\Dumper::toHtml($var, $options + [\RectorPrefix20210810\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210810\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20210810\Tracy\Dumper::LOCATION => self::$showLocation ?: \RectorPrefix20210810\Tracy\Dumper::LOCATION_CLASS | \RectorPrefix20210810\Tracy\Dumper::LOCATION_SOURCE, \RectorPrefix20210810\Tracy\Dumper::LAZY => \true])];
        }
        return $var;
    }
    /**
     ...
     * @param string $level
     */
    public static function log($message, $level = \RectorPrefix20210810\Tracy\ILogger::INFO)
    {
