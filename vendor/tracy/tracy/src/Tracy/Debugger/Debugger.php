...
class Debugger
{
    ...
    public const DEBUG = \RectorPrefix20211127\Tracy\ILogger::DEBUG, INFO = \RectorPrefix20211127\Tracy\ILogger::INFO, WARNING = \RectorPrefix20211127\Tracy\ILogger::WARNING, ERROR = \RectorPrefix20211127\Tracy\ILogger::ERROR, EXCEPTION = \RectorPrefix20211127\Tracy\ILogger::EXCEPTION, CRITICAL = \RectorPrefix20211127\Tracy\ILogger::CRITICAL;
    /********************* misc ****************d*g**/
    ...
    {
        if (self::$productionMode || \RectorPrefix20211127\Tracy\Helpers::isCli()) {
            return;
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (\in_array($error['type'] ?? null, [\E_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_PARSE, \E_RECOVERABLE_ERROR, \E_USER_ERROR], \true)) {
            self::exceptionHandler(\RectorPrefix20211127\Tracy\Helpers::fixStack(new \ErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'])));
    ...
    public static function exceptionHandler($exception) : void
    {
        ...
        \RectorPrefix20211127\Tracy\Helpers::improveException($exception);
        ...
        if (self::$productionMode || \connection_aborted()) {
            ...
            } elseif (\RectorPrefix20211127\Tracy\Helpers::isCli()) {
        ...
        } elseif ($firstTime && \RectorPrefix20211127\Tracy\Helpers::isHtmlMode() || \RectorPrefix20211127\Tracy\Helpers::isAjax()) {
            ...
            try {
                ...
                if (\RectorPrefix20211127\Tracy\Helpers::detectColors()) {
                    echo "\n\n" . \RectorPrefix20211127\Tracy\BlueScreen::highlightPhpCli($exception->getFile(), $exception->getLine()) . "\n";
                }
                echo "{$exception}\n" . ($file ? "\n(stored in {$file})\n" : '');
    ...
    public static function errorHandler($severity, $message, $file, $line, $context = null) : ?bool
    {
        ...
        if ($severity === \E_RECOVERABLE_ERROR || $severity === \E_USER_ERROR) {
            if (\RectorPrefix20211127\Tracy\Helpers::findTrace(\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), '*::__toString')) {
            ...
            if (($severity & self::$logSeverity) === $severity) {
                $e = new \ErrorException($message, 0, $severity, $file, $line);
                $e->context = $context;
                \RectorPrefix20211127\Tracy\Helpers::improveException($e);
            } else {
                $e = 'PHP ' . \RectorPrefix20211127\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20211127\Tracy\Helpers::improveError($message, (array) $context) . " in {$file}:{$line}";
            }
            try {
            ...
            $message = 'PHP ' . \RectorPrefix20211127\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20211127\Tracy\Helpers::improveError($message, (array) $context);
            ...
            if ($count++) {
                ...
                return \RectorPrefix20211127\Tracy\Helpers::isHtmlMode() || \RectorPrefix20211127\Tracy\Helpers::isAjax() ? null : \false;
    ...
    /********************* services ****************d*g**/
    public static function getBlueScreen() : \RectorPrefix20211127\Tracy\BlueScreen
    {
        if (!self::$blueScreen) {
            self::$blueScreen = new \RectorPrefix20211127\Tracy\BlueScreen();
            self::$blueScreen->info = ['PHP ' . \PHP_VERSION, $_SERVER['SERVER_SOFTWARE'] ?? null, 'Tracy ' . self::VERSION];
        }
        return self::$blueScreen;
    }
    public static function getBar() : \RectorPrefix20211127\Tracy\Bar
    {
        if (!self::$bar) {
            self::$bar = new \RectorPrefix20211127\Tracy\Bar();
            self::$bar->addPanel($info = new \RectorPrefix20211127\Tracy\DefaultBarPanel('info'), 'Tracy:info');
            $info->cpuUsage = self::$cpuUsage;
            self::$bar->addPanel(new \RectorPrefix20211127\Tracy\DefaultBarPanel('errors'), 'Tracy:errors');
    ...
    public static function getLogger() : \RectorPrefix20211127\Tracy\ILogger
    {
        if (!self::$logger) {
            self::$logger = new \RectorPrefix20211127\Tracy\Logger(self::$logDirectory, self::$email, self::getBlueScreen());
    ...
    public static function getFireLogger() : \RectorPrefix20211127\Tracy\ILogger
    {
        if (!self::$fireLogger) {
            self::$fireLogger = new \RectorPrefix20211127\Tracy\FireLogger();
        }
    ...
    {
        if ($return) {
            $options = [\RectorPrefix20211127\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211127\Tracy\Dumper::TRUNCATE => self::$maxLength];
            return \RectorPrefix20211127\Tracy\Helpers::isCli() ? \RectorPrefix20211127\Tracy\Dumper::toText($var) : \RectorPrefix20211127\Tracy\Helpers::capture(function () use($var, $options) {
                \RectorPrefix20211127\Tracy\Dumper::dump($var, $options);
            });
        } elseif (!self::$productionMode) {
            \RectorPrefix20211127\Tracy\Dumper::dump($var, [\RectorPrefix20211127\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211127\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20211127\Tracy\Dumper::LOCATION => self::$showLocation, \RectorPrefix20211127\Tracy\Dumper::THEME => self::$dumpTheme, \RectorPrefix20211127\Tracy\Dumper::KEYS_TO_HIDE => self::$keysToHide]);
        }
        return $var;
    }
    /**
    ...
    public static function barDump($var, $title = null, $options = [])
    {
        if (!self::$productionMode) {
            static $panel;
            if (!$panel) {
                self::getBar()->addPanel($panel = new \RectorPrefix20211127\Tracy\DefaultBarPanel('dumps'), 'Tracy:dumps');
            }
            $panel->data[] = ['title' => $title, 'dump' => \RectorPrefix20211127\Tracy\Dumper::toHtml($var, $options + [\RectorPrefix20211127\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211127\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20211127\Tracy\Dumper::LOCATION => self::$showLocation ?: \RectorPrefix20211127\Tracy\Dumper::LOCATION_CLASS | \RectorPrefix20211127\Tracy\Dumper::LOCATION_SOURCE, \RectorPrefix20211127\Tracy\Dumper::LAZY => \true])];
        }
        return $var;
    }
    /**
     ...
     * @param string $level
     */
    public static function log($message, $level = \RectorPrefix20211127\Tracy\ILogger::INFO)
    {
