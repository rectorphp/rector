...
final class Exposer
{
     ...
     * @param object $obj
     */
    public static function exposeObject($obj, Value $value, Describer $describer) : void
    {
        $tmp = (array) $obj;
        $values = $tmp;
        // bug #79477, PHP < 7.4.6
    ...
    private static function getProperties($class) : array
    {
        ...
        foreach ($rc->getProperties() as $prop) {
            ...
            if ($prop->isStatic() || $prop->getDeclaringClass()->getName() !== $class) {
                ...
                $props["\0" . $class . "\0" . $name] = [$name, $class, Value::PROP_PRIVATE];
            } elseif ($prop->isProtected()) {
                $props["\0*\0" . $name] = [$name, $class, Value::PROP_PROTECTED];
            } else {
