...
use RectorPrefix20220427\Tracy\Dumper\Describer;
use RectorPrefix20220427\Tracy\Dumper\Exposer;
use RectorPrefix20220427\Tracy\Dumper\Renderer;
/**
 * Dumps a variable.
 */
class Dumper
{
    ...
    // deprecated
    public const HIDDEN_VALUE = \RectorPrefix20220427\Tracy\Dumper\Describer::HIDDEN_VALUE;
    /** @var Dumper\Value[] */
    ...
    {
        if (\RectorPrefix20220427\Tracy\Helpers::isCli()) {
    ...
    {
        static $sent;
        if (\RectorPrefix20220427\Tracy\Debugger::$productionMode === \true || $sent) {
            return;
        }
        $sent = \true;
        $nonce = \RectorPrefix20220427\Tracy\Helpers::getNonce();
        $nonceAttr = $nonce ? ' nonce="' . \RectorPrefix20220427\Tracy\Helpers::escapeHtml($nonce) . '"' : '';
        ...
        echo "<style{$nonceAttr}>", \str_replace('</', '<\\/', \RectorPrefix20220427\Tracy\Helpers::minifyCss($s)), "</style>\n";
        if (!\RectorPrefix20220427\Tracy\Debugger::isEnabled()) {
            $s = '(function(){' . \file_get_contents(__DIR__ . '/../assets/toggle.js') . '})();' . '(function(){' . \file_get_contents(__DIR__ . '/../Dumper/assets/dumper.js') . '})();';
            echo "<script{$nonceAttr}>", \str_replace(['<!--', '</s'], ['<\\!--', '<\\/s'], \RectorPrefix20220427\Tracy\Helpers::minifyJs($s)), "</script>\n";
        }
    }
    private function __construct(array $options = [])
    {
        ...
        $describer = $this->describer = new \RectorPrefix20220427\Tracy\Dumper\Describer();
        ...
        $renderer = $this->renderer = new \RectorPrefix20220427\Tracy\Dumper\Renderer();
    ...
    {
        $res = "'" . \RectorPrefix20220427\Tracy\Dumper\Renderer::jsonEncode($snapshot[0] ?? []) . "'";
        $snapshot = [];
        return $res;
    }
}
