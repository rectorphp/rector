...
final class Describer
{
    ...
    {
        return $num <= self::JS_SAFE_INTEGER && $num >= -self::JS_SAFE_INTEGER ? $num : new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_NUMBER, "{$num}");
    }
    /**
     * @return Value|float
     */
    ...
    {
        if (!\is_finite($num)) {
            return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_NUMBER, (string) $num);
        }
        $js = \json_encode($num);
        return \strpos($js, '.') ? $num : new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_NUMBER, "{$js}.0");
    ...
    {
        $encoded = \RectorPrefix20220324\Tracy\Helpers::encodeString($s, $depth ? $this->maxLength : null);
        if ($encoded === $s) {
            return $encoded;
        } elseif (\RectorPrefix20220324\Tracy\Helpers::isUtf8($s)) {
            return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_STRING_HTML, $encoded, \RectorPrefix20220324\Tracy\Helpers::utf8Length($s));
        } else {
            return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_BINARY_HTML, $encoded, \strlen($s));
        }
    }
    /**
     * @return Value|array
    ...
    {
        if ($refId) {
            $res = new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_REF, 'p' . $refId);
            $value =& $this->snapshot[$res->value];
            if ($value && $value->depth <= $depth) {
                return $res;
            }
            $value = new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_ARRAY);
            $value->id = $res->value;
            $value->depth = $depth;
            ...
            return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_ARRAY, null, \count($arr));
        } elseif ($depth && $this->maxItems && \count($arr) > $this->maxItems) {
            $res = new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_ARRAY, null, \count($arr));
            $res->depth = $depth;
            $items =& $res->items;
            $arr = \array_slice($arr, 0, $this->maxItems, \true);
        }
        ...
        foreach ($arr as $k => $v) {
            ...
            $items[] = [$this->describeVar($k, $depth + 1), $this->isSensitive((string) $k, $v) ? new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_TEXT, self::hideValue($v)) : $this->describeVar($v, $depth + 1, $refId)] + ($refId ? [2 => $refId] : []);
        }
        return $res ?? $items;
    }
    private function describeObject(object $obj, int $depth = 0) : \RectorPrefix20220324\Tracy\Dumper\Value
    {
        ...
        if ($value && $value->depth <= $depth) {
            return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_REF, $id);
        }
        $value = new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_OBJECT, \RectorPrefix20220324\Tracy\Helpers::getClass($obj));
        $value->id = $id;
        $value->depth = $depth;
        $value->holder = $obj;
        ...
        if ($this->location) {
            ...
            if ($rc->getFileName() && ($editor = \RectorPrefix20220324\Tracy\Helpers::editorUri($rc->getFileName(), $rc->getStartLine()))) {
        ...
        if ($this->maxDepth && $depth < $this->maxDepth) {
            ...
            foreach ($props ?? [] as $k => $v) {
                $this->addPropertyTo($value, (string) $k, $v, \RectorPrefix20220324\Tracy\Dumper\Value::PROP_VIRTUAL, $this->getReferenceId($props, $k));
            }
        }
        return new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_REF, $id);
    }
     ...
     */
    private function describeResource($resource, int $depth = 0) : \RectorPrefix20220324\Tracy\Dumper\Value
    {
        $id = 'r' . (int) $resource;
        $value =& $this->snapshot[$id];
        if (!$value) {
            $type = \is_resource($resource) ? \get_resource_type($resource) : 'closed';
            $value = new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_RESOURCE, $type . ' resource');
            $value->id = $id;
            $value->depth = $depth;
            $value->items = [];
    ...
    public function describeKey(string $key)
    {
        ...
        return \is_string($value) ? new \RectorPrefix20220324\Tracy\Dumper\Value(\RectorPrefix20220324\Tracy\Dumper\Value::TYPE_STRING_HTML, $key, \RectorPrefix20220324\Tracy\Helpers::utf8Length($key)) : $value;
    }
    public function addPropertyTo(\RectorPrefix20220324\Tracy\Dumper\Value $value, string $k, $v, $type = \RectorPrefix20220324\Tracy\Dumper\Value::PROP_VIRTUAL, ?int $refId = null, ?string $class = null)
    {
    ...
    private function exposeObject(object $obj, \RectorPrefix20220324\Tracy\Dumper\Value $value) : ?array
    {
        ...
        \RectorPrefix20220324\Tracy\Dumper\Exposer::exposeObject($obj, $value, $this);
        return null;
    }
    ...
    {
        return self::HIDDEN_VALUE . ' (' . (\is_object($var) ? \RectorPrefix20220324\Tracy\Helpers::getClass($var) : \gettype($var)) . ')';
    }
    ...
    private static function findLocation() : ?array
    {
        foreach (\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS) as $item) {
            if (isset($item['class']) && ($item['class'] === self::class || $item['class'] === \RectorPrefix20220324\Tracy\Dumper::class)) {
                $location = $item;
                continue;
