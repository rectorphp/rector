...
final class Describer
{
    ...
    {
        return $num <= self::JS_SAFE_INTEGER && $num >= -self::JS_SAFE_INTEGER ? $num : new Value(Value::TYPE_NUMBER, "{$num}");
    }
    /**
     * @return Value|float
     */
     ...
     * @param object $obj
     */
    private function describeObject($obj, int $depth = 0) : Value
    {
    ...
    public function describeKey(string $key)
    {
        ...
        return \is_string($value) ? new Value(Value::TYPE_STRING_HTML, $key, \strlen(\utf8_decode($key))) : $value;
    }
     ...
     * @param object $obj
     */
    private function exposeObject($obj, Value $value) : ?array
    {
