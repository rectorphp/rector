...
class BlueScreen
{
    ...
    /** @internal */
    public function renderToAjax(\Throwable $exception, \RectorPrefix20220117\Tracy\DeferredContent $defer) : void
    {
        $defer->addSetup('Tracy.BlueScreen.loadAjax', \RectorPrefix20220117\Tracy\Helpers::capture(function () use($exception) {
    ...
    private function renderTemplate(\Throwable $exception, string $template, $toScreen = \true) : void
    {
        ...
        $obStatus = \RectorPrefix20220117\Tracy\Debugger::$obStatus;
        $showEnvironment = $this->showEnvironment && \strpos($exception->getMessage(), 'Allowed memory size') === \false;
        $info = \array_filter($this->info);
        $source = \RectorPrefix20220117\Tracy\Helpers::getSource();
        $title = $exception instanceof \ErrorException ? \RectorPrefix20220117\Tracy\Helpers::errorTypeToString($exception->getSeverity()) : \RectorPrefix20220117\Tracy\Helpers::getClass($exception);
        ...
        $snapshot = [];
        $dump = $this->getDumper();
        $css = \array_map('file_get_contents', \array_merge([__DIR__ . '/assets/bluescreen.css', __DIR__ . '/../assets/toggle.css', __DIR__ . '/../assets/table-sort.css', __DIR__ . '/../assets/tabs.css', __DIR__ . '/../Dumper/assets/dumper-light.css'], \RectorPrefix20220117\Tracy\Debugger::$customCssFiles));
        $css = \RectorPrefix20220117\Tracy\Helpers::minifyCss(\implode('', $css));
        $nonce = $toScreen ? \RectorPrefix20220117\Tracy\Helpers::getNonce() : null;
    ...
    private function renderPanels(?\Throwable $ex) : array
    {
        ...
        foreach ($this->panels as $callback) {
            ...
            $res[] = (object) ['tab' => "Error in panel {$name}", 'panel' => \nl2br(\RectorPrefix20220117\Tracy\Helpers::escapeHtml($e))];
        }
        return $res;
    }
    /**
    ...
    private function renderActions(\Throwable $ex) : array
    {
        ...
        if (\preg_match('# ([\'"])(\\w{3,}(?:\\\\\\w{3,})+)\\1#i', $ex->getMessage(), $m)) {
            $class = $m[2];
            if (!\class_exists($class, \false) && !\interface_exists($class, \false) && !\trait_exists($class, \false) && ($file = \RectorPrefix20220117\Tracy\Helpers::guessClassFile($class)) && !\is_file($file)) {
                [$content, $line] = $this->generateNewFileContents($file, $class);
                $actions[] = ['link' => \RectorPrefix20220117\Tracy\Helpers::editorUri($file, $line, 'create', '', $content), 'label' => 'create class'];
            }
        }
        if (\preg_match('# ([\'"])((?:/|[a-z]:[/\\\\])\\w[^\'"]+\\.\\w{2,5})\\1#i', $ex->getMessage(), $m)) {
            ...
            $actions[] = ['link' => \RectorPrefix20220117\Tracy\Helpers::editorUri($file, $line, $label, '', $content), 'label' => $label . ' file'];
        }
        $query = ($ex instanceof \ErrorException ? '' : \RectorPrefix20220117\Tracy\Helpers::getClass($ex) . ' ') . \preg_replace('#\'.*\'|".*"#Us', '', $ex->getMessage());
        $actions[] = ['link' => 'https://www.google.com/search?sourceid=tracy&q=' . \urlencode($query), 'label' => 'search', 'external' => \true];
        if ($ex instanceof \ErrorException && !empty($ex->skippable) && \preg_match('#^https?://#', $source = \RectorPrefix20220117\Tracy\Helpers::getSource())) {
    ...
    public static function highlightFile(string $file, int $line, int $lines = 15, bool $php = \true) : ?string
    {
        ...
        if ($editor = \RectorPrefix20220117\Tracy\Helpers::editorUri($file, $line)) {
            $source = \substr_replace($source, ' title="Ctrl-Click to open in editor" data-tracy-href="' . \RectorPrefix20220117\Tracy\Helpers::escapeHtml($editor) . '"', 4, 0);
        }
        return $source;
    }
    /**
    ...
    public function getDumper() : \Closure
    {
        return function ($v, $k = null) : string {
            return \RectorPrefix20220117\Tracy\Dumper::toHtml($v, [\RectorPrefix20220117\Tracy\Dumper::DEPTH => $this->maxDepth, \RectorPrefix20220117\Tracy\Dumper::TRUNCATE => $this->maxLength, \RectorPrefix20220117\Tracy\Dumper::SNAPSHOT => &$this->snapshot, \RectorPrefix20220117\Tracy\Dumper::LOCATION => \RectorPrefix20220117\Tracy\Dumper::LOCATION_CLASS, \RectorPrefix20220117\Tracy\Dumper::SCRUBBER => $this->scrubber, \RectorPrefix20220117\Tracy\Dumper::KEYS_TO_HIDE => $this->keysToHide], $k);
        };
    }
    public function formatMessage(\Throwable $exception) : string
    {
        $msg = \RectorPrefix20220117\Tracy\Helpers::encodeString(\trim((string) $exception->getMessage()), self::MAX_MESSAGE_LENGTH, \false);
        // highlight 'string'
        ...
        $msg = \preg_replace_callback('#(\\w+\\\\[\\w\\\\]+\\w)(?:::(\\w+))?#', function ($m) {
            ...
            return '<a href="' . \RectorPrefix20220117\Tracy\Helpers::escapeHtml(\RectorPrefix20220117\Tracy\Helpers::editorUri($r->getFileName(), $r->getStartLine())) . '" class="tracy-editor">' . $m[0] . '</a>';
        }, $msg);
        // clickable file name
        $msg = \preg_replace_callback('#([\\w\\\\/.:-]+\\.(?:php|phpt|phtml|latte|neon))(?|:(\\d+)| on line (\\d+))?#', function ($m) {
            return @\is_file($m[1]) ? '<a href="' . \RectorPrefix20220117\Tracy\Helpers::escapeHtml(\RectorPrefix20220117\Tracy\Helpers::editorUri($m[1], isset($m[2]) ? (int) $m[2] : null)) . '" class="tracy-editor">' . $m[0] . '</a>' : $m[0];
        }, $msg);
        return $msg;
    }
    private function renderPhpInfo() : void
    {
        ...
        if (\strpos($license, '<body') === \false) {
            echo '<pre class="tracy-dump tracy-light">', \RectorPrefix20220117\Tracy\Helpers::escapeHtml($info), '</pre>';
        } else {
