...
class Bar
{
     ...
     * @return static
     */
    public function addPanel(\RectorPrefix20220426\Tracy\IBarPanel $panel, ?string $id = null) : self
    {
        if ($id === null) {
            $c = 0;
            do {
     ...
     */
    public function getPanel(string $id) : ?\RectorPrefix20220426\Tracy\IBarPanel
    {
     ...
     * @internal
     */
    public function renderLoader(\RectorPrefix20220426\Tracy\DeferredContent $defer) : void
    {
        ...
        $nonce = \RectorPrefix20220426\Tracy\Helpers::getNonce();
        $async = \true;
     ...
     * Renders debug bar.
     */
    public function render(\RectorPrefix20220426\Tracy\DeferredContent $defer) : void
    {
        ...
        if (\RectorPrefix20220426\Tracy\Helpers::isAjax()) {
        ...
        } elseif (\RectorPrefix20220426\Tracy\Helpers::isRedirect()) {
            if ($defer->isAvailable()) {
                $redirectQueue[] = ['content' => $this->renderPartial('redirect', '-r' . \count($redirectQueue)), 'time' => \time()];
            }
        } elseif (\RectorPrefix20220426\Tracy\Helpers::isHtmlMode()) {
            ...
            if ($this->loaderRendered) {
                ...
                $nonce = \RectorPrefix20220426\Tracy\Helpers::getNonce();
                $async = \false;
                \RectorPrefix20220426\Tracy\Debugger::removeOutputBuffers(\false);
                require __DIR__ . '/assets/loader.phtml';
            }
        }
    }
    private function renderPartial(string $type, string $suffix = '') : array
    {
        ...
        return ['bar' => \RectorPrefix20220426\Tracy\Helpers::capture(function () use($type, $panels) {
            require __DIR__ . '/assets/bar.phtml';
        }), 'panels' => \RectorPrefix20220426\Tracy\Helpers::capture(function () use($type, $panels) {
    ...
    private function renderPanels(string $suffix = '') : array
    {
        ...
        foreach ($this->panels as $id => $panel) {
            ...
            try {
                ...
                $idHtml = "error-{$idHtml}";
                $tab = "Error in {$id}";
                $panelHtml = "<h1>Error: {$id}</h1><div class='tracy-inner'>" . \nl2br(\RectorPrefix20220426\Tracy\Helpers::escapeHtml($e)) . '</div>';
                unset($e);
            }
