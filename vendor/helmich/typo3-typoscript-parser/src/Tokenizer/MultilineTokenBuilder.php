...
class MultilineTokenBuilder
{
     ...
     */
    public function endMultilineToken(string $append = '') : \RectorPrefix20210527\Helmich\TypoScriptParser\Tokenizer\TokenInterface
    {
        ...
        if ($type === null || $startLine === null || $startColumn === null) {
            throw new \RectorPrefix20210527\Helmich\TypoScriptParser\Tokenizer\TokenizerException('cannot call "endMultilineToken" before calling "startMultilineToken"');
        }
        $token = new \RectorPrefix20210527\Helmich\TypoScriptParser\Tokenizer\Token($type, \rtrim($value), $startLine, $startColumn);
        $this->reset();
        return $token;
    }
    /**
