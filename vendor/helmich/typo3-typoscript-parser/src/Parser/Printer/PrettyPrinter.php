...
namespace RectorPrefix20210705\Helmich\TypoScriptParser\Parser\Printer;

use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Comment;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\ConditionalStatement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\DirectoryIncludeStatement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\FileIncludeStatement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\IncludeStatement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\MultilineComment;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\NestedAssignment;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\NopStatement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Assignment;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\BinaryObjectOperator;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Copy;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Delete;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Modification;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Reference;
 ...
 */
class PrettyPrinter implements \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\Printer\ASTPrinterInterface
{
    /**
     * @var PrettyPrinterConfiguration
     */
    private $prettyPrinterConfiguration;
    public function __construct(\RectorPrefix20210705\Helmich\TypoScriptParser\Parser\Printer\PrettyPrinterConfiguration $prettyPrinterConfiguration = null)
    {
    ...
    private function printStatementList(array $statements, \RectorPrefix20210705\Symfony\Component\Console\Output\OutputInterface $output, int $nesting = 0) : void
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\NestedAssignment) {
                $this->printNestedAssignment($output, $nesting, $statement);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Assignment) {
                $this->printAssignment($output, $statement, $indent);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\BinaryObjectOperator) {
                $this->printBinaryObjectOperator($output, $statement, $nesting);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Delete) {
                $output->writeln($indent . $statement->object->relativeName . ' >');
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Modification) {
                $output->writeln(\sprintf("%s%s := %s(%s)", $indent, $statement->object->relativeName, $statement->call->method, $statement->call->arguments));
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\ConditionalStatement) {
                $next = $i + 1 < $count ? $statements[$i + 1] : null;
                $previous = $i - 1 >= 0 ? $statements[$i - 1] : null;
                $this->printConditionalStatement($output, $nesting, $statement, $next instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\ConditionalStatement, $previous instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\ConditionalStatement);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\IncludeStatement) {
                $this->printIncludeStatement($output, $statement);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Comment) {
                $output->writeln($indent . $statement->comment);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\MultilineComment) {
                $output->writeln($indent . $statement->comment);
            } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\NopStatement) {
    ...
    private function printBinaryObjectOperator(\RectorPrefix20210705\Symfony\Component\Console\Output\OutputInterface $output, \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\BinaryObjectOperator $operator, int $nesting) : void
    {
        ...
        } elseif ($operator instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Operator\Reference) {
            $output->writeln($this->getIndent($nesting) . $operator->object->relativeName . ' =< ' . $targetObjectPath);
        }
    }
    private function printIncludeStatement(\RectorPrefix20210705\Symfony\Component\Console\Output\OutputInterface $output, \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\IncludeStatement $statement) : void
    {
        ...
        } elseif ($statement instanceof \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\DirectoryIncludeStatement) {
