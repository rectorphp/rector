...
use RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\Token;
...
class TokenStream implements \Iterator, \ArrayAccess
{
     ...
     */
    public function current($lookAhead = 0) : \RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\TokenInterface
    {
     ...
     * @return TokenStream
     */
    public function normalized() : \RectorPrefix20210731\Helmich\TypoScriptParser\Parser\TokenStream
    {
        $filteredTokens = [];
        $maxLine = 0;
        foreach ($this->tokens as $token) {
            ...
            if ($token->getType() === \RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_WHITESPACE) {
                $value = \trim($token->getValue(), "\t ");
                if (\strlen($value) > 0) {
                    $filteredTokens[] = new \RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\Token(\RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_WHITESPACE, $value, $token->getLine(), $token->getColumn());
                }
            } else {
        ...
        $filteredTokens[] = new \RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\Token(\RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_WHITESPACE, "\n", $maxLine + 1, 1);
        $filteredTokens[] = new \RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\Token(\RectorPrefix20210731\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_WHITESPACE, "\n", $maxLine + 2, 1);
        return new \RectorPrefix20210731\Helmich\TypoScriptParser\Parser\TokenStream($filteredTokens);
    }
}
