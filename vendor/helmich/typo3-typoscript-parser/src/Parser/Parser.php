...
namespace RectorPrefix20210705\Helmich\TypoScriptParser\Parser;

use ArrayObject;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Builder;
use RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Statement;
use RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface;
use RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenizerInterface;
/**
 * Class Parser
 *
 ...
 * @subpackage Parser
 */
class Parser implements \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\ParserInterface
{
     ...
     */
    public function __construct(\RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenizerInterface $tokenizer, \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Builder $astBuilder = null)
    {
        $this->tokenizer = $tokenizer;
        $this->builder = $astBuilder ?: new \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\AST\Builder();
    }
    /**
    ...
    public function parseTokens($tokens) : array
    {
        ...
        for (; $state->hasNext(); $state->next()) {
            if ($state->token()->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OBJECT_IDENTIFIER) {
                $objectPath = $this->builder->path($state->token()->getValue(), $state->token()->getValue());
                if ($state->token(1)->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_BRACE_OPEN) {
                    $state->next(2);
    ...
    private function parseToken($state) : void
    {
        switch ($state->token()->getType()) {
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OBJECT_IDENTIFIER:
                ...
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_CONDITION:
                $this->parseCondition($state);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_INCLUDE:
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_INCLUDE_NEW:
                $this->parseInclude($state);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_WHITESPACE:
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_BRACE_CLOSE:
                ...
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_COMMENT_ONELINE:
                $state->statements()->append($this->builder->comment($state->token()->getValue(), $state->token()->getLine()));
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_COMMENT_MULTILINE:
                $state->statements()->append($this->builder->multilineComment($state->token()->getValue(), $state->token()->getLine()));
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_EMPTY_LINE:
    ...
    private function parseNestedStatements($state, $startLine = null) : void
    {
        ...
        for (; $state->hasNext(); $state->next()) {
            ...
            if ($state->token()->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_BRACE_CLOSE) {
                $state->statements()->append($this->builder->nested($state->context(), $statements->getArrayCopy(), $startLine));
                $state->next();
                return;
            }
        }
        throw new \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\ParseError('Unterminated nested statement!');
    }
    /**
     * @param ParserState $state
     * @throws ParseError
    ...
    private function parseCondition($state) : void
    {
        ...
        $state->next();
        for (; $state->hasNext(); $state->next()) {
            if ($state->token()->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_CONDITION_END) {
                ...
                $state->next();
                break;
            } elseif ($state->token()->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_CONDITION_ELSE) {
                ...
                $state->next();
            } elseif ($state->token()->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_CONDITION) {
                $state->statements()->append($this->builder->condition($condition, $ifStatements->getArrayCopy(), $elseStatements->getArrayCopy(), $conditionLine));
                $this->parseCondition($state);
                break;
            }
    ...
    private function parseInclude($state) : void
    {
        ...
        if ($token->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_INCLUDE_NEW || $token->getSubMatch('type') === 'FILE') {
            $node = $this->builder->includeFile($filename, $token->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_INCLUDE_NEW, $condition, $token->getLine());
        } else {
            ...
            switch ($key) {
                case "extensions":
                    if ($token->getSubMatch('type') === 'FILE') {
                        throw new \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\ParseError("FILE includes may not have an 'extension' attribute", 0, $token->getLine());
                    }
                    $extensions = $value;
                    break;
                case "condition":
                    ...
                    break;
                default:
                    throw new \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\ParseError("unknown attribute '{$key}' found in INCLUDE statement", 0, $token->getLine());
            }
        }
    ...
    private function parseValueOperation($state) : void
    {
        switch ($state->token(1)->getType()) {
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_ASSIGNMENT:
                ...
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_COPY:
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_REFERENCE:
                $this->parseCopyOrReference($state);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_MODIFY:
                $this->parseModification($state);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_DELETE:
                $this->parseDeletion($state);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_RIGHTVALUE_MULTILINE:
    ...
    private function parseAssignment($state) : void
    {
        switch ($state->token(2)->getType()) {
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OBJECT_CONSTRUCTOR:
                ...
                $state->next(2);
                break;
            case \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_RIGHTVALUE:
    ...
    private function parseCopyOrReference($state) : void
    {
        ...
        $type = $state->token(1)->getType() === \RectorPrefix20210705\Helmich\TypoScriptParser\Tokenizer\TokenInterface::TYPE_OPERATOR_COPY ? 'copy' : 'reference';
    ...
    private function parseDeletion($state) : void
    {
        ...
        if (!\in_array($state->token(2)->getType(), $allowedTypesInDeletion, \true)) {
            throw new \RectorPrefix20210705\Helmich\TypoScriptParser\Parser\ParseError('Unexpected token ' . $state->token(2)->getType() . ' after delete operator (expected line break).', 1403011201, $state->token()->getLine());
        }
