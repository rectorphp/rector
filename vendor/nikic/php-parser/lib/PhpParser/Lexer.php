...
class Lexer
{
    ...
    protected function postprocessTokens(array &$tokens, \PhpParser\ErrorHandler $errorHandler) : void
    {
        ...
        if ($numTokens === 0) {
            ...
            $tokens[] = new \PhpParser\Token(0, "\x00", 1, 0);
            return;
        }
        ...
        if ($this->isUnterminatedComment($lastToken)) {
            $errorHandler->handleError(new \PhpParser\Error('Unterminated comment', ['startLine' => $lastToken->line, 'endLine' => $lastToken->getEndLine(), 'startFilePos' => $lastToken->pos, 'endFilePos' => $lastToken->getEndPos()]));
        }
        // Add sentinel token.
        $tokens[] = new \PhpParser\Token(0, "\x00", $lastToken->getEndLine(), $lastToken->getEndPos());
    }
}
