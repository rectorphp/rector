    ...
    private function extractDiff(array $trace, int $x, int $y, array $old, array $new) : array
    {
        ...
        for ($d = \count($trace) - 1; $d >= 0; $d--) {
            ...
            while ($x > $prevX && $y > $prevY) {
                $result[] = new \PhpParser\Internal\DiffElem(\PhpParser\Internal\DiffElem::TYPE_KEEP, $old[$x - 1], $new[$y - 1]);
                $x--;
                $y--;
            }
            if ($d === 0) {
            ...
            while ($x > $prevX) {
                $result[] = new \PhpParser\Internal\DiffElem(\PhpParser\Internal\DiffElem::TYPE_REMOVE, $old[$x - 1], null);
                $x--;
            }
            while ($y > $prevY) {
                $result[] = new \PhpParser\Internal\DiffElem(\PhpParser\Internal\DiffElem::TYPE_ADD, null, $new[$y - 1]);
                $y--;
            }
        }
