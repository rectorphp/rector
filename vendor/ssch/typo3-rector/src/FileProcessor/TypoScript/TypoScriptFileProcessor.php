...
final class TypoScriptFileProcessor implements \Ssch\TYPO3Rector\Contract\Processor\ConfigurableProcessorInterface
{
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20220606\Webmozart\Assert\Assert::isArray($allowedFileExtensions);
        \RectorPrefix20220606\Webmozart\Assert\Assert::allString($allowedFileExtensions);
    ...
    private function processFile(\Rector\Core\ValueObject\Application\File $file) : void
    {
        try {
            ...
            $tabMatches = \RectorPrefix20220606\Nette\Utils\Strings::match($file->getFileContent(), "#^\n#");
            $indentStyle = $tabMatches ? 'tab' : 'space';
            $prettyPrinterConfiguration = \RectorPrefix20220606\Helmich\TypoScriptParser\Parser\Printer\PrettyPrinterConfiguration::create();
        ...
        } catch (\RectorPrefix20220606\Helmich\TypoScriptParser\Tokenizer\TokenizerException $exception) {
            return;
        } catch (\RectorPrefix20220606\Helmich\TypoScriptParser\Parser\ParseError $exception) {
            $smartFileInfo = $file->getSmartFileInfo();
    ...
    {
        return \array_filter($this->typoScriptRectors, function (\RectorPrefix20220606\Helmich\TypoScriptParser\Parser\Traverser\Visitor $visitor) : bool {
    ...
    private function filterRemovedStatements(array $originalStatements, \Rector\Core\ValueObject\Application\File $file) : array
    {
        ...
        foreach ($originalStatements as $originalStatement) {
            ...
            if ($originalStatement instanceof \RectorPrefix20220606\Helmich\TypoScriptParser\Parser\AST\NestedAssignment) {
