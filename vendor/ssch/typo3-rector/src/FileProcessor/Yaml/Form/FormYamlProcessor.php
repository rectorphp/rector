...
final class FormYamlProcessor implements \Rector\Core\Contract\Processor\FileProcessorInterface
{
    ...
    private function processFile(\Rector\Core\ValueObject\Application\File $file) : void
    {
        ...
        $smartFileInfo = $file->getSmartFileInfo();
        $yaml = \RectorPrefix20210608\Symfony\Component\Yaml\Yaml::parseFile($smartFileInfo->getRealPath());
        if (!\is_array($yaml)) {
            return;
        }
        $newYaml = $yaml;
        ...
        $newFileContent = \RectorPrefix20210608\Symfony\Component\Yaml\Yaml::dump($newYaml, 99);
