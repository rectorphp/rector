...
use RectorPrefix20210715\Symfony\Component\Yaml\Yaml;
/**
...
final class FormYamlFileProcessor implements \Rector\Core\Contract\Processor\FileProcessorInterface
{
    ...
    public function process($file, $configuration) : void
    {
        ...
        $yaml = \RectorPrefix20210715\Symfony\Component\Yaml\Yaml::parseFile($smartFileInfo->getRealPath());
        if (!\is_array($yaml)) {
            return;
        }
        $newYaml = $yaml;
        ...
        $newFileContent = \RectorPrefix20210715\Symfony\Component\Yaml\Yaml::dump($newYaml, 99);
    ...
    {
        $smartFileInfo = $file->getSmartFileInfo();
        return \RectorPrefix20210715\Nette\Utils\Strings::endsWith($smartFileInfo->getFilename(), 'yaml');
    }
    /**
     * @return string[]
     */
