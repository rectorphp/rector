...
final class GetTemporaryImageWithTextRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Resource\\Processing\\LocalImageProcessor'))) {
            return null;
        }
        if (!$this->isName($node->name, 'getTemporaryImageWithText')) {
            return null;
        }
        return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Imaging\\GraphicalFunctions')]), 'getTemporaryImageWithText', $node->args);
    }
    /**
     * @codeCoverageIgnore
     */
