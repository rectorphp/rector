...
CODE_SAMPLE
    ...
    private function shouldSkip(\PhpParser\Node\Stmt\ClassMethod $node) : bool
    {
        ...
        if (!\RectorPrefix20210518\Nette\Utils\Strings::endsWith($methodName, 'Action')) {
            return \true;
        }
        if (\RectorPrefix20210518\Nette\Utils\Strings::startsWith($methodName, 'initialize')) {
            return \true;
        }
    ...
    private function alreadyResponseReturnType(\PhpParser\Node\Stmt\ClassMethod $node) : bool
    {
        foreach ($this->extractReturnCalls($node) as $returnCall) {
            ...
            if ($returnType->isSuperTypeOf(new \PHPStan\Type\ObjectType('RectorPrefix20210518\\Psr\\Http\\Message\\ResponseInterface'))->yes()) {
                return \true;
            }
        }
        return \false;
