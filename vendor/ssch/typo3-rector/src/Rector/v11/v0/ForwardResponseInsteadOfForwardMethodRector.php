...
CODE_SAMPLE
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        foreach ($forwardMethodCalls as $forwardMethodCall) {
            ...
            $forwardResponse = new \PhpParser\Node\Expr\New_(new \PhpParser\Node\Name\FullyQualified('RectorPrefix20210518\\TYPO3\\CMS\\Extbase\\Http\\ForwardResponse'), $args);
        ...
        if (\is_countable($node->stmts) && 1 === \count($node->stmts)) {
            $node->returnType = new \PhpParser\Node\Name\FullyQualified('RectorPrefix20210518\\Psr\\Http\\Message\\ResponseInterface');
        }
        return $node;
    }
    /**
    ...
    private function extractForwardMethodCalls(\PhpParser\Node\Stmt\ClassMethod $node) : array
    {
        return $this->betterNodeFinder->find((array) $node->stmts, function (\PhpParser\Node $node) : bool {
            ...
            if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Extbase\\Mvc\\Controller\\ActionController'))) {
                return \false;
            }
