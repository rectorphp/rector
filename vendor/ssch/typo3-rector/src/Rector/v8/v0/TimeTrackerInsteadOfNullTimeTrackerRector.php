...
final class TimeTrackerInsteadOfNullTimeTrackerRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        $renamedNode = $this->classRenamer->renameNode($node, ['RectorPrefix20210518\\TYPO3\\CMS\\Core\\TimeTracker\\NullTimeTracker' => 'RectorPrefix20210518\\TYPO3\\CMS\\Core\\TimeTracker\\TimeTracker']);
        if (null === $renamedNode) {
            return null;
        }
...
CODE_SAMPLE
    ...
    private function addAdditionalArgumentIfNeeded(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->valueResolver->isValue($node->args[0]->value, 'RectorPrefix20210518\\TYPO3\\CMS\\Core\\TimeTracker\\NullTimeTracker')) {
            return null;
        }
    ...
    private function isObjectManagerCall(\PhpParser\Node $node) : bool
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Extbase\\Object\\ObjectManager'))) {
            return \false;
        }
