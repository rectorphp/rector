...
CODE_SAMPLE
    ...
    private function isLanguageServiceCall(\PhpParser\Node $node) : bool
    {
        ...
        if ($this->isObjectType($node->var, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Localization\\LanguageService'))) {
            return \true;
        }
    ...
    private function refactorLanguageServiceCall(\PhpParser\Node $node) : ?\PhpParser\Node\Expr\StaticCall
    {
        ...
        if ($this->isName($node->name, 'csConvObj')) {
            return $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Charset\\CharsetConverter')]);
        }
        if ($this->isName($node->name, 'parserFactory')) {
            return $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Localization\\LocalizationFactory')]);
        }
        return null;
    }
}
