...
final class RteHtmlParserRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        if ($this->isName($node->name, 'siteUrl')) {
            return $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'getIndpEnv', [$this->nodeFactory->createArg(new \PhpParser\Node\Scalar\String_('TYPO3_SITE_URL'))]);
        }
        if ($this->isName($node->name, 'getUrl')) {
            return $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'getUrl', [$node->args[0]]);
        }
        return null;
    }
    /**
...
CODE_SAMPLE
    ...
    {
        return !$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Html\\RteHtmlParser'));
    }
}
