...
final class UseFileGetContentsForGetUrlRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        if (\preg_match('#^(?:http|ftp)s?|s(?:ftp|cp):#', $urlValue)) {
            return $this->nodeFactory->createMethodCall($this->nodeFactory->createMethodCall($this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Http\\RequestFactory')]), 'request', $node->args), 'getBody'), 'getContents');
        }
