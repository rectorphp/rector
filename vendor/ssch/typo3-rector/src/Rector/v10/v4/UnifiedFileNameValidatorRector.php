...
final class UnifiedFileNameValidatorRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        if ($node instanceof \PhpParser\Node\Expr\StaticCall && $this->isMethodVerifyFilenameAgainstDenyPattern($node)) {
            return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Resource\\Security\\FileNameValidator')]), 'isValid', $node->args);
        }
        if ($this->isConstFileDenyPatternDefault($node)) {
            return $this->nodeFactory->createClassConstFetch('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Resource\\Security\\FileNameValidator', 'DEFAULT_FILE_DENY_PATTERN');
        }
        return null;
    }
    /**
...
CODE_SAMPLE
    ...
    {
        return $node instanceof \PhpParser\Node\Expr\StaticCall && $this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility')) && $this->isName($node->name, 'verifyFilenameAgainstDenyPattern');
    }
    /**
     * @param ConstFetch|StaticCall $node
     */
