...
final class SwiftMailerBasedMailMessageToMailerBasedMessageRector extends AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new ObjectType('TYPO3\\CMS\\Core\\Mail\\MailMessage'))) {
            return null;
        }
    ...
    {
        return new RuleDefinition('New Mail API based on symfony/mailer and symfony/mime', [new CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    private function refactorAttachMethod(MethodCall $methodCall) : ?Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, new ObjectType('Swift_Attachment'))) {
            return null;
        }
    ...
    private function refactorEmbedMethod(MethodCall $methodCall) : ?Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, new ObjectType('Swift_Image'))) {
            return null;
        }
        if (!$this->isName($firstArgument->name, 'fromPath')) {
            return null;
