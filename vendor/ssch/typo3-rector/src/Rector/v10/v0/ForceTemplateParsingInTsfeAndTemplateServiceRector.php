...
CODE_SAMPLE
    ...
    {
        $staticCallContext = $this->createContext();
        $staticCallAspect = $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', self::MAKE_INSTANCE, [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Context\\TypoScriptAspect'), new \PhpParser\Node\Expr\ConstFetch(new \PhpParser\Node\Name('true'))]);
    ...
    {
        return $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', self::MAKE_INSTANCE, [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Context\\Context')]);
    }
    private function isPropertyForceTemplateParsing(\PhpParser\Node $node) : bool
    {
        ...
        if ($this->isObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Frontend\\Controller\\TypoScriptFrontendController'))) {
            return \true;
        }
        if ($this->isObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\TypoScript\\TemplateService'))) {
            return \true;
        }
