...
final class UseAddJsFileInsteadOfLoadJavascriptLibRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Backend\\Template\\ModuleTemplate'))) {
            return null;
        }
        if (!$this->isName($node->name, 'loadJavascriptLib')) {
            return null;
        }
        return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Page\\PageRenderer')]), 'addJsFile', $node->args);
    }
    /**
     * @codeCoverageIgnore
     */
