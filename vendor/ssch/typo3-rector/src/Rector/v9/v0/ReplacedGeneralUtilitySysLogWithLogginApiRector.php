...
final class ReplacedGeneralUtilitySysLogWithLogginApiRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility'))) {
            return null;
        }
        ...
        $makeInstanceCall = $this->nodeFactory->createStaticCall('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Utility\\GeneralUtility', 'makeInstance', [$this->nodeFactory->createClassConstReference('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Log\\LogManager')]);
        $loggerCall = $this->nodeFactory->createMethodCall($makeInstanceCall, 'getLogger', [new \PhpParser\Node\Scalar\MagicConst\Class_()]);
        $args = [];
        $severity = $this->nodeFactory->createClassConstFetch('RectorPrefix20210518\\TYPO3\\CMS\\Core\\Log\\LogLevel', 'INFO');
