...
CODE_SAMPLE;
    ...
    {
        if (!$this->isObjectType($node, new \PHPStan\Type\ObjectType('RectorPrefix20210518\\TYPO3\\CMS\\Extbase\\Mvc\\Controller\\CommandController'))) {
            return null;
        }
        ...
        $commandNamespace = \sprintf('RectorPrefix20210518\\%s\\%s\\Command', $vendorName, $extensionName);
        ...
        foreach ($commandMethods as $commandMethod) {
            ...
            $commandName = \RectorPrefix20210518\Nette\Utils\Strings::firstUpper($commandMethodName);
            ...
            $newCommandName = \sprintf('%s:%s', \RectorPrefix20210518\Nette\Utils\Strings::lower($vendorName), \RectorPrefix20210518\Nette\Utils\Strings::lower($commandName));
...
CODE_SAMPLE
    ...
    protected function findCommandMethods(\PhpParser\Node\Stmt\Class_ $node) : array
    {
        return $this->betterNodeFinder->find($node->stmts, function (\PhpParser\Node $node) {
            ...
            return \RectorPrefix20210518\Nette\Utils\Strings::endsWith($methodName, 'Command');
        });
    }
    /**
