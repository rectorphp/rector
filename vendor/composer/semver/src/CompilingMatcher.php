...
class CompilingMatcher
{
     ...
     * @return mixed
     */
    public static function match(\RectorPrefix20220207\Composer\Semver\Constraint\ConstraintInterface $constraint, $operator, $version)
    {
        if (self::$enabled === null) {
            self::$enabled = !\in_array('eval', \explode(',', (string) \ini_get('disable_functions')), \true);
        }
        if (!self::$enabled) {
            return $constraint->matches(new \RectorPrefix20220207\Composer\Semver\Constraint\Constraint(self::$transOpInt[$operator], $version));
        }
