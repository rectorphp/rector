...
use RectorPrefix20220109\Psr\Log\LogLevel;
/**
...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \RectorPrefix20220109\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger !== null) {
            $this->logger->log($level !== null ? $level : \RectorPrefix20220109\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \RectorPrefix20220109\Psr\Log\LogLevel::WARNING);
    }
    /**
     * @param string $info
     *
    ...
    {
        $this->output($this->getLoadedMessage());
        \RectorPrefix20220109\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    /**
     * @return void
     */
    private function reportRestarted()
    {
        ...
        $level = $this->loaded !== null ? \RectorPrefix20220109\Psr\Log\LogLevel::WARNING : null;
