...
use RectorPrefix20220317\Psr\Log\LogLevel;
/**
...
class Status
{
    ...
    public function __construct(string $envAllowXdebug, bool $debug)
    {
        ...
        \RectorPrefix20220317\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
     ...
     * @return void
     */
    public function setLogger(\RectorPrefix20220317\Psr\Log\LoggerInterface $logger) : void
    {
        $this->logger = $logger;
    }
    /**
    ...
    {
        if ($this->logger !== null) {
            $this->logger->log($level !== null ? $level : \RectorPrefix20220317\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \RectorPrefix20220317\Psr\Log\LogLevel::WARNING);
    }
    /**
     * Info status message
     */
    ...
    {
        $this->output($this->getLoadedMessage());
        \RectorPrefix20220317\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    /**
    ...
    private function reportRestarted() : void
    {
        ...
        $level = $this->loaded !== null ? \RectorPrefix20220317\Psr\Log\LogLevel::WARNING : null;
