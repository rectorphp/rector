...
class Process
{
    ...
    public static function escape(string $arg, bool $meta = \true, bool $module = \false) : string
    {
        ...
        $arg = \RectorPrefix20220412\Composer\Pcre\Preg::replace('/(\\\\*)"/', '$1$1\\"', $arg, -1, $dquotes);
        if ($meta) {
            $meta = $dquotes || \RectorPrefix20220412\Composer\Pcre\Preg::isMatch('/%[^%]+%/', $arg);
            if (!$meta) {
        ...
        if ($quote) {
            $arg = '"' . \RectorPrefix20220412\Composer\Pcre\Preg::replace('/(\\\\*)$/', '$1$1', $arg) . '"';
        }
        if ($meta) {
            $arg = \RectorPrefix20220412\Composer\Pcre\Preg::replace('/(["^&|<>()%])/', '^$1', $arg);
        }
        return $arg;
    }
    /**
