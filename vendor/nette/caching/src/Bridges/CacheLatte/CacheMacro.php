...
final class CacheMacro implements Latte\IMacro
{
    ...
    public function nodeOpened(Latte\MacroNode $node)
    {
        ...
        $this->used = \true;
        $node->empty = \false;
        $node->openingCode = Latte\PhpWriter::using($node)->write('<?php if (Nette\\Bridges\\CacheLatte\\CacheMacro::createCache($this->global->cacheStorage, %var, $this->global->cacheStack, %node.array?)) /* line %var */ try { ?>', Nette\Utils\Random::generate(), $node->startLine);
    }
    /**
     * Node is closed.
     * @return void
    ...
    {
        $node->closingCode = Latte\PhpWriter::using($node)->write('<?php
