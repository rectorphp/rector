 ...
 */
class MemcachedStorage implements \RectorPrefix20210524\Nette\Caching\Storage, \RectorPrefix20210524\Nette\Caching\BulkReader
{
    ...
    public function __construct(string $host = 'localhost', int $port = 11211, string $prefix = '', \RectorPrefix20210524\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!static::isAvailable()) {
            throw new \RectorPrefix20210524\Nette\NotSupportedException("PHP extension 'memcached' is not loaded.");
        }
        $this->prefix = $prefix;
    ...
    public function addServer(string $host = 'localhost', int $port = 11211) : void
    {
        if (@$this->memcached->addServer($host, $port, 1) === \false) {
            ...
            throw new \RectorPrefix20210524\Nette\InvalidStateException("Memcached::addServer(): {$error['message']}.");
        }
    }
    ...
    public function write(string $key, $data, array $dp) : void
    {
        if (isset($dp[\RectorPrefix20210524\Nette\Caching\Cache::ITEMS])) {
            throw new \RectorPrefix20210524\Nette\NotSupportedException('Dependent items are not supported by MemcachedStorage.');
        }
        $key = \urlencode($this->prefix . $key);
        $meta = [self::META_DATA => $data];
        $expire = 0;
        if (isset($dp[\RectorPrefix20210524\Nette\Caching\Cache::EXPIRATION])) {
            $expire = (int) $dp[\RectorPrefix20210524\Nette\Caching\Cache::EXPIRATION];
