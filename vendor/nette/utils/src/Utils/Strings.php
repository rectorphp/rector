...
class Strings
{
    ...
    public static function chr(int $code) : string
    {
        if ($code < 0 || $code >= 0xd800 && $code <= 0xdfff || $code > 0x10ffff) {
            throw new \RectorPrefix20220223\Nette\InvalidArgumentException('Code point must be in range 0x0 to 0xD7FF or 0xE000 to 0x10FFFF.');
        } elseif (!\extension_loaded('iconv')) {
            throw new \RectorPrefix20220223\Nette\NotSupportedException(__METHOD__ . '() requires ICONV extension that is not loaded.');
        }
    ...
    public static function substring(string $s, int $start, ?int $length = null) : string
    {
        if (\function_exists('mb_substr')) {
            ...
            throw new \RectorPrefix20220223\Nette\NotSupportedException(__METHOD__ . '() requires extension ICONV or MBSTRING, neither is loaded.');
        } elseif ($length === null) {
    ...
    private static function pos(string $haystack, string $needle, int $nth = 1) : ?int
    {
        ...
        return \RectorPrefix20220223\Nette\Utils\Helpers::falseToNull($pos);
    }
    /**
    ...
    public static function replace(string $subject, $pattern, $replacement = '', int $limit = -1) : string
    {
        if (\is_object($replacement) || \is_array($replacement)) {
            if (!\is_callable($replacement, \false, $textual)) {
                throw new \RectorPrefix20220223\Nette\InvalidStateException("Callback '{$textual}' is not callable.");
            }
    ...
    {
        $res = \RectorPrefix20220223\Nette\Utils\Callback::invokeSafe($func, $args, function (string $message) use($args) : void {
            // compile-time error, not detectable by preg_last_error
            throw new \RectorPrefix20220223\Nette\Utils\RegexpException($message . ' in pattern: ' . \implode(' or ', (array) $args[0]));
        });
        if (($code = \preg_last_error()) && ($res === null || !\in_array($func, ['preg_filter', 'preg_replace_callback', 'preg_replace'], \true))) {
            throw new \RectorPrefix20220223\Nette\Utils\RegexpException((\RectorPrefix20220223\Nette\Utils\RegexpException::MESSAGES[$code] ?? 'Unknown error') . ' (pattern: ' . \implode(' or ', (array) $args[0]) . ')', $code);
        }
        return $res;
    }
}
