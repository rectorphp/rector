...
namespace RectorPrefix20211123\Nette\Utils;

use RectorPrefix20211123\Nette;
/**
...
class Image
{
     ...
     */
    public static function rgb($red, $green, $blue, $transparency = 0) : array
    {
    ...
    public static function fromFile($file, &$type = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \RectorPrefix20211123\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
        $type = self::detectTypeFromFile($file);
        if (!$type) {
            throw new \RectorPrefix20211123\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
        $method = 'imagecreatefrom' . self::FORMATS[$type];
        return new static(\RectorPrefix20211123\Nette\Utils\Callback::invokeSafe($method, [$file], function (string $message) : void {
            throw new \RectorPrefix20211123\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromString($s, &$type = null)
    {
        ...
        if (!$type) {
            throw new \RectorPrefix20211123\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
        return new static(\RectorPrefix20211123\Nette\Utils\Callback::invokeSafe('imagecreatefromstring', [$s], function (string $message) : void {
            throw new \RectorPrefix20211123\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromBlank($width, $height, $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \RectorPrefix20211123\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
     ...
     * @param string $file
     */
    public static function detectTypeFromFile($file) : ?int
    {
     ...
     * @param string $s
     */
    public static function detectTypeFromString($s) : ?int
    {
     ...
     * @param int $type
     */
    public static function typeToExtension($type) : string
    {
        if (!isset(self::FORMATS[$type])) {
            throw new \RectorPrefix20211123\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
     ...
     * @param int $type
     */
    public static function typeToMimeType($type) : string
    {
    ...
    protected function setImageResource($image)
    {
        if (!$image instanceof \GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
            throw new \RectorPrefix20211123\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
     ...
     * @param int $flags
     */
    public static function calculateSize($srcWidth, $srcHeight, $newWidth, $newHeight, $flags = self::FIT) : array
    {
        if ($newWidth === null) {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (!$newWidth || !$newHeight) {
                throw new \RectorPrefix20211123\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (!$newWidth && !$newHeight) {
                throw new \RectorPrefix20211123\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
     ...
     * @param int $srcHeight
     */
    public static function calculateCutout($srcWidth, $srcHeight, $left, $top, $newWidth, $newHeight) : array
    {
     ...
     * @param int|null $type
     */
    public function save($file, $quality = null, $type = null) : void
    {
        if ($type === null) {
            $extensions = \array_flip(self::FORMATS) + ['jpg' => self::JPEG];
            $ext = \strtolower(\pathinfo($file, \PATHINFO_EXTENSION));
            if (!isset($extensions[$ext])) {
                throw new \RectorPrefix20211123\Nette\InvalidArgumentException("Unsupported file extension '{$ext}'.");
            }
            $type = $extensions[$ext];
        }
     ...
     * @param int|null $quality
     */
    public function toString($type = self::JPEG, $quality = null) : string
    {
        return \RectorPrefix20211123\Nette\Utils\Helpers::capture(function () use($type, $quality) {
     ...
     * @param int|null $quality
     */
    public function send($type = self::JPEG, $quality = null) : void
    {
    ...
    private function output(int $type, ?int $quality, string $file = null) : void
    {
        ...
        if (!$success) {
            throw new \RectorPrefix20211123\Nette\Utils\ImageException(\RectorPrefix20211123\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \RectorPrefix20211123\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
    ...
    private static function isPercent(&$num) : bool
    {
        ...
        throw new \RectorPrefix20211123\Nette\InvalidArgumentException("Expected dimension in int|string, '{$num}' given.");
    }
    /**
    ...
    {
        throw new \RectorPrefix20211123\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
