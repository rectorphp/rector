...
final class Process
{
    ...
    private const PollInterval = 10000;
    ...
    private array $status = ['running' => \true];
    /** @var resource */
    private $inputPipe;
     ...
     */
    public static function runExecutable(string $executable, array $arguments = [], ?array $env = null, array $options = [], $stdin = '', $stdout = null, $stderr = null, ?string $directory = null, ?float $timeout = self::DefaultTimeout): self
    {
        return new self(array_merge([$executable], $arguments), $env, $options, $directory, $stdin, $stdout, $stderr, $timeout);
    }
    /**
     ...
     * @param  float|null     $timeout    Time limit in seconds, checked while waiting for or reading the process; null disables it.
     */
    public static function runCommand(string $command, ?array $env = null, array $options = [], $stdin = '', $stdout = null, $stderr = null, ?string $directory = null, ?float $timeout = self::DefaultTimeout): self
    {
     ...
     * @param mixed $stderr
     */
    private function __construct($command, ?array $env, array $options, ?string $directory, $stdin, $stdout, $stderr, ?float $timeout)
    {
        $this->timeout = $timeout;
        $descriptors = [self::StdIn => $this->createInputDescriptor($stdin), self::StdOut => $this->createOutputDescriptor(self::StdOut, $stdout), self::StdErr => $this->createOutputDescriptor(self::StdErr, $stderr)];
        ...
        if ($stdin instanceof self) {
            ...
            unset($stdin->outputBuffers[self::StdOut], $stdin->outputBufferOffsets[self::StdOut], $stdin->outputPipes[self::StdOut]);
        }
    ...
    public function terminate(): void
    {
        if (!$this->isRunning()) {
            ...
            // 9 = SIGKILL: cannot be trapped, so the following proc_close() won't hang
        }
    ...
    {
        $this->wait();
        if (!isset($this->outputBuffers[self::StdErr])) {
            throw new Nette\InvalidStateException('Cannot read output: it is not captured (it was redirected, discarded or piped).');
        }
        return $this->outputBuffers[self::StdErr];
    }
    /**
    ...
    private function extractNewData(int $id): string
    {
        ...
        $res = (string) substr($this->outputBuffers[$id], $this->outputBufferOffsets[$id]);
    ...
    private function writeToPipe(string $string): void
    {
        ...
        for ($written = 0; $written < $length; $written += $bytes) {
            $bytes = fwrite($this->inputPipe, (string) substr($string, $written));
            if (!$bytes) {
                break;
            }
        }
    ...
    {
        if ($this->timeout !== null && microtime(\true) - $this->startTime >= $this->timeout) {
            $this->terminate();
     ...
     * @param mixed $input
     */
    private function writeInitialInput($input): void
    {
     ...
     * @return mixed
     */
    private function createInputDescriptor($input)
    {
     ...
     * @return mixed
     */
    private function createOutputDescriptor(int $id, $output)
    {
        if (is_resource($output)) {
            $this->callerOutputs[$id] = \true;
            return $output;
