...
final class Parser
{
    ...
    {
        $res = new \RectorPrefix20211125\Nette\Neon\Node\ArrayNode($indent, $this->tokens->getPos());
        $keyCheck = [];
        loop:
    ...
    private function parseValue() : \RectorPrefix20211125\Nette\Neon\Node
    {
        if ($token = $this->tokens->consume(\RectorPrefix20211125\Nette\Neon\Token::STRING)) {
            $node = new \RectorPrefix20211125\Nette\Neon\Node\StringNode($this->decodeString($token->value), $this->tokens->getPos() - 1);
        } elseif ($token = $this->tokens->consume(\RectorPrefix20211125\Nette\Neon\Token::LITERAL)) {
            $pos = $this->tokens->getPos() - 1;
            $node = new \RectorPrefix20211125\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value, $this->tokens->isNext(':', '=')), $pos);
        } elseif ($this->tokens->isNext('[', '(', '{')) {
    ...
    private function parseEntity(\RectorPrefix20211125\Nette\Neon\Node $node) : \RectorPrefix20211125\Nette\Neon\Node
    {
        ...
        while ($token = $this->tokens->consume(\RectorPrefix20211125\Nette\Neon\Token::LITERAL)) {
            $valueNode = new \RectorPrefix20211125\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value), $this->tokens->getPos() - 1);
            if ($this->tokens->isNext('(')) {
                $attributes = $this->parseBraces();
    ...
    private function parseBraces() : \RectorPrefix20211125\Nette\Neon\Node\ArrayNode
    {
        $token = $this->tokens->consume();
        $endBrace = ['[' => ']', '{' => '}', '(' => ')'][$token->value];
        $res = new \RectorPrefix20211125\Nette\Neon\Node\ArrayNode(null, $this->tokens->getPos() - 1);
        $keyCheck = [];
        loop:
        while ($this->tokens->consume(\RectorPrefix20211125\Nette\Neon\Token::NEWLINE)) {
        }
