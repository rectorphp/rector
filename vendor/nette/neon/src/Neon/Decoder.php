...
namespace RectorPrefix20210801\Nette\Neon;
...
final class Decoder
{
    ...
    public function decode(string $input)
    {
        ...
        if ($this->tokens === \false) {
            throw new \RectorPrefix20210801\Nette\Neon\Exception('Invalid UTF-8 sequence.');
        }
        ...
        for (; $n < $count; $n++) {
            ...
            if ($t === ',') {
                ...
                if ($hasValue) {
                    ...
                    $n++;
                    if ($value instanceof \RectorPrefix20210801\Nette\Neon\Entity && $value->value === \RectorPrefix20210801\Nette\Neon\Neon::CHAIN) {
                        \end($value->attributes)->attributes = $this->parse(\false, []);
                    } else {
                        $value = new \RectorPrefix20210801\Nette\Neon\Entity($value, $this->parse(\false, []));
                    }
                } else {
                    $n++;
                ...
                if ($hasValue) {
                    if ($value instanceof \RectorPrefix20210801\Nette\Neon\Entity) {
                        // Entity chaining
                        if ($value->value !== \RectorPrefix20210801\Nette\Neon\Neon::CHAIN) {
                            $value = new \RectorPrefix20210801\Nette\Neon\Entity(\RectorPrefix20210801\Nette\Neon\Neon::CHAIN, [$value]);
                        }
                        $value->attributes[] = new \RectorPrefix20210801\Nette\Neon\Entity($converted);
                    } else {
                        $this->error();
                    }
                } else {
    ...
    private function error(string $message = "Unexpected '%s'")
    {
        ...
        throw new \RectorPrefix20210801\Nette\Neon\Exception(\str_replace('%s', $token, $message) . " on line {$line}, column {$col}.");
    }
}
