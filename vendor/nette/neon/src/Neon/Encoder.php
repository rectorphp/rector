...
final class Encoder
{
    ...
    public function valueToNode($val, bool $blockMode = \false) : \RectorPrefix20220420\Nette\Neon\Node
    {
        if ($val instanceof \DateTimeInterface) {
            ...
            return $node;
        } elseif ($val instanceof \RectorPrefix20220420\Nette\Neon\Entity) {
            return new \RectorPrefix20220420\Nette\Neon\Node\EntityNode($this->valueToNode($val->value), $this->arrayToNodes((array) $val->attributes));
        } elseif (\is_object($val) || \is_array($val)) {
            if ($blockMode) {
                $node = new \RectorPrefix20220420\Nette\Neon\Node\BlockArrayNode();
            } else {
                $isList = \is_array($val) && (!$val || \array_keys($val) === \range(0, \count($val) - 1));
                $node = new \RectorPrefix20220420\Nette\Neon\Node\InlineArrayNode($isList ? '[' : '{');
            }
            $node->items = $this->arrayToNodes($val, $blockMode);
            return $node;
        } elseif (\is_string($val) && \RectorPrefix20220420\Nette\Neon\Lexer::requiresDelimiters($val)) {
            return new \RectorPrefix20220420\Nette\Neon\Node\StringNode($val);
        } else {
            return new \RectorPrefix20220420\Nette\Neon\Node\LiteralNode($val);
        }
    }
    ...
    private function arrayToNodes($val, bool $blockMode = \false) : array
    {
        ...
        $hide = \true;
        foreach ($val as $k => $v) {
            $res[] = $item = new \RectorPrefix20220420\Nette\Neon\Node\ArrayItemNode();
