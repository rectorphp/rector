...
/** @internal */
final class ArrayItemNode extends \RectorPrefix20220124\Nette\Neon\Node
{
    /** @var ?Node */
    public $key;
    /** @var Node */
    ...
    public static function itemsToBlockString(array $items) : string
    {
        ...
        foreach ($items as $item) {
            ...
            $res .= ($item->key ? $item->key->toString() . ':' : '-') . ($item->value instanceof \RectorPrefix20220124\Nette\Neon\Node\BlockArrayNode && $item->value->items ? "\n" . $v . (\substr($v, -2, 1) === "\n" ? '' : "\n") : ' ' . $v . "\n");
        }
        return $res;
    }
