...
final class Traverser
{
     ...
     */
    public function traverse(\RectorPrefix20220531\Nette\Neon\Node $node, ?callable $enter = null, ?callable $leave = null) : \RectorPrefix20220531\Nette\Neon\Node
    {
        $this->enter = $enter;
        $this->leave = $leave;
        $this->stop = \false;
    ...
    private function traverseNode(\RectorPrefix20220531\Nette\Neon\Node $node) : \RectorPrefix20220531\Nette\Neon\Node
    {
        $children = \true;
        if ($this->enter) {
            $res = ($this->enter)($node);
            if ($res instanceof \RectorPrefix20220531\Nette\Neon\Node) {
                $node = $res;
