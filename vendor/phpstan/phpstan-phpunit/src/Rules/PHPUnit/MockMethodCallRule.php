...
use RectorPrefix20220606\PHPUnit\Framework\MockObject\Stub;
...
class MockMethodCallRule implements \PHPStan\Rules\Rule
{
    ...
    public function processNode(\PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) : array
    {
        ...
        if ($type instanceof \PHPStan\Type\IntersectionType && (\in_array(\RectorPrefix20220606\PHPUnit\Framework\MockObject\MockObject::class, $type->getReferencedClasses(), \true) || \in_array(\RectorPrefix20220606\PHPUnit\Framework\MockObject\Stub::class, $type->getReferencedClasses(), \true)) && !$type->hasMethod($method)->yes()) {
            $mockClass = \array_filter($type->getReferencedClasses(), static function (string $class) : bool {
                return $class !== \RectorPrefix20220606\PHPUnit\Framework\MockObject\MockObject::class && $class !== \RectorPrefix20220606\PHPUnit\Framework\MockObject\Stub::class;
            });
            return [\sprintf('Trying to mock an undefined method %s() on class %s.', $method, \implode('&', $mockClass))];
        }
        if ($type instanceof \PHPStan\Type\Generic\GenericObjectType && $type->getClassName() === \RectorPrefix20220606\PHPUnit\Framework\MockObject\Builder\InvocationMocker::class && \count($type->getTypes()) > 0) {
