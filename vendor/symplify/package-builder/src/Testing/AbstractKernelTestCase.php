...
namespace RectorPrefix20211101\Symplify\PackageBuilder\Testing;

use RectorPrefix20211101\PHPUnit\Framework\TestCase;
use ReflectionClass;
use RectorPrefix20211101\Symfony\Component\Console\Output\OutputInterface;
use RectorPrefix20211101\Symfony\Component\Console\Style\SymfonyStyle;
use RectorPrefix20211101\Symfony\Component\DependencyInjection\ContainerInterface;
use RectorPrefix20211101\Symfony\Component\HttpKernel\KernelInterface;
use RectorPrefix20211101\Symfony\Contracts\Service\ResetInterface;
use RectorPrefix20211101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use RectorPrefix20211101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \RectorPrefix20211101\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService($type)
    {
        if (self::$container === null) {
            throw new \RectorPrefix20211101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \RectorPrefix20211101\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    {
        if (\is_a($kernelClass, \RectorPrefix20211101\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
        ...
        if (!$kernel instanceof \RectorPrefix20211101\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \RectorPrefix20211101\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \RectorPrefix20211101\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    {
        if ($kernel instanceof \RectorPrefix20211101\Symplify\SymplifyKernel\Contract\LightKernelInterface) {
            return;
        }
        if ($kernel instanceof \RectorPrefix20211101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \RectorPrefix20211101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \RectorPrefix20211101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\RectorPrefix20211101\Symfony\Component\HttpKernel\KernelInterface $kernel) : \RectorPrefix20211101\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \RectorPrefix20211101\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \RectorPrefix20211101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\RectorPrefix20211101\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\RectorPrefix20211101\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\RectorPrefix20211101\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
