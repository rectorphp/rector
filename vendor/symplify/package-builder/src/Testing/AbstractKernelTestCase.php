...
namespace RectorPrefix20220101\Symplify\PackageBuilder\Testing;
...
use RectorPrefix20220101\Symfony\Component\HttpKernel\KernelInterface;
use RectorPrefix20220101\Symfony\Contracts\Service\ResetInterface;
use RectorPrefix20220101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use RectorPrefix20220101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
use Symplify\SmartFileSystem\SmartFileInfo;
use RectorPrefix20220101\Symplify\SymplifyKernel\Contract\LightKernelInterface;
use RectorPrefix20220101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException;
/**
 * Inspiration
 *
 * @see https://github.com/symfony/symfony/blob/master/src/Symfony/Bundle/FrameworkBundle/Test/KernelTestCase.php
 */
abstract class AbstractKernelTestCase extends \RectorPrefix20220101\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService(string $type)
    {
        if (self::$container === null) {
            throw new \RectorPrefix20220101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \RectorPrefix20220101\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    {
        if (\is_a($kernelClass, \RectorPrefix20220101\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
        ...
        if (!$kernel instanceof \RectorPrefix20220101\Symfony\Component\HttpKernel\KernelInterface) {
            throw new \RectorPrefix20220101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \RectorPrefix20220101\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \RectorPrefix20220101\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    {
        if ($kernel instanceof \RectorPrefix20220101\Symplify\SymplifyKernel\Contract\LightKernelInterface) {
            return;
        }
        if ($kernel instanceof \RectorPrefix20220101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \RectorPrefix20220101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \RectorPrefix20220101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\RectorPrefix20220101\Symfony\Component\HttpKernel\KernelInterface $kernel) : \RectorPrefix20220101\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \RectorPrefix20220101\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \RectorPrefix20220101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\RectorPrefix20220101\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\RectorPrefix20220101\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\RectorPrefix20220101\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
