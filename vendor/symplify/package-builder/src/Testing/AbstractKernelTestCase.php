...
namespace RectorPrefix20210705\Symplify\PackageBuilder\Testing;

use RectorPrefix20210705\PHPUnit\Framework\TestCase;
use ReflectionClass;
...
use RectorPrefix20210705\Symfony\Component\DependencyInjection\ContainerInterface;
...
use RectorPrefix20210705\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use RectorPrefix20210705\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
use Symplify\SmartFileSystem\SmartFileInfo;
use RectorPrefix20210705\Symplify\SymplifyKernel\Exception\ShouldNotHappenException;
/**
 * Inspiration
 *
 * @see https://github.com/symfony/symfony/blob/master/src/Symfony/Bundle/FrameworkBundle/Test/KernelTestCase.php
 */
abstract class AbstractKernelTestCase extends \RectorPrefix20210705\PHPUnit\Framework\TestCase
{
    /**
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \RectorPrefix20210705\Symfony\Component\HttpKernel\KernelInterface
    {
     ...
     * @param string[]|SmartFileInfo[] $configs
     */
    protected function bootKernelWithConfigsAndStaticCache(string $kernelClass, array $configs) : \RectorPrefix20210705\Symfony\Component\HttpKernel\KernelInterface
    {
        // unwrap file infos to real paths
        $configFilePaths = $this->resolveConfigFilePaths($configs);
        $configsHash = $this->resolveConfigsHash($configFilePaths);
    ...
    protected function getService(string $type)
    {
        if (self::$container === null) {
            throw new \RectorPrefix20210705\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, crewate container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \RectorPrefix20210705\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \RectorPrefix20210705\Symfony\Component\HttpKernel\KernelInterface) {
            throw new \RectorPrefix20210705\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \RectorPrefix20210705\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    private function bootAndReturnKernel(\RectorPrefix20210705\Symfony\Component\HttpKernel\KernelInterface $kernel) : \RectorPrefix20210705\Symfony\Component\HttpKernel\KernelInterface
    {
        ...
        if (!$container instanceof \RectorPrefix20210705\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \RectorPrefix20210705\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\RectorPrefix20210705\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\RectorPrefix20210705\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\RectorPrefix20210705\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
