...
namespace RectorPrefix20210615\Symplify\Astral\Naming;
...
final class SimpleNameResolver
{
    ...
    public function isName($node, string $desiredName) : bool
    {
        ...
        if (\RectorPrefix20210615\Nette\Utils\Strings::contains($desiredName, '*')) {
    ...
    public function resolveShortNameFromNode(\PhpParser\Node\Stmt\ClassLike $classLike) : ?string
    {
        ...
        if (\RectorPrefix20210615\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
    ...
    public function isNameMatch(\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        ...
        return (bool) \RectorPrefix20210615\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
    public function resolveShortName(string $className) : string
    {
        if (!\RectorPrefix20210615\Nette\Utils\Strings::contains($className, '\\')) {
            return $className;
        }
        return (string) \RectorPrefix20210615\Nette\Utils\Strings::after($className, '\\', -1);
    }
}
