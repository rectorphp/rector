...
namespace RectorPrefix20220308\Symplify\ComposerJsonManipulator\Sorter;
...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return \RectorPrefix20220308\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
                if ($name === 'php') {
                    return '0-' . $name;
                }
    ...
    {
        return (bool) \RectorPrefix20220308\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
