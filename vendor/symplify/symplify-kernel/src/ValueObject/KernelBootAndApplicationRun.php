...
use RectorPrefix20210816\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \RectorPrefix20210816\Symfony\Component\HttpKernel\KernelInterface
    {
        ...
        $kernel = new $kernelClass($environment, \RectorPrefix20210816\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        if ($kernel instanceof \RectorPrefix20210816\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface && $this->extraConfigs !== []) {
            $kernel->setConfigs($this->extraConfigs);
        }
        $kernel->boot();
        $container = $kernel->getContainer();
        /** @var Application $application */
        $application = $container->get(\RectorPrefix20210816\Symfony\Component\Console\Application::class);
        exit($application->run());
    }
    private function setExtraConfigs(\RectorPrefix20210816\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
        if ($this->extraConfigs === []) {
            return;
        }
        if (\is_a($kernel, \RectorPrefix20210816\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class, \true)) {
            /** @var ExtraConfigAwareKernelInterface $kernel */
            $kernel->setConfigs($this->extraConfigs);
        } else {
            $message = \sprintf('Extra configs are set, but the "%s" kernel class is missing "%s" interface', $kernelClass, \RectorPrefix20210816\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class);
            throw new \RectorPrefix20210816\Symplify\SymplifyKernel\Exception\BootException($message);
        }
    }
    /**
     * @param class-string $kernelClass
    ...
    {
        if (!\is_a($kernelClass, \RectorPrefix20210816\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \RectorPrefix20210816\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \RectorPrefix20210816\Symplify\SymplifyKernel\Exception\BootException($message);
        }
