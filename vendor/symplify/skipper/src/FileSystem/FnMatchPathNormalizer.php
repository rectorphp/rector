...
namespace RectorPrefix20220415\Symplify\Skipper\FileSystem;
...
final class FnMatchPathNormalizer
{
    ...
    {
        // ends with *
        if (\RectorPrefix20220415\Nette\Utils\Strings::match($path, self::ONLY_ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\RectorPrefix20220415\Nette\Utils\Strings::match($path, self::ONLY_STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
