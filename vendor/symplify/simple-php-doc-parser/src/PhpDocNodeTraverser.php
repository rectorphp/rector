...
use RectorPrefix20220111\Symplify\SimplePhpDocParser\Exception\InvalidTraverseException;
use RectorPrefix20220111\Symplify\SimplePhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor;
/**
 * @api
 *
 * Mimics
...
final class PhpDocNodeTraverser
{
    ...
    public function addPhpDocNodeVisitor(\RectorPrefix20220111\Symplify\SimplePhpDocParser\Contract\PhpDocNodeVisitorInterface $phpDocNodeVisitor) : void
    {
    ...
    {
        $callablePhpDocNodeVisitor = new \RectorPrefix20220111\Symplify\SimplePhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor($callable, $docContent);
                ...
                foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                    ...
                    if ($return !== null) {
                        if ($return instanceof \PHPStan\PhpDocParser\Ast\Node) {
                            ...
                            throw new \RectorPrefix20220111\Symplify\SimplePhpDocParser\Exception\InvalidTraverseException('enterNode() returned invalid value of type ' . \gettype($return));
                        }
                    }
                }
                if ($traverseChildren) {
            ...
            foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                ...
                if ($return !== null) {
                    if ($return instanceof \PHPStan\PhpDocParser\Ast\Node) {
                        ...
                        throw new \RectorPrefix20220111\Symplify\SimplePhpDocParser\Exception\InvalidTraverseException('leaveNode() returned invalid value of type ' . \gettype($return));
                    }
                }
                if ($breakVisitorIndex === $visitorIndex) {
                    break;
