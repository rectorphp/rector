...
final class ProcessPool
{
     ...
     */
    private $tcpServer;
    public function __construct(\RectorPrefix20220407\React\Socket\TcpServer $tcpServer)
    {
        $this->tcpServer = $tcpServer;
    }
    public function getProcess(string $identifier) : \RectorPrefix20220407\Symplify\EasyParallel\ValueObject\ParallelProcess
    {
        if (!\array_key_exists($identifier, $this->processes)) {
            throw new \RectorPrefix20220407\Symplify\EasyParallel\Exception\ParallelShouldNotHappenException(\sprintf('Process "%s" not found.', $identifier));
        }
        return $this->processes[$identifier];
    }
    public function attachProcess(string $identifier, \RectorPrefix20220407\Symplify\EasyParallel\ValueObject\ParallelProcess $parallelProcess) : void
    {
