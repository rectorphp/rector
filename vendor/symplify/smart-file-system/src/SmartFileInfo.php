...
namespace RectorPrefix20220210\Symplify\SmartFileSystem;

use RectorPrefix20220210\Nette\Utils\Strings;
use RectorPrefix20220210\Symfony\Component\Finder\SplFileInfo;
use RectorPrefix20220210\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use RectorPrefix20220210\Symplify\EasyTesting\StaticFixtureSplitter;
use RectorPrefix20220210\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
use RectorPrefix20220210\Symplify\SmartFileSystem\Exception\FileNotFoundException;
/**
 * @see \Symplify\SmartFileSystem\Tests\SmartFileInfo\SmartFileInfoTest
 */
final class SmartFileInfo extends \RectorPrefix20220210\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \RectorPrefix20220210\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \RectorPrefix20220210\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
    ...
    {
        return \RectorPrefix20220210\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \RectorPrefix20220210\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\RectorPrefix20220210\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\RectorPrefix20220210\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
 ...
 * @see \Symplify\SmartFileSystem\Tests\SmartFileInfo\SmartFileInfoTest
 */
\class_alias('RectorPrefix20220210\\Symplify\\SmartFileSystem\\SmartFileInfo', 'Symplify\\SmartFileSystem\\SmartFileInfo', \false);
