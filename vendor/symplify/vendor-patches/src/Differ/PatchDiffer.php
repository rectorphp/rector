...
namespace RectorPrefix20211231\Symplify\VendorPatches\Differ;
...
final class PatchDiffer
{
     ...
     */
    private $differ;
    public function __construct(\RectorPrefix20211231\SebastianBergmann\Diff\Differ $differ)
    {
        $this->differ = $differ;
    }
    public function diff(\RectorPrefix20211231\Symplify\VendorPatches\ValueObject\OldAndNewFileInfo $oldAndNewFileInfo) : string
    {
        ...
        $clearedDiff = \RectorPrefix20211231\Nette\Utils\Strings::replace($diff, self::START_ORIGINAL_REGEX, '--- /dev/null');
        return \RectorPrefix20211231\Nette\Utils\Strings::replace($clearedDiff, self::START_NEW_REGEX, '+++ ' . $patchedFileRelativePath);
    }
    private function resolveFileInfoPathRelativeFilePath(\Symplify\SmartFileSystem\SmartFileInfo $beforeFileInfo) : string
    {
        $match = \RectorPrefix20211231\Nette\Utils\Strings::match($beforeFileInfo->getRealPath(), self::LOCAL_PATH_REGEX);
