 ...
 */
final class TcpServer extends \RectorPrefix20220501\Evenement\EventEmitter implements \RectorPrefix20220501\React\Socket\ServerInterface
{
    private $master;
    private $loop;
     ...
     */
    public function __construct($uri, \RectorPrefix20220501\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        ...
        if (\false === $this->master) {
            if ($errno === 0) {
                ...
                $errno = \RectorPrefix20220501\React\Socket\SocketServer::errno($errstr);
            }
            throw new \RuntimeException('Failed to listen on "' . $uri . '": ' . $errstr . \RectorPrefix20220501\React\Socket\SocketServer::errconst($errno), $errno);
        }
    ...
    public function resume()
    {
        ...
        $this->loop->addReadStream($this->master, function ($master) use($that) {
            try {
                $newSocket = \RectorPrefix20220501\React\Socket\SocketServer::accept($master);
    ...
    {
        $this->emit('connection', array(new \RectorPrefix20220501\React\Socket\Connection($socket, $this->loop)));
    }
}
