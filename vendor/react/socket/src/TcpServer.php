 ...
 */
final class TcpServer extends \RectorPrefix20211203\Evenement\EventEmitter implements \RectorPrefix20211203\React\Socket\ServerInterface
{
    private $master;
    private $loop;
     ...
     */
    public function __construct($uri, \RectorPrefix20211203\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        ...
        if (\false === $this->master) {
            if ($errno === 0) {
                ...
                $errno = \RectorPrefix20211203\React\Socket\SocketServer::errno($errstr);
            }
            throw new \RuntimeException('Failed to listen on "' . $uri . '": ' . $errstr . \RectorPrefix20211203\React\Socket\SocketServer::errconst($errno), $errno);
        }
    ...
    {
        $this->emit('connection', array(new \RectorPrefix20211203\React\Socket\Connection($socket, $this->loop)));
    }
}
