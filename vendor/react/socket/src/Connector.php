...
use RectorPrefix20211129\React\Dns\Config\Config as DnsConfig;
use RectorPrefix20211129\React\Dns\Resolver\Factory as DnsFactory;
 ...
 */
final class Connector implements \RectorPrefix20211129\React\Socket\ConnectorInterface
{
    ...
    public function __construct($context = array(), $loop = null)
    {
        ...
        if (($context instanceof \RectorPrefix20211129\React\EventLoop\LoopInterface || $context === null) && (\func_num_args() <= 1 || \is_array($loop))) {
            $swap = $loop === null ? array() : $loop;
            $loop = $context;
            $context = $swap;
        }
        if (!\is_array($context) || $loop !== null && !$loop instanceof \RectorPrefix20211129\React\EventLoop\LoopInterface) {
        ...
        if ($context['tcp'] instanceof \RectorPrefix20211129\React\Socket\ConnectorInterface) {
            $tcp = $context['tcp'];
        } else {
            $tcp = new \RectorPrefix20211129\React\Socket\TcpConnector($loop, \is_array($context['tcp']) ? $context['tcp'] : array());
        }
        if ($context['dns'] !== \false) {
            if ($context['dns'] instanceof \RectorPrefix20211129\React\Dns\Resolver\ResolverInterface) {
                ...
                if ($context['dns'] !== \true) {
                    ...
                    $config = \RectorPrefix20211129\React\Dns\Config\Config::loadSystemConfigBlocking();
                    if (!$config->nameservers) {
                        $config->nameservers[] = '8.8.8.8';
                        // @codeCoverageIgnore
                    }
                }
                $factory = new \RectorPrefix20211129\React\Dns\Resolver\Factory();
                $resolver = $factory->createCached($config, $loop);
            }
            if ($context['happy_eyeballs'] === \true) {
                $tcp = new \RectorPrefix20211129\React\Socket\HappyEyeBallsConnector($loop, $tcp, $resolver);
            } else {
                $tcp = new \RectorPrefix20211129\React\Socket\DnsConnector($tcp, $resolver);
            }
        }
        if ($context['tcp'] !== \false) {
            $context['tcp'] = $tcp;
            if ($context['timeout'] !== \false) {
                $context['tcp'] = new \RectorPrefix20211129\React\Socket\TimeoutConnector($context['tcp'], $context['timeout'], $loop);
            }
            $this->connectors['tcp'] = $context['tcp'];
        }
        if ($context['tls'] !== \false) {
            if (!$context['tls'] instanceof \RectorPrefix20211129\React\Socket\ConnectorInterface) {
                $context['tls'] = new \RectorPrefix20211129\React\Socket\SecureConnector($tcp, $loop, \is_array($context['tls']) ? $context['tls'] : array());
            }
            if ($context['timeout'] !== \false) {
                $context['tls'] = new \RectorPrefix20211129\React\Socket\TimeoutConnector($context['tls'], $context['timeout'], $loop);
            }
            $this->connectors['tls'] = $context['tls'];
        }
        if ($context['unix'] !== \false) {
            if (!$context['unix'] instanceof \RectorPrefix20211129\React\Socket\ConnectorInterface) {
                $context['unix'] = new \RectorPrefix20211129\React\Socket\UnixConnector($loop);
            }
