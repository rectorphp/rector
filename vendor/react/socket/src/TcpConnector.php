...
final class TcpConnector implements \RectorPrefix20220423\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    public function __construct(\RectorPrefix20220423\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
    ...
    public function connect($uri)
    {
        ...
        if (@\inet_pton($ip) === \false) {
            return \RectorPrefix20220423\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $uri . '" does not contain a valid host IP (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : 22));
        }
        ...
        if (\false === $stream) {
            return \RectorPrefix20220423\React\Promise\reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \RectorPrefix20220423\React\Socket\SocketServer::errconst($errno), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new \RectorPrefix20220423\React\Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    // @codeCoverageIgnoreEnd
                    \fclose($stream);
                    $reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \RectorPrefix20220423\React\Socket\SocketServer::errconst($errno), $errno));
                } else {
                    $resolve(new \RectorPrefix20220423\React\Socket\Connection($stream, $loop));
                }
            });
