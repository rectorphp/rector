...
final class DnsConnector implements \RectorPrefix20211215\React\Socket\ConnectorInterface
{
    private $connector;
    private $resolver;
    public function __construct(\RectorPrefix20211215\React\Socket\ConnectorInterface $connector, \RectorPrefix20211215\React\Dns\Resolver\ResolverInterface $resolver)
    {
    ...
    public function connect($uri)
    {
        ...
        if (!$parts || !isset($parts['host'])) {
            return \RectorPrefix20211215\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $original . '" is invalid (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : 22));
        }
        $host = \trim($parts['host'], '[]');
        ...
        $resolved = null;
        return new \RectorPrefix20211215\React\Promise\Promise(function ($resolve, $reject) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
            // resolve/reject with result of DNS lookup
            $promise->then(function ($ip) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
                $resolved = $ip;
                return $promise = $connector->connect(\RectorPrefix20211215\React\Socket\Connector::uri($parts, $host, $ip))->then(null, function (\Exception $e) use($uri) {
            ...
            if ($promise instanceof \RectorPrefix20211215\React\Promise\CancellablePromiseInterface) {
