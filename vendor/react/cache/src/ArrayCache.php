...
namespace RectorPrefix20211210\React\Cache;

use RectorPrefix20211210\React\Promise;
use RectorPrefix20211210\React\Promise\PromiseInterface;
class ArrayCache implements \RectorPrefix20211210\React\Cache\CacheInterface
{
    private $limit;
    private $data = array();
    ...
    public function get($key, $default = null)
    {
        ...
        if (!\array_key_exists($key, $this->data)) {
            return \RectorPrefix20211210\React\Promise\resolve($default);
        }
        // remove and append to end of array to keep track of LRU info
        $value = $this->data[$key];
        unset($this->data[$key]);
        $this->data[$key] = $value;
        return \RectorPrefix20211210\React\Promise\resolve($value);
    }
    ...
    public function has($key)
    {
        ...
        if (!\array_key_exists($key, $this->data)) {
            return \RectorPrefix20211210\React\Promise\resolve(\false);
        }
        // remove and append to end of array to keep track of LRU info
        $value = $this->data[$key];
        unset($this->data[$key]);
