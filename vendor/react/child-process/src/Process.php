...
namespace RectorPrefix20220517\React\ChildProcess;
...
use RectorPrefix20220517\React\Stream\ReadableResourceStream;
use RectorPrefix20220517\React\Stream\ReadableStreamInterface;
 ...
 */
class Process extends \RectorPrefix20220517\Evenement\EventEmitter
{
    /**
     ...
     */
    public function start(\RectorPrefix20220517\React\EventLoop\LoopInterface $loop = null, $interval = 0.1)
    {
        if ($this->isRunning()) {
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \RectorPrefix20220517\React\Stream\DuplexResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            } elseif ($mode === 'w') {
                $stream = new \RectorPrefix20220517\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \RectorPrefix20220517\React\Stream\ReadableResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            }
            $this->pipes[$n] = $stream;
