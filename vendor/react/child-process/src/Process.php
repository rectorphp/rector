...
namespace RectorPrefix20211126\React\ChildProcess;
...
use RectorPrefix20211126\React\Stream\ReadableResourceStream;
use RectorPrefix20211126\React\Stream\ReadableStreamInterface;
 ...
 */
class Process extends \RectorPrefix20211126\Evenement\EventEmitter
{
    /**
    ...
    public function start($loop = null, $interval = 0.1)
    {
        ...
        $loop = $loop ?: \RectorPrefix20211126\React\EventLoop\Loop::get();
        $cmd = $this->cmd;
        $fdSpec = $this->fds;
        $sigchild = null;
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \RectorPrefix20211126\React\Stream\DuplexResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            } elseif ($mode === 'w') {
                $stream = new \RectorPrefix20211126\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \RectorPrefix20211126\React\Stream\ReadableResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            }
            $this->pipes[$n] = $stream;
