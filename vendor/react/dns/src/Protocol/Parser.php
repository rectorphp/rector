...
namespace RectorPrefix20211226\React\Dns\Protocol;

use RectorPrefix20211226\React\Dns\Model\Message;
use RectorPrefix20211226\React\Dns\Model\Record;
...
final class Parser
{
    ...
    private function parse($data, $consumed)
    {
        ...
        $message = new \RectorPrefix20211226\React\Dns\Model\Message();
        $message->id = $id;
    ...
    private function parseQuestion($data, $consumed)
    {
        ...
        $consumed += 4;
        return array(new \RectorPrefix20211226\React\Dns\Query\Query(\implode('.', $labels), $type, $class), $consumed);
    }
    /**
     * @param string $data
     * @param int $consumed
    ...
    private function parseRecord($data, $consumed)
    {
        ...
        if (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_A === $type) {
            if ($rdLength === 4) {
                $rdata = \inet_ntop(\substr($data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_AAAA === $type) {
            if ($rdLength === 16) {
                $rdata = \inet_ntop(\substr($data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_CNAME === $type || \RectorPrefix20211226\React\Dns\Model\Message::TYPE_PTR === $type || \RectorPrefix20211226\React\Dns\Model\Message::TYPE_NS === $type) {
            list($rdata, $consumed) = $this->readDomain($data, $consumed);
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_TXT === $type || \RectorPrefix20211226\React\Dns\Model\Message::TYPE_SPF === $type) {
            $rdata = array();
        ...
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_MX === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_SRV === $type) {
            if ($rdLength > 6) {
        ...
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_SSHFP === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_SOA === $type) {
        ...
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_OPT === $type) {
            $rdata = array();
            while (isset($data[$consumed + 4 - 1])) {
                ...
                if ($code === \RectorPrefix20211226\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value === '') {
                    $value = null;
                } elseif ($code === \RectorPrefix20211226\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $length === 2) {
                    list($value) = \array_values(\unpack('n', $value));
                    $value = \round($value * 0.1, 1);
                } elseif ($code === \RectorPrefix20211226\React\Dns\Model\Message::OPT_TCP_KEEPALIVE) {
                    break;
                }
                $rdata[$code] = $value;
                $consumed += 4 + $length;
            }
        } elseif (\RectorPrefix20211226\React\Dns\Model\Message::TYPE_CAA === $type) {
            if ($rdLength > 3) {
        ...
        return array(new \RectorPrefix20211226\React\Dns\Model\Record($name, $type, $class, $ttl, $rdata), $consumed);
    }
