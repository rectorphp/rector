...
namespace RectorPrefix20220422\React\Dns\Query;

use RectorPrefix20220422\React\Dns\Model\Message;
use RectorPrefix20220422\React\Dns\Protocol\BinaryDumper;
use RectorPrefix20220422\React\Dns\Protocol\Parser;
use RectorPrefix20220422\React\EventLoop\Loop;
use RectorPrefix20220422\React\EventLoop\LoopInterface;
use RectorPrefix20220422\React\Promise\Deferred;
/**
 ...
 */
final class UdpTransportExecutor implements \RectorPrefix20220422\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
     ...
     */
    public function __construct($nameserver, \RectorPrefix20220422\React\EventLoop\LoopInterface $loop = null)
    {
        ...
        $this->parser = new \RectorPrefix20220422\React\Dns\Protocol\Parser();
        $this->dumper = new \RectorPrefix20220422\React\Dns\Protocol\BinaryDumper();
    }
    public function query(\RectorPrefix20220422\React\Dns\Query\Query $query)
    {
        $request = \RectorPrefix20220422\React\Dns\Model\Message::createRequestForQuery($query);
        $queryData = $this->dumper->toBinary($request);
        if (isset($queryData[$this->maxPacketSize])) {
            return \RectorPrefix20220422\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \RectorPrefix20220422\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            ...
            return \RectorPrefix20220422\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . (isset($m[2]) ? $m[2] : $error['message']) . ')', isset($m[1]) ? (int) $m[1] : 0));
        }
        $loop = $this->loop;
        $deferred = new \RectorPrefix20220422\React\Promise\Deferred(function () use($loop, $socket, $query) {
            // cancellation should remove socket from loop and close socket
            $loop->removeReadStream($socket);
            \fclose($socket);
            throw new \RectorPrefix20220422\React\Dns\Query\CancellationException('DNS query for ' . $query->describe() . ' has been cancelled');
        });
