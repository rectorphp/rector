...
use RectorPrefix20211201\React\Promise\Promise;
final class CachingExecutor implements \RectorPrefix20211201\React\Dns\Query\ExecutorInterface
{
    /**
    ...
    private $executor;
    private $cache;
    public function __construct(\RectorPrefix20211201\React\Dns\Query\ExecutorInterface $executor, \RectorPrefix20211201\React\Cache\CacheInterface $cache)
    {
    ...
    public function query($query)
    {
        ...
        return new \RectorPrefix20211201\React\Promise\Promise(function ($resolve, $reject) use($query, $id, $cache, $executor, &$pending, $that) {
            $pending->then(function ($message) use($query, $id, $cache, $executor, &$pending, $that) {
                ...
                return $pending = $executor->query($query)->then(function (\RectorPrefix20211201\React\Dns\Model\Message $message) use($cache, $id, $that) {
