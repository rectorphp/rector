...
final class CachingExecutor implements \RectorPrefix20220430\React\Dns\Query\ExecutorInterface
{
    /**
    ...
    private $executor;
    private $cache;
    public function __construct(\RectorPrefix20220430\React\Dns\Query\ExecutorInterface $executor, \RectorPrefix20220430\React\Cache\CacheInterface $cache)
    {
    ...
    public function query(\RectorPrefix20220430\React\Dns\Query\Query $query)
    {
        ...
        return new \RectorPrefix20220430\React\Promise\Promise(function ($resolve, $reject) use($query, $id, $cache, $executor, &$pending, $that) {
            $pending->then(function ($message) use($query, $id, $cache, $executor, &$pending, $that) {
                ...
                return $pending = $executor->query($query)->then(function (\RectorPrefix20220430\React\Dns\Model\Message $message) use($cache, $id, $that) {
     ...
     * @internal
     */
    public function ttl(\RectorPrefix20220430\React\Dns\Model\Message $message)
    {
