 ...
 */
final class HostsFileExecutor implements \RectorPrefix20211123\React\Dns\Query\ExecutorInterface
{
    private $hosts;
    private $fallback;
    public function __construct(\RectorPrefix20211123\React\Dns\Config\HostsFile $hosts, \RectorPrefix20211123\React\Dns\Query\ExecutorInterface $fallback)
    {
        $this->hosts = $hosts;
    ...
    {
        if ($query->class === \RectorPrefix20211123\React\Dns\Model\Message::CLASS_IN && ($query->type === \RectorPrefix20211123\React\Dns\Model\Message::TYPE_A || $query->type === \RectorPrefix20211123\React\Dns\Model\Message::TYPE_AAAA)) {
            // forward lookup for type A or AAAA
            $records = array();
            $expectsColon = $query->type === \RectorPrefix20211123\React\Dns\Model\Message::TYPE_AAAA;
            foreach ($this->hosts->getIpsForHost($query->name) as $ip) {
                ...
                if ((\strpos($ip, ':') !== \false) === $expectsColon) {
                    $records[] = new \RectorPrefix20211123\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $ip);
                }
            }
            if ($records) {
                return \RectorPrefix20211123\React\Promise\resolve(\RectorPrefix20211123\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
            }
        } elseif ($query->class === \RectorPrefix20211123\React\Dns\Model\Message::CLASS_IN && $query->type === \RectorPrefix20211123\React\Dns\Model\Message::TYPE_PTR) {
            ...
            if ($ip !== null) {
                ...
                foreach ($this->hosts->getHostsForIp($ip) as $host) {
                    $records[] = new \RectorPrefix20211123\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $host);
                }
                if ($records) {
                    return \RectorPrefix20211123\React\Promise\resolve(\RectorPrefix20211123\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
                }
