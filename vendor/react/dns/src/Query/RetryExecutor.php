...
use RectorPrefix20220610\React\Promise\CancellablePromiseInterface;
use RectorPrefix20220610\React\Promise\Deferred;
use RectorPrefix20220610\React\Promise\PromiseInterface;
final class RetryExecutor implements ExecutorInterface
{
    ...
    public function tryQuery(Query $query, $retries)
    {
        $deferred = new Deferred(function () use(&$promise) {
            if ($promise instanceof CancellablePromiseInterface || !\interface_exists('RectorPrefix20220610\\React\\Promise\\CancellablePromiseInterface') && \method_exists($promise, 'cancel')) {
                $promise->cancel();
            }
        });
