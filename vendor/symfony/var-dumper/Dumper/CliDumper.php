...
use RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \RectorPrefix20210514\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
    ...
    public function enterHash(\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild)
    {
        ...
        if (\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
            $prefix = $this->style('note', $class . ' resource', $attr) . ($hasChild ? ' {' : ' ');
        } else {
            $prefix = $class && !(self::DUMP_LIGHT_ARRAY & $this->flags) ? $this->style('note', 'array:' . $class) . ' [' : '[';
        }
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
    ...
    public function leaveHash(\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild, int $cut)
    {
        if (empty($cursor->attr['cut_hash'])) {
            ...
            $this->line .= \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
     ...
     */
    protected function dumpEllipsis(\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $hasChild, $cut)
    {
        if ($cut) {
            $this->line .= ' …';
            if (0 < $cut) {
     ...
     */
    protected function dumpKey(\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\0~\0" . $key;
                // no break
                case \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue(\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (-1 === $cursor->hashType) {
            return;
        }
        if (\RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \RectorPrefix20210514\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {
