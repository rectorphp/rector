...
final class CompleteCommand extends \RectorPrefix20220604\Symfony\Component\Console\Command\Command
{
    ...
    {
        $this->addOption('shell', 's', \RectorPrefix20220604\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The shell type ("' . \implode('", "', \array_keys($this->completionOutputs)) . '")')->addOption('input', 'i', \RectorPrefix20220604\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED | \RectorPrefix20220604\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY, 'An array of input tokens (e.g. COMP_WORDS or argv)')->addOption('current', 'c', \RectorPrefix20220604\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The index of the "input" array that the cursor is in (e.g. COMP_CWORD)')->addOption('symfony', 'S', \RectorPrefix20220604\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The version of the completion script');
    }
    ...
    protected function execute(\RectorPrefix20220604\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20220604\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        try {
            ...
            $suggestions = new \RectorPrefix20220604\Symfony\Component\Console\Completion\CompletionSuggestions();
            ...
            if (null === $command) {
                ...
                if (\RectorPrefix20220604\Symfony\Component\Console\Completion\CompletionInput::TYPE_OPTION_NAME === $completionInput->getCompletionType()) {
                    $this->log('  Completing option names for the <comment>' . \get_class($command instanceof \RectorPrefix20220604\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> command.');
                    $suggestions->suggestOptions($command->getDefinition()->getOptions());
                } else {
                    $this->log(['  Completing using the <comment>' . \get_class($command instanceof \RectorPrefix20220604\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> class.', '  Completing <comment>' . $completionInput->getCompletionType() . '</> for <comment>' . $completionInput->getCompletionName() . '</>']);
    ...
    private function createCompletionInput(\RectorPrefix20220604\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20220604\Symfony\Component\Console\Completion\CompletionInput
    {
    ...
    private function findCommand(\RectorPrefix20220604\Symfony\Component\Console\Completion\CompletionInput $completionInput, \RectorPrefix20220604\Symfony\Component\Console\Output\OutputInterface $output) : ?\RectorPrefix20220604\Symfony\Component\Console\Command\Command
    {
        try {
