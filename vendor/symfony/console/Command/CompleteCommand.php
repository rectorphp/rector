...
use RectorPrefix20220607\Symfony\Component\Console\Completion\Output\BashCompletionOutput;
use RectorPrefix20220607\Symfony\Component\Console\Completion\Output\CompletionOutputInterface;
use RectorPrefix20220607\Symfony\Component\Console\Completion\Output\FishCompletionOutput;
...
final class CompleteCommand extends \RectorPrefix20220607\Symfony\Component\Console\Command\Command
{
    /**
    ...
    protected function execute(\RectorPrefix20220607\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20220607\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        try {
            ...
            $suggestions = new \RectorPrefix20220607\Symfony\Component\Console\Completion\CompletionSuggestions();
            ...
            if (null === $command) {
                ...
                if (\RectorPrefix20220607\Symfony\Component\Console\Completion\CompletionInput::TYPE_OPTION_NAME === $completionInput->getCompletionType()) {
                    $this->log('  Completing option names for the <comment>' . \get_class($command instanceof \RectorPrefix20220607\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> command.');
                    $suggestions->suggestOptions($command->getDefinition()->getOptions());
                } else {
                    $this->log(['  Completing using the <comment>' . \get_class($command instanceof \RectorPrefix20220607\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> class.', '  Completing <comment>' . $completionInput->getCompletionType() . '</> for <comment>' . $completionInput->getCompletionName() . '</>']);
    ...
    private function createCompletionInput(\RectorPrefix20220607\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20220607\Symfony\Component\Console\Completion\CompletionInput
    {
        $currentIndex = $input->getOption('current');
        if (!$currentIndex || !\ctype_digit($currentIndex)) {
            throw new \RuntimeException('The "--current" option must be set and it must be an integer.');
        }
        $completionInput = \RectorPrefix20220607\Symfony\Component\Console\Completion\CompletionInput::fromTokens($input->getOption('input'), (int) $currentIndex);
        try {
    ...
    private function findCommand(\RectorPrefix20220607\Symfony\Component\Console\Completion\CompletionInput $completionInput, \RectorPrefix20220607\Symfony\Component\Console\Output\OutputInterface $output) : ?\RectorPrefix20220607\Symfony\Component\Console\Command\Command
    {
        try {
        ...
        } catch (\RectorPrefix20220607\Symfony\Component\Console\Exception\CommandNotFoundException $exception) {
        }
        return null;
    }
