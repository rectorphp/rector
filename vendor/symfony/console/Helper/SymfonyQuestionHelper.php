    ...
    protected function writePrompt(\RectorPrefix20220101\Symfony\Component\Console\Output\OutputInterface $output, \RectorPrefix20220101\Symfony\Component\Console\Question\Question $question)
    {
        ...
        switch (\true) {
            ...
            case $question instanceof \RectorPrefix20220101\Symfony\Component\Console\Question\ChoiceQuestion && $question->isMultiselect():
                ...
                $text = \sprintf(' <info>%s</info> [<comment>%s</comment>]:', $text, \RectorPrefix20220101\Symfony\Component\Console\Formatter\OutputFormatter::escape(\implode(', ', $default)));
                break;
            case $question instanceof \RectorPrefix20220101\Symfony\Component\Console\Question\ChoiceQuestion:
                $choices = $question->getChoices();
                $text = \sprintf(' <info>%s</info> [<comment>%s</comment>]:', $text, \RectorPrefix20220101\Symfony\Component\Console\Formatter\OutputFormatter::escape($choices[$default] ?? $default));
                break;
            default:
                $text = \sprintf(' <info>%s</info> [<comment>%s</comment>]:', $text, \RectorPrefix20220101\Symfony\Component\Console\Formatter\OutputFormatter::escape($default));
        }
        $output->writeln($text);
        $prompt = ' > ';
