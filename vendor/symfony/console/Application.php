...
class Application implements \RectorPrefix20220102\Symfony\Contracts\Service\ResetInterface
{
    ...
    public function renderThrowable(\Throwable $e, \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
        ...
        if (null !== $this->runningCommand) {
            $output->writeln(\sprintf('<info>%s</info>', \RectorPrefix20220102\Symfony\Component\Console\Formatter\OutputFormatter::escape(\sprintf($this->runningCommand->getSynopsis(), $this->getName()))), \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
            $output->writeln('', \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
    }
    protected function doRenderThrowable(\Throwable $e, \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
        do {
            ...
            if (!$e instanceof \RectorPrefix20220102\Symfony\Component\Console\Exception\ExceptionInterface || \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $messages[] = \sprintf('<comment>%s</comment>', \RectorPrefix20220102\Symfony\Component\Console\Formatter\OutputFormatter::escape(\sprintf('In %s line %s:', \basename($e->getFile()) ?: 'n/a', $e->getLine() ?: 'n/a')));
            }
            $messages[] = $emptyLine = \sprintf('<error>%s</error>', \str_repeat(' ', $len));
            if ('' === $message || \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $messages[] = \sprintf('<error>%s%s</error>', $title, \str_repeat(' ', \max(0, $len - \RectorPrefix20220102\Symfony\Component\Console\Helper\Helper::width($title))));
            }
            foreach ($lines as $line) {
                $messages[] = \sprintf('<error>  %s  %s</error>', \RectorPrefix20220102\Symfony\Component\Console\Formatter\OutputFormatter::escape($line[0]), \str_repeat(' ', $len - $line[1]));
            }
            $messages[] = $emptyLine;
            $messages[] = '';
            ...
            if (\RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $output->writeln('<comment>Exception trace:</comment>', \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
                ...
                for ($i = 0, $count = \count($trace); $i < $count; ++$i) {
                    ...
                    $output->writeln(\sprintf(' %s%s at <info>%s:%s</info>', $class, $function ? $type . $function . '()' : '', $file, $line), \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
                }
                $output->writeln('', \RectorPrefix20220102\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
            }
