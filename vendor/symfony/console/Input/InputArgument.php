...
class InputArgument
{
    ...
    public function __construct(string $name, int $mode = null, string $description = '', $default = null, $suggestedValues = [])
    {
        if (null === $mode) {
            ...
            throw new \RectorPrefix20220605\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Argument mode "%s" is not valid.', $mode));
        }
        $this->name = $name;
        $this->mode = $mode;
    ...
    public function setDefault($default = null)
    {
        if ($this->isRequired() && null !== $default) {
            throw new \RectorPrefix20220605\Symfony\Component\Console\Exception\LogicException('Cannot set a default value except for InputArgument::OPTIONAL mode.');
        }
        if ($this->isArray()) {
            if (null === $default) {
                $default = [];
            } elseif (!\is_array($default)) {
                throw new \RectorPrefix20220605\Symfony\Component\Console\Exception\LogicException('A default value for an array argument must be an array.');
            }
        }
    ...
    public function complete(\RectorPrefix20220605\Symfony\Component\Console\Completion\CompletionInput $input, \RectorPrefix20220605\Symfony\Component\Console\Completion\CompletionSuggestions $suggestions) : void
    {
        ...
        if ($values instanceof \Closure && !\is_array($values = $values($input))) {
            throw new \RectorPrefix20220605\Symfony\Component\Console\Exception\LogicException(\sprintf('Closure for argument "%s" must return an array. Got "%s".', $this->name, \get_debug_type($values)));
        }
        if ($values) {
