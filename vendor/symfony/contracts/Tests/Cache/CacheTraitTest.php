 ...
 */
namespace RectorPrefix20211201\Symfony\Contracts\Tests\Cache;
...
use RectorPrefix20211201\Symfony\Contracts\Cache\CacheTrait;
/**
 * @author Tobias Nyholm <tobias.nyholm@gmail.com>
 */
class CacheTraitTest extends \RectorPrefix20211201\PHPUnit\Framework\TestCase
{
    public function testSave()
    {
        $item = $this->createMock(\RectorPrefix20211201\Psr\Cache\CacheItemInterface::class);
        $item->method('set')->willReturn($item);
        $item->method('isHit')->willReturn(\false);
        $item->expects($this->once())->method('set')->with('computed data');
        $cache = $this->getMockBuilder(\RectorPrefix20211201\Symfony\Contracts\Tests\Cache\TestPool::class)->setMethods(['getItem', 'save'])->getMock();
        $cache->expects($this->once())->method('getItem')->with('key')->willReturn($item);
        $cache->expects($this->once())->method('save');
        $callback = function (\RectorPrefix20211201\Psr\Cache\CacheItemInterface $item) {
...
class TestPool implements \RectorPrefix20211201\Psr\Cache\CacheItemPoolInterface
{
    use CacheTrait;
