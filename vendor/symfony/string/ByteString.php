 ...
 */
class ByteString extends \RectorPrefix20211027\Symfony\Component\String\AbstractString
{
    ...
    public static function fromRandom($length = 16, $alphabet = null) : self
    {
        ...
        if ($bits <= 0 || $bits > 56) {
            throw new \RectorPrefix20211027\Symfony\Component\String\Exception\InvalidArgumentException('The length of the alphabet must in the [2^1, 2^56] range.');
        }
        $ret = '';
        while ($length > 0) {
            ...
            if (\false === $match($regexp, $this->string, $matches, $flags, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \substr($k, -6)) {
                        throw new \RectorPrefix20211027\Symfony\Component\String\Exception\RuntimeException('Matching failed with ' . $k . '.');
                    }
                }
                throw new \RectorPrefix20211027\Symfony\Component\String\Exception\RuntimeException('Matching failed with unknown error code.');
            }
    ...
    public function snake() : parent
    {
        ...
        $str->string = \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z\\d])([A-Z])/'], 'RectorPrefix20211027\\1_\\2', $str->string));
        return $str;
    }
    /**
    ...
    public function toCodePointString($fromEncoding = null) : \RectorPrefix20211027\Symfony\Component\String\CodePointString
    {
        ...
        if (!$validEncoding) {
            throw new \RectorPrefix20211027\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Invalid "%s" string.', $fromEncoding ?? 'Windows-1252'));
        }
