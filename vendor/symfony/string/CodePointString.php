 ...
 */
namespace RectorPrefix20220506\Symfony\Component\String;

use RectorPrefix20220506\Symfony\Component\String\Exception\ExceptionInterface;
use RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException;
/**
 ...
 */
class CodePointString extends \RectorPrefix20220506\Symfony\Component\String\AbstractUnicodeString
{
    public function __construct(string $string = '')
    {
        if ('' !== $string && !\preg_match('//u', $string)) {
            throw new \RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
        $this->string = $string;
    }
    /**
    ...
    {
        if (1 > $length) {
            throw new \RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException('The chunk length must be greater than zero.');
        }
        if ('' === $this->string) {
            return [];
        }
    ...
    {
        if ($suffix instanceof \RectorPrefix20220506\Symfony\Component\String\AbstractString) {
    ...
    {
        if ($string instanceof \RectorPrefix20220506\Symfony\Component\String\AbstractString) {
    ...
    {
        if ($needle instanceof \RectorPrefix20220506\Symfony\Component\String\AbstractString) {
    ...
    public function split(string $delimiter, int $limit = null, int $flags = null) : array
    {
        if (1 > ($limit = $limit ?? \PHP_INT_MAX)) {
            throw new \RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException('Split limit must be a positive integer.');
        }
        if ('' === $delimiter) {
            throw new \RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException('Split delimiter is empty.');
        }
        if (null !== $flags) {
            return parent::split($delimiter . 'u', $limit, $flags);
        }
        if (!\preg_match('//u', $delimiter)) {
            throw new \RectorPrefix20220506\Symfony\Component\String\Exception\InvalidArgumentException('Split delimiter is not a valid UTF-8 string.');
        }
        $str = clone $this;
    ...
    {
        if ($prefix instanceof \RectorPrefix20220506\Symfony\Component\String\AbstractString) {
