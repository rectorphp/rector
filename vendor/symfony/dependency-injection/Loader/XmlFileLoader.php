...
use RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils;
...
use RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\BoundArgument;
 ...
 */
class XmlFileLoader extends \RectorPrefix20220604\Symfony\Component\DependencyInjection\Loader\FileLoader
{
    ...
    private function parseImports(\DOMDocument $xml, string $file, \DOMNode $root = null)
    {
        ...
        foreach ($imports as $import) {
            ...
            $this->import($import->getAttribute('resource'), \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('type')) ?: null, \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('ignore-errors')) ?: \false, $file);
        }
    }
    private function parseDefinitions(\DOMDocument $xml, string $file, \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition $defaults, \DOMNode $root = null)
    {
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        ...
        foreach ($instanceof as $service) {
            $this->setDefinition((string) $service->getAttribute('id'), $this->parseDefinition($service, $file, new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition()));
        }
            ...
            if ('stack' === $service->tagName) {
                ...
                foreach ($this->getChildren($service, 'service') as $k => $frame) {
                    ...
                    if ($alias = $frame->getAttribute('alias')) {
                        ...
                        $stack[$k] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference($alias);
                    } else {
                ...
                if ('prototype' === $service->tagName) {
                    ...
                    if ($service->hasAttribute('exclude')) {
                        if (\count($excludes) > 0) {
                            throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('You cannot use both the attribute "exclude" and <exclude> tags at the same time.');
                        }
    ...
    private function getServiceDefaults(\DOMDocument $xml, string $file, \DOMNode $root = null) : \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition
    {
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        if (null === ($defaultsNode = $xpath->query('.//container:services/container:defaults', $root)->item(0))) {
            return new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition();
        }
        $defaultsNode->setAttribute('id', '<defaults>');
        return $this->parseDefinition($defaultsNode, $file, new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition());
    }
    /**
     * Parses an individual Definition.
     */
    private function parseDefinition(\DOMElement $service, string $file, \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition $defaults) : ?\RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition
    {
        if ($alias = $service->getAttribute('alias')) {
            $this->validateAlias($service, $file);
            $this->container->setAlias($service->getAttribute('id'), $alias = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Alias($alias));
            if ($publicAttr = $service->getAttribute('public')) {
                $alias->setPublic(\RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($publicAttr));
            ...
            if ($deprecated = $this->getChildren($service, 'deprecated')) {
                ...
                if (!$deprecated[0]->hasAttribute('package')) {
                    throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Missing attribute "package" at node "deprecated" in "%s".', $file));
                }
                if (!$deprecated[0]->hasAttribute('version')) {
                    throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Missing attribute "version" at node "deprecated" in "%s".', $file));
                }
        ...
        if ($this->isLoadingInstanceof) {
            $definition = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\ChildDefinition('');
        } elseif ($parent = $service->getAttribute('parent')) {
            $definition = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\ChildDefinition($parent);
        } else {
            $definition = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition();
        }
        ...
        foreach (['class', 'public', 'shared', 'synthetic', 'abstract'] as $key) {
            if ($value = $service->getAttribute($key)) {
                $method = 'set' . $key;
                $definition->{$method}($value = \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($value));
            }
        }
        if ($value = $service->getAttribute('lazy')) {
            $definition->setLazy((bool) ($value = \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($value)));
            if (\is_string($value)) {
                $definition->addTag('proxy', ['interface' => $value]);
            }
        }
        if ($value = $service->getAttribute('autowire')) {
            $definition->setAutowired(\RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($value));
        }
        if ($value = $service->getAttribute('autoconfigure')) {
            $definition->setAutoconfigured(\RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($value));
        }
        ...
        $definition->setArguments($this->getArgumentsAsPhp($service, 'argument', $file, $definition instanceof \RectorPrefix20220604\Symfony\Component\DependencyInjection\ChildDefinition));
        ...
        foreach ($this->getChildren($service, 'call') as $call) {
            $definition->addMethodCall($call->getAttribute('method'), $this->getArgumentsAsPhp($call, 'argument', $file), \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($call->getAttribute('returns-clone')));
        }
        ...
        foreach ($tags as $tag) {
            ...
            foreach ($tag->attributes as $name => $node) {
                ...
                if (\strpos($name, '-') !== \false && \strpos($name, '_') === \false && !\array_key_exists($normalizedName = \str_replace('-', '_', $name), $parameters)) {
                    $parameters[$normalizedName] = \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
                }
                // keep not normalized key
                $parameters[$name] = \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
            }
            if ('' === $tagName && '' === ($tagName = $tag->getAttribute('name'))) {
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The tag name for service "%s" in "%s" must be a non-empty string.', $service->getAttribute('id'), $file));
            }
        ...
        $bindingType = $this->isLoadingInstanceof ? \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\BoundArgument::INSTANCEOF_BINDING : \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\BoundArgument::SERVICE_BINDING;
        foreach ($bindings as $argument => $value) {
            $bindings[$argument] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\BoundArgument($value, \true, $bindingType, $file);
        }
        ...
        if ($decorates = $service->getAttribute('decorates')) {
            ...
            if ('exception' === $decorationOnInvalid) {
                $invalidBehavior = \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE;
            } elseif ('ignore' === $decorationOnInvalid) {
                $invalidBehavior = \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE;
                ...
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value "%s" for attribute "decoration-on-invalid" on service "%s". Did you mean "exception", "ignore" or "null" in "%s"?', $decorationOnInvalid, $service->getAttribute('id'), $file));
            }
    ...
    private function processAnonymousServices(\DOMDocument $xml, string $file, \DOMNode $root = null)
    {
        $definitions = [];
        $count = 0;
        $suffix = '~' . \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder::hash($file);
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        ...
        if (\false !== ($nodes = $xpath->query('.//container:services/container:service[not(@id)]', $root))) {
            foreach ($nodes as $node) {
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Top-level services must have "id" attribute, none found in "%s" at line %d.', $file, $node->getLineNo()));
            }
        }
        // resolve definitions
        \uksort($definitions, 'strnatcmp');
        foreach (\array_reverse($definitions) as $id => [$domElement, $file]) {
            if (null !== ($definition = $this->parseDefinition($domElement, $file, new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition()))) {
    ...
    private function getArgumentsAsPhp(\DOMElement $node, string $name, string $file, bool $isChildDefinition = \false) : array
    {
        ...
        foreach ($this->getChildren($node, $name) as $arg) {
            ...
            if ('ignore' == $onInvalid) {
                ...
                $invalidBehavior = \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE;
            ...
            switch ($arg->getAttribute('type')) {
                case 'service':
                    ...
                    $arguments[$key] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference($arg->getAttribute('id'), $invalidBehavior);
                    break;
                case 'expression':
                    ...
                    $arguments[$key] = new \RectorPrefix20220604\Symfony\Component\ExpressionLanguage\Expression($arg->nodeValue);
                    break;
                case 'collection':
                ...
                case 'tagged_locator':
                    ...
                    $arguments[$key] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument($arg->getAttribute('tag'), $arg->getAttribute('index-by') ?: null, $arg->getAttribute('default-index-method') ?: null, $forLocator, $arg->getAttribute('default-priority-method') ?: null);
                    if ($forLocator) {
                        $arguments[$key] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments[$key]);
                    }
                    break;
                case 'binary':
                    if (\false === ($value = \base64_decode($arg->nodeValue))) {
                        throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="binary" is not a valid base64 encoded string.', $name));
                    }
                    $arguments[$key] = $value;
                    break;
                case 'abstract':
                    $arguments[$key] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\AbstractArgument($arg->nodeValue);
                    break;
                case 'string':
                ...
                case 'constant':
                    ...
                    break;
                default:
                    $arguments[$key] = \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::phpize($arg->nodeValue);
            }
        }
        return $arguments;
    }
        ...
        if ($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) {
            ...
            for ($i = 0, $nb = \count($items); $i < $nb; $i += 2) {
                ...
                if (($extension = $this->container->getExtension($items[$i])) && \false !== $extension->getXsdValidationBasePath()) {
                    ...
                    if (!\is_file($path)) {
                        throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Extension "%s" references a non-existent XSD file "%s".', \get_debug_type($extension), $path));
                    }
...
<xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <xsd:complexType name="testType"/>
    ...
    private function validateAlias(\DOMElement $alias, string $file)
    {
        foreach ($alias->attributes as $name => $node) {
            if (!\in_array($name, ['alias', 'id', 'public'])) {
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid attribute "%s" defined for alias "%s" in "%s".', $name, $alias->getAttribute('id'), $file));
            }
        }
        foreach ($alias->childNodes as $child) {
            ...
            if (!\in_array($child->localName, ['deprecated'], \true)) {
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid child element "%s" defined for alias "%s" in "%s".', $child->localName, $alias->getAttribute('id'), $file));
            }
        }
    }
    /**
    ...
    private function validateExtensions(\DOMDocument $dom, string $file)
    {
        foreach ($dom->documentElement->childNodes as $node) {
            ...
            if (!$this->container->hasExtension($node->namespaceURI)) {
                $extensionNamespaces = \array_filter(\array_map(function (\RectorPrefix20220604\Symfony\Component\DependencyInjection\Extension\ExtensionInterface $ext) {
                    return $ext->getNamespace();
                }, $this->container->getExtensions()));
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('There is no extension able to load the configuration for "%s" (in "%s"). Looked for namespace "%s", found "%s".', $node->tagName, $file, $node->namespaceURI, $extensionNamespaces ? \implode('", "', $extensionNamespaces) : 'none'));
            }
        }
    }
    /**
    ...
    {
        return \RectorPrefix20220604\Symfony\Component\Config\Util\XmlUtils::convertDomElementToArray($element);
    }
}
