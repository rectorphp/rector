...
use RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\Container;
...
use RectorPrefix20210904\Symfony\Component\DependencyInjection\ExpressionLanguage;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface as ProxyDumper;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper;
...
use RectorPrefix20210904\Symfony\Component\DependencyInjection\ServiceLocator as BaseServiceLocator;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\TypedReference;
use RectorPrefix20210904\Symfony\Component\DependencyInjection\Variable;
...
use RectorPrefix20210904\Symfony\Component\HttpKernel\Kernel;
/**
...
class PhpDumper extends \RectorPrefix20210904\Symfony\Component\DependencyInjection\Dumper\Dumper
{
     ...
     * {@inheritdoc}
     */
    public function __construct(\RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        if (!$container->isCompiled()) {
            throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\LogicException('Cannot dump an uncompiled container.');
        }
    ...
    public function dump($options = [])
    {
        ...
        if (\strncmp($baseClass = $options['base_class'], '\\', \strlen('\\')) !== 0 && 'Container' !== $baseClass) {
            ...
            $this->baseClass = \RectorPrefix20210904\Symfony\Component\DependencyInjection\Container::class;
        } else {
            $this->baseClass = $baseClass;
        }
        $this->initializeMethodNamesMap('Container' === $baseClass ? \RectorPrefix20210904\Symfony\Component\DependencyInjection\Container::class : $baseClass);
        if ($this->getProxyDumper() instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper) {
            (new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\true, \false))->process($this->container);
            try {
                (new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass())->process($this->container);
                ...
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($e->getServiceId(), $path);
            }
        }
...
EOF;
            ...
            $hash = \ucfirst(\strtr(\RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerBuilder::hash($files), '._', 'xx'));
            $code = [];
...
EOF;
        ...
        if ($unusedEnvs) {
            throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\EnvParameterException($unusedEnvs, null, 'Environment variables "%s" are never used. Please, check your container\'s configuration.');
        }
        return $code;
    }
    /**
    ...
    {
        if (!$this->proxyDumper) {
            $this->proxyDumper = new \RectorPrefix20210904\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper();
        }
        return $this->proxyDumper;
    }
    private function analyzeReferences()
    {
        (new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\false, !$this->getProxyDumper() instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper))->process($this->container);
        $checkedNodes = [];
        $this->circularReferences = [];
    ...
    private function collectCircularReferences(string $sourceId, array $edges, array &$checkedNodes, array &$loops = [], array $path = [], bool $byConstructor = \true) : void
    {
        ...
        foreach ($edges as $edge) {
            ...
            $id = $node->getId();
            if ($sourceId === $id || !$node->getValue() instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition || $edge->isLazy() || $edge->isWeak()) {
                continue;
            }
            if (isset($path[$id])) {
                $loop = null;
    ...
    private function generateProxyClasses() : array
    {
        ...
        $strip = '' === $this->docStar && \method_exists(\RectorPrefix20210904\Symfony\Component\HttpKernel\Kernel::class, 'stripComments');
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = \substr(\RectorPrefix20210904\Symfony\Component\HttpKernel\Kernel::stripComments($proxyCode), 5);
            }
    ...
    private function addServiceInclude(string $cId, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition) : string
    {
        $code = '';
        if ($this->inlineRequires && (!$this->isHotPath($definition) || $this->getProxyDumper()->isProxyCandidate($definition))) {
            ...
            foreach ($this->serviceCalls as $id => [$callCount, $behavior]) {
                if ('service_container' !== $id && $id !== $cId && \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $behavior && $this->container->has($id) && $this->isTrivialInstance($def = $this->container->findDefinition($id))) {
                    foreach ($this->getClasses($def, $cId) as $class) {
                        $this->collectLineage($class, $lineage);
                    }
                }
     ...
     */
    private function addServiceInstance(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, bool $isSimpleInstance) : string
    {
        ...
        if (\strncmp($class, "'", \strlen("'")) === 0 && \strpos($class, '$') === \false && !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid class name for the "%s" service.', $class, $id));
        }
    ...
    private function isTrivialInstance(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        ...
        foreach ($definition->getArguments() as $arg) {
            ...
            if (\is_array($arg) && 3 >= \count($arg)) {
                foreach ($arg as $k => $v) {
                    ...
                    if ($v instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $v) && $this->container->findDefinition($id)->isSynthetic()) {
                        continue;
                    }
            ...
            } elseif ($arg instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $arg) && $this->container->findDefinition($id)->isSynthetic()) {
                continue;
    ...
    private function addServiceMethodCalls(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $variableName, ?string $sharedNonLazyId) : string
    {
        $lastWitherIndex = null;
        foreach ($definition->getMethodCalls() as $k => $call) {
            if ($call[2] ?? \false) {
    ...
    private function addServiceProperties(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        $code = '';
    ...
    private function addServiceConfigurator(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        if (!($callable = $definition->getConfigurator())) {
            return '';
        }
        if (\is_array($callable)) {
            if ($callable[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
    ...
    private function addService(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition) : array
    {
        $this->definitionVariables = new \SplObjectStorage();
        $this->referenceVariables = [];
        $this->variableCount = 0;
        $this->referenceVariables[$id] = new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Variable('instance');
        $return = [];
        if ($class = $definition->getClass()) {
            $class = $class instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
            $return[] = \sprintf(\strncmp($class, '%', \strlen('%')) === 0 ? '@return object A %1$s instance' : '@return \\%s', \ltrim($class, '\\'));
        } elseif ($definition->getFactory()) {
            $factory = $definition->getFactory();
            if (\is_string($factory)) {
                $return[] = \sprintf('@return object An instance returned by %s()', $factory);
            } elseif (\is_array($factory) && (\is_string($factory[0]) || $factory[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition || $factory[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference)) {
                $class = $factory[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition ? $factory[0]->getClass() : (string) $factory[0];
                $class = $class instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
...
EOF;
    ...
    private function addInlineVariables(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, array $arguments, bool $forConstructor) : string
    {
        $code = '';
    ...
    private function addInlineReference(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $targetId, bool $forConstructor) : string
    {
        ...
        $this->referenceVariables[$targetId] = new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Variable($name);
        $reference = \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $behavior ? new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference($targetId, $behavior) : null;
...
, $this->container->getDefinition($id)->isPublic() ? 'services' : 'privates', $this->doExport($id));
    ...
    private function addInlineService(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $inlineDef = null, bool $forConstructor = \true) : string
    {
        $code = '';
        ...
        if (isset($this->definitionVariables[$inlineDef])) {
            ...
            $this->definitionVariables[$inlineDef] = new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Variable($name);
            $code .= '' !== $code ? "\n" : '';
            if ('instance' === $name) {
    ...
    private function addNewInstance(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $return = '', string $id = null) : string
    {
        $tail = $return ? ";\n" : '';
        if (\RectorPrefix20210904\Symfony\Component\DependencyInjection\ServiceLocator::class === $definition->getClass() && $definition->hasTag($this->serviceLocatorTag)) {
            $arguments = [];
            foreach ($definition->getArgument(0) as $k => $argument) {
                $arguments[$k] = $argument->getValues()[0];
            }
            return $return . $this->dumpValue(new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments)) . $tail;
        }
        $arguments = [];
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_array($callable)) {
                if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $callable[1])) {
                    throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $callable[1] ?: 'n/a'));
                }
                if ($callable[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
                ...
                if (\strncmp($class, "'", \strlen("'")) === 0 && \strpos($class, '$') === \false) {
                    if ("''" === $class) {
                        throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition: "%s" service is defined to be created by a factory but is missing the service reference, did you forget to define the factory service id or class?', $id ? 'The "' . $id . '"' : 'inline'));
                    }
        ...
        if (null === ($class = $definition->getClass())) {
            throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have no class nor factory.');
        }
...
use RectorPrefix20210904\\Symfony\\Component\\DependencyInjection\\Exception\\LogicException;
use RectorPrefix20210904\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
use RectorPrefix20210904\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;
...
EOF;
        ...
        if (\RectorPrefix20210904\Symfony\Component\DependencyInjection\Container::class !== $this->baseClass) {
            $r = $this->container->getReflectionClass($this->baseClass, \false);
            if (null !== $r && null !== ($constructor = $r->getConstructor()) && 0 === $constructor->getNumberOfRequiredParameters() && \RectorPrefix20210904\Symfony\Component\DependencyInjection\Container::class !== $constructor->getDeclaringClass()->name) {
...
EOF;
    ...
    private function addRemovedIds() : string
    {
        ...
        if ($this->asFiles) {
            ...
            \sort($ids);
            foreach ($ids as $id) {
                if (\preg_match(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Loader\FileLoader::ANONYMOUS_ID_REGEXP, $id)) {
                    continue;
                }
                $code .= '            ' . $this->doExport($id) . " => true,\n";
            }
...
EOF;
    ...
    private function exportParameters(array $parameters, string $path = '', int $indent = 12) : string
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Argument\ArgumentInterface) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain special arguments. "%s" found in "%s".', \get_debug_type($value), $path . '/' . $key));
            } elseif ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Variable) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".', $value, $path . '/' . $key));
            } elseif ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".', $value->getClass(), $path . '/' . $key));
            } elseif ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").', $value, $path . '/' . $key));
            } elseif ($value instanceof \RectorPrefix20210904\Symfony\Component\ExpressionLanguage\Expression) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain expressions. Expression "%s" found in "%s".', $value, $path . '/' . $key));
            } else {
...
EOF;
    ...
    {
        $conditions = [];
        foreach (\RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerBuilder::getInitializedConditionals($value) as $service) {
        ...
        foreach (\RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerBuilder::getServiceConditionals($value) as $service) {
        ...
        if (\is_array($value)) {
            ...
            try {
                if ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                    ...
                    $returnedType = '';
                    if ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\TypedReference) {
                        $returnedType = \sprintf(': %s\\%s', \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $value->getInvalidBehavior() ? '' : '?', $value->getType());
                    }
                ...
                if ($value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                    $serviceMap = '';
                    $serviceTypes = '';
                    foreach ($value->getValues() as $k => $v) {
                        ...
                        $serviceTypes .= \sprintf("\n            %s => %s,", $this->export($k), $this->export($v instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\TypedReference ? $v->getType() : '?'));
            ...
            if ($value->getMethodCalls()) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have method calls.');
            }
            if ($value->getProperties()) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have properties.');
            }
            if (null !== $value->getConfigurator()) {
                throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have a configurator.');
            }
    ...
    private function dumpLiteralClass(string $class) : string
    {
        ...
        if (\strncmp($class, "'", \strlen("'")) !== 0 || !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \RectorPrefix20210904\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid class name (%s).', $class ?: 'n/a'));
        }
    ...
    private function getServiceCall(string $id, \RectorPrefix20210904\Symfony\Component\DependencyInjection\Reference $reference = null) : string
    {
        ...
        if ($this->container->hasDefinition($id) && ($definition = $this->container->getDefinition($id))) {
            ...
            } elseif (null !== $reference && \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
                if (!$definition->isShared()) {
                    return $code;
                }
        ...
        if (null !== $reference && \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
            return 'null';
        }
        if (null !== $reference && \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $reference->getInvalidBehavior()) {
            $code = \sprintf('$this->get(%s, /* ContainerInterface::NULL_ON_INVALID_REFERENCE */ %d)', $this->doExport($id), \RectorPrefix20210904\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE);
        } else {
    ...
    private function generateMethodName(string $id) : string
    {
        ...
        $i = \strrpos($id, '\\');
        $name = \RectorPrefix20210904\Symfony\Component\DependencyInjection\Container::camelize(\false !== $i && isset($id[1 + $i]) ? \substr($id, 1 + $i) : $id);
    ...
    private function getExpressionLanguage() : \RectorPrefix20210904\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            ...
            $this->expressionLanguage = new \RectorPrefix20210904\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $providers, function ($arg) {
    ...
    private function isHotPath(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        return $this->hotPathTag && $definition->hasTag($this->hotPathTag) && !$definition->isDeprecated();
    }
    private function isSingleUsePrivateNode(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode $node) : bool
    {
        ...
        foreach ($node->getInEdges() as $edge) {
            ...
            if ($edge->isLazy() || !$value instanceof \RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition || !$value->isShared()) {
                return \false;
            }
    ...
    private function getAutoloadFile() : ?string
    {
        ...
        foreach (\spl_autoload_functions() as $autoloader) {
            ...
            if ($autoloader[0] instanceof \RectorPrefix20210904\Symfony\Component\ErrorHandler\DebugClassLoader || $autoloader[0] instanceof \RectorPrefix20210904\Symfony\Component\Debug\DebugClassLoader) {
                $autoloader = $autoloader[0]->getClassLoader();
            }
            if (!\is_array($autoloader) || !$autoloader[0] instanceof \RectorPrefix20210904\Composer\Autoload\ClassLoader || !$autoloader[0]->findFile(__CLASS__)) {
                continue;
            }
    ...
    private function getClasses(\RectorPrefix20210904\Symfony\Component\DependencyInjection\Definition $definition, string $id) : array
    {
        $classes = [];
