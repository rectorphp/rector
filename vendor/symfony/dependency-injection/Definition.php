...
class Definition
{
    ...
    public function setFactory($factory)
    {
        ...
        } elseif ($factory instanceof \RectorPrefix20210728\Symfony\Component\DependencyInjection\Reference) {
     ...
     */
    public function setDecoratedService($id, $renamedId = null, $priority = 0, $invalidBehavior = \RectorPrefix20210728\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE)
    {
        if ($renamedId && $id === $renamedId) {
            throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The decorated service inner name for "%s" must be different than the service name itself.', $id));
        }
    ...
    public function replaceArgument($index, $argument)
    {
        if (0 === \count($this->arguments)) {
            throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException('Cannot replace arguments if none have been configured yet.');
        }
        if (\is_int($index) && ($index < 0 || $index > \count($this->arguments) - 1)) {
            throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The index "%d" is not in the range [0, %d].', $index, \count($this->arguments) - 1));
        }
        if (!\array_key_exists($index, $this->arguments)) {
            throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The argument "%s" doesn\'t exist.', $index));
        }
    ...
    {
        if (empty($method)) {
            throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Method name cannot be empty.');
        }
    ...
    public function setDeprecated()
    {
        ...
        if ('' !== $message) {
            ...
            if (\strpos($message, '%service_id%') === \false) {
                throw new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%service_id%" placeholder.');
            }
        }
    ...
    public function setConfigurator($configurator)
    {
        ...
        } elseif ($configurator instanceof \RectorPrefix20210728\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setBindings($bindings)
    {
        foreach ($bindings as $key => $binding) {
            ...
            if (!$binding instanceof \RectorPrefix20210728\Symfony\Component\DependencyInjection\Argument\BoundArgument) {
                $bindings[$key] = new \RectorPrefix20210728\Symfony\Component\DependencyInjection\Argument\BoundArgument($binding);
            }
        }
