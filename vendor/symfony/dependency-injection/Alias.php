...
class Alias
{
    ...
    public function setDeprecated()
    {
        ...
        if ('' !== $message) {
            if (\preg_match('#[\\r\\n]|\\*/#', $message)) {
                throw new \RectorPrefix20211016\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Invalid characters found in deprecation template.');
            }
            if (\strpos($message, '%alias_id%') === \false) {
                throw new \RectorPrefix20211016\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%alias_id%" placeholder.');
            }
        }
