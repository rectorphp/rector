...
class Alias
{
    ...
    public function setDeprecated()
    {
        ...
        if ('' !== $message) {
            if (\preg_match('#[\\r\\n]|\\*/#', $message)) {
                throw new \RectorPrefix20210610\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Invalid characters found in deprecation template.');
            }
            if (\false === \strpos($message, '%alias_id%')) {
                throw new \RectorPrefix20210610\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%alias_id%" placeholder.');
            }
        }
