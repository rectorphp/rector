...
class Autowire
{
    ...
    public function __construct(?string $value = null, ?string $service = null, ?string $expression = null)
    {
        if (!($service xor $expression xor null !== $value)) {
            throw new \RectorPrefix20220607\Symfony\Component\DependencyInjection\Exception\LogicException('#[Autowire] attribute must declare exactly one of $service, $expression, or $value.');
        }
        ...
        switch (\true) {
            case null !== $service:
                $this->value = new \RectorPrefix20220607\Symfony\Component\DependencyInjection\Reference($service);
                break;
            case null !== $expression:
                if (!\class_exists(\RectorPrefix20220607\Symfony\Component\ExpressionLanguage\Expression::class)) {
                    throw new \RectorPrefix20220607\Symfony\Component\DependencyInjection\Exception\LogicException('Unable to use expressions as the Symfony ExpressionLanguage component is not installed. Try running "composer require symfony/expression-language".');
                }
                $this->value = new \RectorPrefix20220607\Symfony\Component\ExpressionLanguage\Expression($expression);
                break;
            case null !== $value:
                $this->value = $value;
                break;
