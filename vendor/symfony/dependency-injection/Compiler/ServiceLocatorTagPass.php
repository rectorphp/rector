 ...
 */
final class ServiceLocatorTagPass extends \RectorPrefix20220604\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (!$value instanceof \RectorPrefix20220604\Symfony\Component\DependencyInjection\Definition || !$value->hasTag('container.service_locator')) {
            return parent::processValue($value, $isRoot);
        }
        if (!$value->getClass()) {
            $value->setClass(\RectorPrefix20220604\Symfony\Component\DependencyInjection\ServiceLocator::class);
        }
        $services = $value->getArguments()[0] ?? null;
        if ($services instanceof \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {
            $services = $this->findAndSortTaggedServices($services, $this->container);
        }
        if (!\is_array($services)) {
            throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": an array of references is expected as first argument when the "container.service_locator" tag is set.', $this->currentId));
        }
        $i = 0;
        foreach ($services as $k => $v) {
            ...
            $services[$k] = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument($v);
        }
        \ksort($services);
        $value->setArgument(0, $services);
        $id = '.service_locator.' . \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder::hash($value);
        if ($isRoot) {
            if ($id !== $this->currentId) {
                $this->container->setAlias($id, new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Alias($this->currentId, \false));
            }
            return $value;
        }
    ...
    public static function register(\RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder $container, array $refMap, string $callerId = null) : \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference
    {
        ...
        if (!$container->hasDefinition($id = '.service_locator.' . \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder::hash($locator))) {
        ...
        if (null !== $callerId) {
            ...
            $container->register($id .= '.' . $callerId, \RectorPrefix20220604\Symfony\Component\DependencyInjection\ServiceLocator::class)->setFactory([new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference($locatorId), 'withContext'])->addTag('container.service_locator_context', ['id' => $callerId])->addArgument($callerId)->addArgument(new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference('service_container'));
        }
        return new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference($id);
    }
}
