...
final class AttributeAutoconfigurationPass extends \RectorPrefix20210822\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    {
        if (!$value instanceof \RectorPrefix20210822\Symfony\Component\DependencyInjection\Definition || !$value->isAutoconfigured() || $value->isAbstract() || $value->hasTag('container.ignore_attributes') || !($reflector = $this->container->getReflectionClass($value->getClass(), \false))) {
            return parent::processValue($value, $isRoot);
        }
        $autoconfiguredAttributes = $this->container->getAutoconfiguredAttributes();
        $instanceof = $value->getInstanceofConditionals();
        $conditionals = $instanceof[$reflector->getName()] ?? new \RectorPrefix20210822\Symfony\Component\DependencyInjection\ChildDefinition('');
        foreach ($reflector->getAttributes() as $attribute) {
            if ($configurator = $autoconfiguredAttributes[$attribute->getName()] ?? null) {
                $configurator($conditionals, $attribute->newInstance(), $reflector);
            }
        }
        if (!isset($instanceof[$reflector->getName()]) && new \RectorPrefix20210822\Symfony\Component\DependencyInjection\ChildDefinition('') != $conditionals) {
