...
use RectorPrefix20220604\Symfony\Component\DependencyInjection\Attribute\AsTaggedItem;
...
trait PriorityTaggedServiceTrait
{
    ...
    private function findAndSortTaggedServices($tagName, \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder $container) : array
    {
        ...
        foreach ($services as [, , $index, $serviceId, $class]) {
            if (!$class) {
                $reference = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Reference($serviceId);
            } elseif ($index === $serviceId) {
                $reference = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\TypedReference($serviceId, $class);
            } else {
                $reference = new \RectorPrefix20220604\Symfony\Component\DependencyInjection\TypedReference($serviceId, $class, \RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder::EXCEPTION_ON_INVALID_REFERENCE, $index);
            }
            if (null === $index) {
                $refs[] = $reference;
            } else {
...
class PriorityTaggedServiceUtil
{
     ...
     */
    public static function getDefault(\RectorPrefix20220604\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $serviceId, string $class, string $defaultMethod, string $tagName, ?string $indexAttribute, bool $checkTaggedItem)
    {
        ...
        if ($checkTaggedItem && !$r->hasMethod($defaultMethod)) {
            foreach (\method_exists($r, 'getAttributes') ? $r->getAttributes(\RectorPrefix20220604\Symfony\Component\DependencyInjection\Attribute\AsTaggedItem::class) : [] as $attribute) {
        ...
        if (!($rm = $r->getMethod($defaultMethod))->isStatic()) {
            throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode('be static', $message));
        }
        if (!$rm->isPublic()) {
            throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode('be public', $message));
        }
        $default = $rm->invoke(null);
        if ('priority' === $indexAttribute) {
            if (!\is_int($default)) {
                throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode(\sprintf('return int (got "%s")', \get_debug_type($default)), $message));
            }
            return $default;
        }
        if (\is_int($default)) {
        ...
        if (!\is_string($default)) {
            throw new \RectorPrefix20220604\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode(\sprintf('return string|int (got "%s")', \get_debug_type($default)), $message));
        }
        return $default;
    }
}
