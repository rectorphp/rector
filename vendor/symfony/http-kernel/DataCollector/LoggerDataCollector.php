 ...
 * @final
 */
class LoggerDataCollector extends \RectorPrefix20210720\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \RectorPrefix20210720\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
    private $containerPathPrefix;
    private $currentRequest;
    private $requestStack;
    public function __construct($logger = null, string $containerPathPrefix = null, \RectorPrefix20210720\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \RectorPrefix20210720\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \RectorPrefix20210720\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \RectorPrefix20210720\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \RectorPrefix20210720\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
