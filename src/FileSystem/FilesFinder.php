...
use RectorPrefix20210525\Nette\Caching\Cache;
use RectorPrefix20210525\Nette\Utils\Strings;
use Rector\Core\Configuration\Configuration;
use RectorPrefix20210525\Symfony\Component\Finder\Finder;
use RectorPrefix20210525\Symfony\Component\Finder\SplFileInfo;
use RectorPrefix20210525\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver;
use RectorPrefix20210525\Symplify\SmartFileSystem\FileSystemFilter;
use RectorPrefix20210525\Symplify\SmartFileSystem\Finder\FinderSanitizer;
...
final class FilesFinder
{
     ...
     */
    private $cache;
    public function __construct(\Rector\Core\FileSystem\FilesystemTweaker $filesystemTweaker, \RectorPrefix20210525\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \RectorPrefix20210525\Symplify\SmartFileSystem\FileSystemFilter $fileSystemFilter, \RectorPrefix20210525\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver $skippedPathsResolver, \Rector\Core\Configuration\Configuration $configuration, \RectorPrefix20210525\Nette\Caching\Cache $cache)
    {
    ...
    public function findInDirectoriesAndFiles(array $source, array $suffixes) : array
    {
        ...
        if (!$this->configuration->isCacheEnabled() || $this->configuration->shouldClearCache()) {
            $this->cache->clean([\RectorPrefix20210525\Nette\Caching\Cache::ALL => \true]);
    ...
    private function findInDirectories(array $directories, array $suffixes) : array
    {
        ...
        $finder = \RectorPrefix20210525\Symfony\Component\Finder\Finder::create()->followLinks()->files()->size('> 0')->in($directories)->name($suffixesPattern)->sortByName();
    ...
    private function addFilterWithExcludedPaths(\RectorPrefix20210525\Symfony\Component\Finder\Finder $finder) : void
    {
        $excludePaths = $this->skippedPathsResolver->resolve();
        if ($excludePaths === []) {
            return;
        }
        $finder->filter(function (\RectorPrefix20210525\Symfony\Component\Finder\SplFileInfo $splFileInfo) use($excludePaths) : bool {
            ...
            foreach ($excludePaths as $excludePath) {
                ...
                if (\RectorPrefix20210525\Nette\Utils\Strings::match($realPath, '#' . \preg_quote($excludePath, '#') . '#')) {
                    return \false;
                }
    ...
    {
        // ends with *
        if (\RectorPrefix20210525\Nette\Utils\Strings::match($path, self::ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\RectorPrefix20210525\Nette\Utils\Strings::match($path, self::STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        return $path;
    }
