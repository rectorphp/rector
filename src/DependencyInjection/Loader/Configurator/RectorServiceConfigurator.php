...
use RectorPrefix20220120\Symfony\Component\DependencyInjection\Loader\Configurator\ServiceConfigurator;
use RectorPrefix20220120\Symplify\SymfonyPhpConfig\ValueObjectInliner;
/**
 * @api
 * Same as Symfony service configurator, with extra "configure()" method for easier DX
 */
final class RectorServiceConfigurator extends \RectorPrefix20220120\Symfony\Component\DependencyInjection\Loader\Configurator\ServiceConfigurator
{
    /**
    ...
    public function configure(array $configuration) : self
    {
        ...
        \array_walk_recursive($configuration, function (&$value) {
            if (\is_object($value)) {
                $value = \RectorPrefix20220120\Symplify\SymfonyPhpConfig\ValueObjectInliner::inline($value);
            }
            return $value;
        });
    ...
    {
        if ($class === null) {
            throw new \RectorPrefix20220120\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException('The class is missing');
        }
        if (!\is_a($class, \Rector\Core\Contract\Rector\ConfigurableRectorInterface::class, \true)) {
            ...
            throw new \RectorPrefix20220120\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($errorMessage);
        }
    }
}
