...
use RectorPrefix20220214\Symplify\EasyParallel\CpuCoreCountProvider;
use RectorPrefix20220214\Symplify\EasyParallel\Exception\ParallelShouldNotHappenException;
use RectorPrefix20220214\Symplify\EasyParallel\FileSystem\FilePathNormalizer;
use RectorPrefix20220214\Symplify\EasyParallel\ScheduleFactory;
...
final class ApplicationFileProcessor
{
     ...
     */
    public function __construct(\RectorPrefix20220214\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \Rector\Core\Application\FileDecorator\FileDiffFileDecorator $fileDiffFileDecorator, \Rector\FileFormatter\FileFormatter $fileFormatter, \Rector\Core\Application\FileSystem\RemovedAndAddedFilesProcessor $removedAndAddedFilesProcessor, \Rector\Core\Contract\Console\OutputStyleInterface $rectorOutputStyle, \Rector\Core\ValueObjectFactory\Application\FileFactory $fileFactory, \PHPStan\Analyser\NodeScopeResolver $nodeScopeResolver, \RectorPrefix20220214\Symplify\PackageBuilder\Yaml\ParametersMerger $parametersMerger, \Rector\Parallel\Application\ParallelFileProcessor $parallelFileProcessor, \RectorPrefix20220214\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider, \RectorPrefix20220214\Symplify\EasyParallel\ScheduleFactory $scheduleFactory, \RectorPrefix20220214\Symplify\EasyParallel\FileSystem\FilePathNormalizer $filePathNormalizer, \RectorPrefix20220214\Symplify\EasyParallel\CpuCoreCountProvider $cpuCoreCountProvider, array $fileProcessors = [])
    {
     ...
     */
    public function run(\Rector\Core\ValueObject\Configuration $configuration, \RectorPrefix20220214\Symfony\Component\Console\Input\InputInterface $input) : array
    {
     ...
     * @return array{system_errors: SystemError[], file_diffs: FileDiff[]}
     */
    private function runParallel(array $fileInfos, \Rector\Core\ValueObject\Configuration $configuration, \RectorPrefix20220214\Symfony\Component\Console\Input\InputInterface $input) : array
    {
        ...
        if ($mainScript === null) {
            throw new \RectorPrefix20220214\Symplify\EasyParallel\Exception\ParallelShouldNotHappenException('[parallel] Main script was not found');
        }
    ...
    {
        \RectorPrefix20220214\Webmozart\Assert\Assert::allIsAOf($files, \Rector\Core\ValueObject\Application\File::class);
        $filePaths = [];
        foreach ($files as $file) {
            $smartFileInfo = $file->getSmartFileInfo();
