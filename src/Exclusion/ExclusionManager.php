...
final class ExclusionManager
{
    ...
    private function matchesNoRectorComment(\PhpParser\Node $node, string $rectorClass) : bool
    {
        foreach ($node->getComments() as $comment) {
            if (\RectorPrefix20210821\Nette\Utils\Strings::match($comment->getText(), self::NO_RECTOR_START_REGEX)) {
                return \true;
            }
            $noRectorWithRule = '#@noRector \\\\?' . \preg_quote($rectorClass, '#') . '$#';
            if (\RectorPrefix20210821\Nette\Utils\Strings::match($comment->getText(), $noRectorWithRule)) {
                return \true;
            }
        }
        return \false;
