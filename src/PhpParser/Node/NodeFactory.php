...
use RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder;
use RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder;
use RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder;
/**
...
final class NodeFactory
{
    ...
    {
        $methodBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder($name);
        $methodBuilder->makePublic();
        return $methodBuilder->getNode();
    }
    public function createParamFromNameAndType(string $name, ?\PHPStan\Type\Type $type) : \PhpParser\Node\Param
    {
        $paramBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder($name);
        if ($type !== null) {
    ...
    {
        $propertyBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder($name);
    ...
    {
        return new \PhpParser\Node\Expr\StaticCall(new \PhpParser\Node\Name(\Rector\Core\Enum\ObjectReference::PARENT), new \PhpParser\Node\Identifier(\Rector\Core\ValueObject\MethodName::CONSTRUCT), $this->createArgsFromParams($params));
    }
    ...
    {
        $methodBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder('get' . \ucfirst($propertyName));
        $methodBuilder->makePublic();
    ...
    {
        $methodBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder('set' . \ucfirst($propertyName));
        $methodBuilder->makePublic();
        $variable = new \PhpParser\Node\Expr\Variable($propertyName);
    ...
    {
        $name = new \PhpParser\Node\Name(\Rector\Core\Enum\ObjectReference::SELF);
    ...
    {
        $paramBuilder = new \RectorPrefix20220604\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder($propertyMetadata->getName());
    ...
    public function createClassConstFetchFromName(\PhpParser\Node\Name $className, string $constantName) : \PhpParser\Node\Expr\ClassConstFetch
    {
        ...
        if (\in_array($classNameString, [\Rector\Core\Enum\ObjectReference::SELF, \Rector\Core\Enum\ObjectReference::STATIC], \true)) {
