...
final class WorkerCommand extends \Rector\Core\Console\Command\AbstractProcessCommand
{
    ...
    protected function execute(\RectorPrefix20220125\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20220125\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $promise->then(function (\RectorPrefix20220125\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \RectorPrefix20220125\Clue\React\NDJson\Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            $outEncoder = new \RectorPrefix20220125\Clue\React\NDJson\Encoder($connection, \JSON_INVALID_UTF8_IGNORE);
            // handshake?
            $outEncoder->write([\RectorPrefix20220125\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \RectorPrefix20220125\Symplify\EasyParallel\Enum\Action::HELLO, \RectorPrefix20220125\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
