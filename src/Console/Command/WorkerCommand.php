...
use RectorPrefix20220209\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Rector\Core\Console\Command\AbstractProcessCommand
{
    ...
    protected function execute(\RectorPrefix20220209\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20220209\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $tcpConnector = new \RectorPrefix20220209\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\RectorPrefix20220209\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \RectorPrefix20220209\Clue\React\NDJson\Decoder($connection, \true, 512, 0);
            $outEncoder = new \RectorPrefix20220209\Clue\React\NDJson\Encoder($connection, 0);
            // handshake?
            $outEncoder->write([\RectorPrefix20220209\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \RectorPrefix20220209\Symplify\EasyParallel\Enum\Action::HELLO, \RectorPrefix20220209\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
