...
use RectorPrefix20220604\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Rector\Core\Console\Command\AbstractProcessCommand
{
    ...
    protected function execute(\RectorPrefix20220604\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20220604\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $tcpConnector = new \RectorPrefix20220604\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\RectorPrefix20220604\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \RectorPrefix20220604\Clue\React\NDJson\Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            $outEncoder = new \RectorPrefix20220604\Clue\React\NDJson\Encoder($connection, \JSON_INVALID_UTF8_IGNORE);
            // handshake?
            $outEncoder->write([\RectorPrefix20220604\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \RectorPrefix20220604\Symplify\EasyParallel\Enum\Action::HELLO, \RectorPrefix20220604\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
